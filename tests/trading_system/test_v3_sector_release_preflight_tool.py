...
from tools import preflight_v3_sector_release as subject
...
def test_current_preflight_requires_release_manifest(
    ...
    with pytest.raises(ValueError, match="requires v3_release_manifest"):
...
def test_current_preflight_exposes_verified_release_receipt(
    ...
    artifact = tmp_path / "result.json"
    manifest = tmp_path / "v3_release_manifest.json"
    artifact.write_text("{}", encoding="utf-8")
