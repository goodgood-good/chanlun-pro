...
def test_capture_records_normal_and_suspended_without_same_session_authority(
    ...
    assert document["point_in_time_scope"] == ("SUBSEQUENT_SESSION_DECISIONS_ONLY")
...
def test_incomplete_capture_is_retried_and_old_attempt_remains_immutable(
    ...
    assert first.snapshot.errors[0].reason_code == ("QMT_INSTRUMENT_DETAIL_UNAVAILABLE")
    ...
    assert (
        QmtInstrumentStatusSnapshot.from_document(
