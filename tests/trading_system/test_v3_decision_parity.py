...
from chanlun.decision_support.trading_system.v3_portfolio import CycleLedger
...
def facts(*, ledger: CycleLedger | None = None) -> V3DecisionInput:
    return V3DecisionInput(
        symbol="SH.600000",
        decision_time=NOW,
        confirmation_time=NOW - timedelta(minutes=1),
        structure_snapshot_id="structure:v1",
        selection_snapshot_id="selection:v1",
        account_snapshot_id="account:v1",
...
def test_trading_continuity_exit_precedes_technical_exit() -> None:
    ...
    assert intent.rule_id == "V3_PRIORITY_02_TRADING_CONTINUITY_LOST"
