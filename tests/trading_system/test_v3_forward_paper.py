...
PARAMETER_FIXTURE_ROOT = (
    Path(__file__).resolve().parents[1] / "fixtures" / "v3_forward_paper"
)
PARAMETER_SOURCE = PARAMETER_FIXTURE_ROOT / "parameter_snapshot.json"
APPROX_PARAMETER_SOURCE = PARAMETER_SOURCE.with_name(
    "parameter_snapshot_approximate.json"
)
HUMAN_PARAMETER_SOURCE = PARAMETER_SOURCE.with_name(
...
def test_forward_contract_fixtures_are_test_owned() -> None:
    ...
    for path in (
        PARAMETER_SOURCE,
        APPROX_PARAMETER_SOURCE,
        HUMAN_PARAMETER_SOURCE,
    ):
        assert path.is_file()
    ...
    gate = {
        ...
        "attestation_audit": {
            ...
            "selection_evidence_ids": (
                ["sha256:" + "d" * 64] if not blocked else []
            ),
            "catalog_entry_sha256s": (
                ["sha256:" + "e" * 64] if not blocked else []
            ),
        ...
        "paper_ledger_prefix_archive": {
            ...
            "path": (
                "objects/human_paper_ledger_prefix/"
                + ledger_hash[7:]
                + ".json"
            ),
...
def test_entry_selection_settlement_gate_artifact_semantics() -> None:
    assert _human_paper_entry_selection_gate_proven(
        _selection_settlement()
    )
    assert _human_paper_entry_selection_gate_proven(
        _selection_settlement(blocked=True)
    )
    # Old immutable sessions predate the nested gate and remain readable.
    assert _human_paper_entry_selection_gate_proven(
        {"status": "NO_PENDING_VIRTUAL_INTENTS"}
    )

    forged = json.loads(json.dumps(_selection_settlement()))
    forged["entry_selection_settlement_gate"][
...
def audit_forward_paper_session_delivery(*args, **kwargs):
    """Give legacy delivery cases explicit, proven calendar evidence."""
...
def test_forward_contract_binds_frozen_strategy_without_live_transport() -> None:
    ...
    assert contract.technical_mode == "STRICT_DIRECT_RECURSION"
    assert "technical_mode" not in contract.document()


def test_approximate_forward_contract_binds_both_technical_identities() -> None:
    contract = load_frozen_forward_contract(APPROX_PARAMETER_SOURCE)

    assert contract.strategy_parameter_set_id == FROZEN_RESEARCH_PARAMETER_SET_ID
    assert contract.technical_mode == "RESEARCH_APPROXIMATION"
    assert (
    ...
    assert contract.document()["technical_mode"] == "RESEARCH_APPROXIMATION"
    assert contract.live_status == "LIVE_DISABLED"


def test_human_review_forward_contract_is_separate_and_has_no_order_authority() -> None:
    contract = load_frozen_forward_contract(HUMAN_PARAMETER_SOURCE)
    ...
    assert contract.real_account_access is False
    assert contract.real_order_transport is False
    assert contract.document()["schema"].endswith("/v3")
...
def test_forward_ledger_appends_recovery_after_a_later_phase_block(
    ...
    assert recovered["previous_event_sha256"] == ledger["events"][-2][
        "event_sha256"
    ]
...
def test_forward_implementation_continuity_preflight_is_fail_closed_and_recoverable(
    ...
    legacy = _forward_session_events(
        tmp_path / "legacy-preflight",
        ("CAPTURE", "CAPTURED", datetime(2026, 7, 30, 9, 12, tzinfo=CN)),
        implementation_provenance=False,
    )
    legacy_result = audit_forward_implementation_continuity(
        legacy,
        session=session,
        current_implementation_provenance=current,
    )
    assert legacy_result["reason_code"] == (
    ...
    changed = audit_forward_implementation_continuity(
        captured,
        session=session,
        current_implementation_provenance=_implementation_provenance(
            source_digit="b"
        ),
    )
...
def test_forward_delivery_requires_both_capture_and_evaluated_events(
    ...
    assert complete["capture_implementation_provenance_id"] == (
...
def test_forward_delivery_rejects_legacy_and_mixed_implementation_provenance(
    tmp_path: Path,
) -> None:
    capture_evidence, capture_readiness = _capture_delivery_evidence()
    statuses = (
    ...
    legacy_result = audit_forward_paper_session_delivery(
        legacy,
        session=date(2026, 7, 30),
        observed_at=datetime(2026, 7, 30, 15, 25, tzinfo=CN),
        sector_capture_readiness=capture_readiness,
    )
    assert legacy_result["ready"] is False
    assert legacy_result["reason_code"] == (
        "IMPLEMENTATION_PROVENANCE_UNATTESTED"
    )
    assert legacy_result["implementation_provenance_present"] is False
    assert legacy_result["implementation_provenance_proven"] is False
