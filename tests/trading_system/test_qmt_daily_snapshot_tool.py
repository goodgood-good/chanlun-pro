...
def _single_receipt(receipts: Path, session: str) -> Path:
    candidates = tuple(sorted((receipts / session).glob("*.json")))
...
def test_daily_snapshot_is_idempotent_and_reconstructs_prior_prefix(
    ...
    first_receipt_path = _single_receipt(receipts, "2026-07-27")
    ...
    second_receipt_path = _single_receipt(receipts, "2026-07-28")
    ...
    audit = audit_sector_capture_receipts(
        output=output,
    ...
    required = audit_sector_capture_receipts(
        output=output,
        receipt_dir=receipts,
    )
    assert required["required_capture_session"] is None
    due_missing = audit_sector_capture_receipts(
        output=output,
        receipt_dir=receipts,
    ...
    receipt_bytes = second_receipt_path.read_bytes()
    assert subject.main(argv) == 0
    ...
    assert second_receipt_path.read_bytes() == receipt_bytes
...
def test_receipt_audit_requires_a_receipt_for_every_ledger_entry(
    monkeypatch,
    tmp_path: Path,
) -> None:
    output = tmp_path / "ledger.json"
    ...
    assert audit["required_missing_capture_sessions"] == ("2026-07-27",)
...
def test_receipt_audit_rejects_safety_tampering(
    ...
    receipt_path = _single_receipt(receipts, "2026-07-27")
