...
import pandas as pd
import tools.backtest_sector_first_full_market as FULL_MARKET
...
from tools.backtest_sector_first_full_market import (
    SymbolContext,
    _entry_dispatch_at,
    _execution_bars,
    _fingerprint_value,
...
def _warmup_semantic_documents(
    ...
    empty_supply = WarmupMappingSupplySnapshot(
        periods=(("M", None), ("W", None), ("D", None))
    )
    envelope = bind_warmup_mapping_supply_diagnostic(
        envelope,
        snapshots=(empty_supply,) * len(observations),
    )
    empty_lineage = WarmupStructureLineageSnapshotSet(
        periods=(("M", None), ("W", None), ("D", None))
    )
    envelope = bind_warmup_structure_lineage_diagnostic(
        envelope,
        snapshots=(empty_lineage,) * len(observations),
    )
        ...
        return WarmupSemanticSnapshot(
            periods=tuple(
                WarmupPeriodSemanticFacts(
                    ...
                    mapped_center_id=(center_id if period == "W" and unique else None),
                    mapping_candidate_ids=(
...
def _effectiveness_evidence(
    ...
    *,
    ...
    semantic = WarmupSemanticSnapshot(
        periods=tuple(
            WarmupPeriodSemanticFacts(
                period=period,
                state="NONE",
    ...
    observations = tuple(
        WarmupPrefixObservation(
            bar_count=count,
            starts_at=observed_at - timedelta(days=count),
            signature_sha256=semantic.signature_sha256,
        )
    ...
    convergence = bind_warmup_convergence_diagnostic(
        convergence,
        snapshots=(semantic,) * len(observations),
    )
    empty_supply = WarmupMappingSupplySnapshot(
        periods=(("M", None), ("W", None), ("D", None))
    )
    ...
    evidence: dict[str, object] = {
        ...
        "daily": state,
        "period_diagnostics": [
            _period_diagnostic_document(
                period=period,
                state=("NONE" if state == "UNRESOLVED" else state),
                observed_at=observed_at,
            )
            for period in ("M", "W", "D")
        ],
    ...
    if subject == "sector":
        ...
        coverage = QmtSectorSameBaseCoverageEvidence(
            ...
            physical_source_requested_start_at=datetime(2023, 5, 1, 9, 30, tzinfo=CN),
        ...
        evidence.update(
            {
                ...
                "strict_same_5m_warmup_convergence": convergence.document(),
...
def _period_diagnostic_document(
    *,
    period: str,
    state: str,
    observed_at: datetime,
    active_top_interval: tuple[str, str] | None = None,
    mapping_supply: dict[str, object] | None = None,
    blocker_codes: tuple[str, ...] = (),
) -> dict[str, object]:
    supply = (
        None
        if mapping_supply is None
        else RiskMappingSupplyFacts.from_document(mapping_supply)
    )
    interval = (
        None
        if active_top_interval is None
        else tuple(datetime.fromisoformat(value) for value in active_top_interval)
    )
    candidate_ids = (
        ()
        if supply is None or supply.highest_candidate_center_count == 0
        else tuple(
            dict.fromkeys(
                point.center_id
                for point in supply.point_evidence
                if point.highest_mapping_candidate
            )
        )
    )
    mapping_unique = state == "NONE" or bool(
        supply is not None and supply.classification == "UNIQUE_MAPPING"
    )
    mapped_center_id = candidate_ids[0] if state != "NONE" and mapping_unique else None
    return HigherTimeframePeriodDiagnostic(
        period=period,  # type: ignore[arg-type]
        state=state,
        completed_bar_count=480,
        evidence_bar_end=observed_at - timedelta(days=1),
        active_top_interval=interval,  # type: ignore[arg-type]
        mapping_unique=mapping_unique,
        mapped_center_id=mapped_center_id,
        ...
        source_revision=sha256_json(
            {
                ...
                "state": state,
                "observed_at": observed_at,
                "active_top_interval": active_top_interval,
...
def _effectiveness_row(
    ...
    *,
    ...
    row: dict[str, object] = {
        ...
        "accepted": all(
            value in {"GREEN", "AMBER"} for value in (market, sector, symbol)
        ),
    ...
    for subject, gate in (("market", market), ("sector", sector), ("symbol", symbol)):
        ...
        row[f"{subject}_risk_warmup_evidence"] = _effectiveness_evidence(
            state=("UNRESOLVED" if gate == "UNRESOLVED" else "NONE"),
            subject=subject,
            gate=gate,
            observed_at=observed_at,
        )
        if subject != "sector":
            row[f"{subject}_risk_native_daily_calendar_coverage_evidence"] = (
                _native_calendar_coverage(
                    symbol=("SH.000300" if subject == "market" else "SZ.000001"),
                    observed_at=observed_at,
                ).document()
            )
    return row
...
def test_higher_timeframe_effectiveness_separates_strict_and_research_gates() -> None:
    ...
    assert (
        audit["subjects"]["symbol"]["blocker_candidate_counts"][
    ...
    assert audit["subjects"]["sector"]["strict_same_base_warmup_reason_counts"] == {
        "QMT_HIGHER_TIMEFRAME_WARMUP_HISTORY_INSUFFICIENT": 2,
        "QMT_HIGHER_TIMEFRAME_WARMUP_TAIL_STABLE": 1,
    }
    assert audit["subjects"]["sector"]["strict_same_base_source_boundary_counts"] == {
...
def test_higher_timeframe_effectiveness_surfaces_multi_prefix_false_stability() -> None:
    ...
    convergence, semantic, supply, lineage = _warmup_semantic_documents(observed_at)
    for subject in ("market", "sector", "symbol"):
        evidence = row[f"{subject}_risk_warmup_evidence"]
        assert isinstance(evidence, dict)
        evidence["warmup"] = {"reason_code": "QMT_HIGHER_TIMEFRAME_WARMUP_TAIL_STABLE"}
        ...
        evidence["warmup_structure_lineage_diagnostic"] = copy.deepcopy(lineage)
    sector_evidence = row["sector_risk_warmup_evidence"]
    assert isinstance(sector_evidence, dict)
    sector_evidence["strict_same_5m_warmup_convergence"] = copy.deepcopy(convergence)
    sector_evidence["strict_same_5m_warmup_convergence_diagnostic"] = copy.deepcopy(
        semantic
    )
    sector_evidence["strict_same_5m_warmup_mapping_supply_diagnostic"] = copy.deepcopy(
        supply
    )
    sector_evidence["strict_same_5m_warmup_structure_lineage_diagnostic"] = (
    ...
    for subject in ("market", "sector", "symbol"):
        ...
        assert summary["pairwise_stable_without_all_prefix_stability_count"] == 1
...
def test_higher_timeframe_effectiveness_explains_non_monotonic_mwd_facts() -> None:
    row = _effectiveness_row(
        0,
        market="GREEN",
        sector="GREEN",
    ...
    observed_at = row["decision_at"]
    assert isinstance(observed_at, datetime)
    convergence, diagnostic, supply, lineage = _warmup_semantic_documents(observed_at)
    for subject in ("market", "sector", "symbol"):
        evidence = row[f"{subject}_risk_warmup_evidence"]
        assert isinstance(evidence, dict)
        evidence["warmup"] = {"reason_code": "QMT_HIGHER_TIMEFRAME_WARMUP_TAIL_STABLE"}
        evidence["warmup_convergence"] = copy.deepcopy(convergence)
        evidence["warmup_convergence_diagnostic"] = copy.deepcopy(diagnostic)
        evidence["warmup_mapping_supply_diagnostic"] = copy.deepcopy(supply)
        evidence["warmup_structure_lineage_diagnostic"] = copy.deepcopy(lineage)
    sector_evidence = row["sector_risk_warmup_evidence"]
    assert isinstance(sector_evidence, dict)
    ...
    assert audit["schema"] == ("chanlun-higher-timeframe-effectiveness-audit")
    for subject in ("market", "sector", "symbol"):
        summary = audit["subjects"][subject]["warmup_convergence"]
        assert summary["semantic_diagnostic_status_counts"] == {"NON_MONOTONIC": 1}
        ...
        point_audit = audit["subjects"][subject]["warmup_non_monotonic_point_audit"]
        ...
        assert all(
            value["diagnostic_only"] is True and value["active_gate_unchanged"] is True
            for value in point_audit["points"]
        )
    market_points = audit["subjects"]["market"]["warmup_non_monotonic_point_audit"][
        "points"
    ]
    assert {value["source_symbol"] for value in market_points} == {"SH.000001"}
    assert {tuple(value["changed_paths"]) for value in market_points} == {
        ("D.ma5",),
        ("W.state",),
    }
    daily = next(value for value in market_points if value["source_frequency"] == "d")
    weekly = next(value for value in market_points if value["source_frequency"] == "w")
...
def test_higher_timeframe_effectiveness_rejects_rehashed_semantic_diagnostic() -> None:
    row = _effectiveness_row(
        0,
        market="GREEN",
        sector="GREEN",
    ...
    diagnostic["observations"][1]["changed_paths_from_longest"] = ["M.state"]
    ...
    market["warmup_structure_lineage_diagnostic"] = lineage
...
def test_higher_timeframe_effectiveness_explains_mapping_supply_resegmentation() -> (
    ...
    observed_at = row["decision_at"]
    assert isinstance(observed_at, datetime)
    convergence, semantic, supply, lineage = _warmup_mapping_supply_documents(
        observed_at
    )
    for subject in ("market", "sector", "symbol"):
        evidence = row[f"{subject}_risk_warmup_evidence"]
    ...
    for subject in ("market", "sector", "symbol"):
        ...
        warmup_point = audit["subjects"][subject]["warmup_non_monotonic_point_audit"][
            "points"
        ][0]
        assert (
            warmup_point["mapping_supply_comparison"]["delta"]["prefix_classification"]
            == "UNIQUE_MAPPING"
        )
        supply_points = audit["subjects"][subject]["warmup_mapping_supply_point_audit"]
    ...
    market_points = audit["subjects"]["market"]["warmup_mapping_supply_point_audit"][
        "points"
    ]
...
def test_higher_timeframe_effectiveness_rejects_rehashed_mapping_supply_delta() -> None:
    row = _effectiveness_row(
        0,
        market="GREEN",
        sector="GREEN",
    ...
    market = row["market_risk_warmup_evidence"]
    assert isinstance(market, dict)
    supply["comparisons"][0]["delta"]["transition_codes"] = ["MAPPING_SUPPLY_UNCHANGED"]
    stable = dict(supply)
    stable.pop("content_sha256")
...
def test_higher_timeframe_effectiveness_rejects_rehashed_convergence_verdict() -> None:
    row = _effectiveness_row(
        0,
        market="GREEN",
        sector="GREEN",
    ...
    market = row["market_risk_warmup_evidence"]
    assert isinstance(market, dict)
    forged, semantic, supply, lineage = _warmup_semantic_documents(observed_at)
...
def test_higher_timeframe_effectiveness_counts_native_daily_calendar_gaps() -> None:
    row = _effectiveness_row(
        0,
        market="GREEN",
        sector="AMBER",
    ...
    row["symbol_risk_blocker_codes"] = ["QMT_NATIVE_DAILY_TRADING_CALENDAR_MISMATCH"]
    ...
    audit = higher_timeframe_effectiveness_audit((row,))
    symbol_coverage = audit["subjects"]["symbol"]["native_daily_calendar_coverage"]
    assert symbol_coverage["status_counts"] == {
        "UNEXPLAINED_CALENDAR_SESSION_MISSING": 1
    }
    assert symbol_coverage["unexplained_missing_session_occurrence_count"] == 1
    assert symbol_coverage["gap_rows"][0]["unexplained_calendar_only_sessions"] == [
        "2025-07-31"
    ]
    assert symbol_coverage["missing_session_interpretation"] == (
        "UNEXPLAINED_NEVER_INFERRED_AS_SUSPENSION"
    )
    assert audit["subjects"]["market"]["native_daily_calendar_coverage"][
        "status_counts"
    ] == {"EXACT": 1}
...
def test_higher_timeframe_effectiveness_rejects_missing_resolved_evidence() -> None:
    ...
    with pytest.raises(ValueError, match="market current risk evidence is required"):
        higher_timeframe_effectiveness_audit((row,))
...
def test_higher_timeframe_effectiveness_rejects_relabelled_period_or_exact_green() -> (
    ...
    relabelled["market_risk_warmup_evidence"]["monthly"] = "FORMED"
    ...
    unresolved["period_diagnostics"] = [
        _period_diagnostic_document(
            ...
            mapping_supply=_identified_third_sell_supply(
                inside=True, source_symbol="SZ.000001"
            ),
            blocker_codes=(f"{period}_CENTER_MAPPING_UNRESOLVED",),
        )
        for period in ("M", "W", "D")
    ]
    unresolved["warmup"] = {"reason_code": "QMT_HIGHER_TIMEFRAME_WARMUP_TAIL_DIVERGED"}
...
def test_higher_timeframe_effectiveness_audits_mapping_supply_without_promoting_third_points() -> (
    ...
    market["period_diagnostics"][0] = _period_diagnostic_document(
        period="M",
        state="FORMED_UNRESOLVED",
        observed_at=row["decision_at"],
        active_top_interval=(
        ...
        mapping_supply=_identified_third_sell_supply(inside=True),
        blocker_codes=("NO_COMPLETED_LOWER_1SELL_2SELL_CENTER_IN_TOP_FRACTAL",),
    )

    audit = higher_timeframe_effectiveness_audit((row,))
    subject = audit["subjects"]["market"]
    assert subject["mapping_supply_class_counts"] == {"ONLY_THIRD_CLASS_POINTS": 1}
    ...
    assert subject["diagnostic_directional_class_counts"] == {
        "NO_FIRST_SECOND_DIRECTIONAL_POINTS": 1
    }
    ...
    forged["market_risk_warmup_evidence"]["period_diagnostics"][0]["mapping_supply"][
        "point_type_counts"
    ]["1sell"] = 1
...
def test_higher_timeframe_effectiveness_surfaces_buy_side_directional_supply() -> None:
    row = _effectiveness_row(
        0,
        market="AMBER",
        sector="AMBER",
    ...
    market["period_diagnostics"][0] = _period_diagnostic_document(
        ...
        mapping_supply=_identified_diagnostic_buy_supply(inside=True),
        blocker_codes=("NO_COMPLETED_LOWER_1SELL_2SELL_CENTER_IN_TOP_FRACTAL",),
    )
    ...
    assert subject["mapping_supply_totals"]["diagnostic_buy_point_evidence_count"] == 1
...
def test_higher_timeframe_effectiveness_separates_repeated_exposure_from_unique_top_event() -> (
    ...
    supplies = (
        _identified_third_sell_supply(inside=True),
        _identified_sell12_supply(inside=False),
    )
    for row, supply in zip(rows, supplies):
        evidence = row["market_risk_warmup_evidence"]
        evidence["monthly"] = "FORMED_UNRESOLVED"
        evidence["period_diagnostics"][0] = _period_diagnostic_document(
            period="M",
            state="FORMED_UNRESOLVED",
            observed_at=row["decision_at"],
            active_top_interval=(
...
def _identified_third_sell_supply(
    *,
    ...
    return {
        ...
        "diagnostic_buy_point_type_counts": {"1buy": 0, "2buy": 0},
        "diagnostic_directional_classification": ("NO_FIRST_SECOND_DIRECTIONAL_POINTS"),
        "diagnostic_buy_point_evidence": [],
    }


def _identified_sell12_supply(*, inside: bool) -> dict[str, object]:
    anchor = (
        datetime(2025, 2, 3, 15, tzinfo=CN)
        if inside
        else datetime(2024, 12, 3, 15, tzinfo=CN)
    )
    available = anchor + timedelta(days=1)
    point_id = RiskMappingPointEvidenceFacts.identity(
        source_symbol="SH.000001",
        source_frequency="w",
        center_id="market-week-sell12-center",
        center_level_rank=2,
        point_type="1sell",
        point_anchor_at=anchor,
        point_available_at=available,
    ...
    point = RiskMappingPointEvidenceFacts(
        ...
        point_available_at=available,
        inside_active_top_interval=inside,
        highest_mapping_candidate=inside,
    )
    return RiskMappingSupplyFacts(
        classification=("UNIQUE_MAPPING" if inside else "SELL12_OUTSIDE_TOP_FRACTAL"),
        lower_structure_available=True,
        point_evidence_count=1,
        point_type_counts=(("1sell", 1), ("2sell", 0), ("3sell", 0), ("3buy", 0)),
        completed_sell12_count=1,
        in_top_interval_sell12_count=int(inside),
        completed_in_top_interval_sell12_count=int(inside),
        incomplete_in_top_interval_sell12_count=0,
        outside_top_interval_sell12_count=int(not inside),
        highest_candidate_center_count=int(inside),
        point_evidence=(point,),
        diagnostic_buy_point_type_counts=(("1buy", 0), ("2buy", 0)),
        diagnostic_buy_point_evidence=(),
    ).document()
    ...
    supply.update(
        {
            ...
            "diagnostic_directional_classification": ("BUY12_PRESENT_SELL12_ABSENT"),
...
def test_higher_timeframe_effectiveness_globally_deduplicates_stable_points() -> None:
    rows = [
...
def test_higher_timeframe_effectiveness_separately_deduplicates_diagnostic_buys() -> (
    ...
    for index, (row, interval) in enumerate(zip(rows, intervals)):
        ...
        evidence["period_diagnostics"][0] = _period_diagnostic_document(
            ...
            observed_at=row["decision_at"],
            active_top_interval=interval,
            mapping_supply=_identified_diagnostic_buy_supply(inside=index == 0),
            blocker_codes=("NO_COMPLETED_LOWER_1SELL_2SELL_CENTER_IN_TOP_FRACTAL",),
        )

    audit = higher_timeframe_effectiveness_audit(tuple(rows))
    market = audit["subjects"]["market"]
    diagnostic = market["globally_deduplicated_diagnostic_buy_point_audit"]
    ...
    assert (
        audit["global_diagnostic_buy_point_identity_audit"]["distinct_point_id_count"]
        == 1
    )
...
def test_higher_timeframe_effectiveness_rejects_same_time_point_state_conflict() -> (
    ...
    base = _effectiveness_row(0, market="AMBER", sector="AMBER", symbol="AMBER")
    rows = [copy.deepcopy(base), copy.deepcopy(base)]
...
def test_result_source_snapshot_rejects_unsupported_or_stale_code_identity() -> None:
    snapshot = _decision_source_snapshot()
    paths = tuple(row["path"] for row in snapshot["files"])

    assert snapshot["schema"] == ("chanlun-replay-decision-source-snapshot")
    ...
    assert (
        "src/chanlun/decision_support/trading_system/human_paper_ledger.py" not in paths
    )
    assert "web/chanlun_chart/cl_app/services/human_review_screening.py" not in paths
...
def test_market_history_keeps_native_daily_warmup_before_one_minute_cache() -> None:
    later_one_minute = SimpleNamespace(
        facts=SimpleNamespace(source_start=datetime(2025, 6, 10, 14, 0, tzinfo=CN))
    )

    assert _market_history_start({"SH.600000": later_one_minute}) == datetime(
        2023, 5, 1, 9, 30, tzinfo=CN
    )
    assert _market_history_start({}) == datetime(2023, 5, 1, 9, 30, tzinfo=CN)
...
def test_new_entry_skips_0930_without_a_completed_one_minute_bar() -> None:
    decisions = tuple(
        value[0] for value in _market_grids((date(2025, 8, 1), date(2025, 8, 4)))
    )
...
def test_session_checkpoint_retirement_is_effective_only_next_session() -> None:
    ...
    same_session = FULL_MARKET._carry_active_into_session(
        active,
        current_session=date(2025, 8, 1),
        resolution_sessions=resolution_sessions,
    )
    next_session = FULL_MARKET._carry_active_into_session(
        active,
...
def test_historical_ablations_rebuild_independent_source_signal_schedules(
    ...
    def fake_replay(**kwargs: object):
        calls.append(kwargs)
        kinds = tuple(
            signal.kind for rows in kwargs["signals"].values() for signal in rows
        )
    ...
    assert audits == {
        ...
        "EXACT_GREEN_HIGHER_TIMEFRAME_ONLY": {"event_schedule_sha256": "schedule-2"},
    }
...
def test_full_market_event_identity_is_content_addressed_not_ordinal() -> None:
    ...
    assert event_id == (
        "FM:2025-08-01T14:30:00+08:00:ENTRY:SH.600000:sha256:" + "a" * 64
    )
...
def test_tactical_execution_audit_explains_zero_lot_and_missing_dispatch() -> None:
    ...
    result = SimpleNamespace(
        intents=(
            SimpleNamespace(
                persistent_intent_id=persistent_id,
                event_id="FM:test:TACTICAL_SELL:SZ.300880:0",
                intent=intent,
            ),
        ),
        orders=(),
...
def test_risk_gate_preserves_mwd_warmup_evidence(
    ...
    period_diagnostics = tuple(
        {
            "period": period,
            "state": "UNRESOLVED",
            "blocker_codes": ["QMT_HIGHER_TIMEFRAME_WARMUP_HISTORY_INSUFFICIENT"],
        }
        for period in ("M", "W", "D")
    )
    convergence = SimpleNamespace(
        document=lambda: {"contract_id": "warmup-convergence-current"},
        diagnostic=SimpleNamespace(
            document=lambda: {"contract_id": "warmup-semantic-current"}
        ),
        mapping_supply_diagnostic=SimpleNamespace(
            document=lambda: {"contract_id": "warmup-supply-current"}
        ),
        structure_lineage_diagnostic=SimpleNamespace(
            document=lambda: {"contract_id": "warmup-lineage-current"}
        ),
    )
    monkeypatch.setattr(
        FULL_MARKET,
        ...
        lambda _envelope: SimpleNamespace(
            ...
            reason_codes=("QMT_HIGHER_TIMEFRAME_WARMUP_HISTORY_INSUFFICIENT",),
    ...
    assert blockers == ("QMT_HIGHER_TIMEFRAME_WARMUP_HISTORY_INSUFFICIENT",)
    assert evidence == {
        "source_revision": "sha256:" + "b" * 64,
        "monthly": "UNRESOLVED",
        "weekly": "UNRESOLVED",
        ...
        "warmup": warmup,
        "warmup_convergence": {"contract_id": "warmup-convergence-current"},
        "warmup_convergence_diagnostic": {"contract_id": "warmup-semantic-current"},
        "warmup_mapping_supply_diagnostic": {"contract_id": "warmup-supply-current"},
        "warmup_structure_lineage_diagnostic": {
            "contract_id": "warmup-lineage-current"
        },
...
def test_sector_risk_gate_uses_canonical_period_documents_for_mapping_supply(
    ...
    observed_at = datetime(2025, 8, 1, 14, 30, tzinfo=CN)
    mapping_supply = _identified_third_sell_supply(inside=True)
    diagnostic_document = _period_diagnostic_document(
        period="M",
        state="FORMED_UNRESOLVED",
        observed_at=observed_at,
        active_top_interval=(
    ...
    # The public period document keeps point counts keyed by point type.
    diagnostic_documents = (
        diagnostic_document,
        _period_diagnostic_document(period="W", state="NONE", observed_at=observed_at),
        _period_diagnostic_document(period="D", state="NONE", observed_at=observed_at),
    )
    diagnostics = tuple(
        SimpleNamespace(document=lambda row=row: row) for row in diagnostic_documents
    )
    convergence = SimpleNamespace(
        document=lambda: {"contract_id": "warmup-convergence-current"},
        diagnostic=SimpleNamespace(
    ...
    evidence = SimpleNamespace(
        ...
        reason_codes=("NO_COMPLETED_LOWER_1SELL_2SELL_CENTER_IN_TOP_FRACTAL",),
    ...
    monkeypatch.setattr(
        ...
        lambda **_kwargs: SimpleNamespace(
            ...
            strict_warmup_convergence_evidence=convergence,
...
def test_market_calendar_requires_two_matching_index_daily_paths() -> None:
    sessions = tuple(pd.bdate_range("2024-01-02", periods=480, tz=CN))
    ...
    with pytest.raises(ValueError, match="do not reconcile"):
        _reconciled_market_calendar(
            daily(
                tuple(value.normalize() + pd.Timedelta(hours=15) for value in sessions)
            ),
            daily(
                tuple(
                    value.normalize() + pd.Timedelta(hours=15) for value in mismatched
                )
            ),
        )
...
def test_terminal_accounting_attribution_closes_the_equity_identity() -> None:
    ...
    assert value["pnl_decomposition"]["open_cycle_marked_net_pnl"] == (Decimal("60"))
    ...
    assert value["concentration"]["max_symbol_invested_fraction"] == Decimal("1")
    assert value["sector_attribution"][0]["total_attributed_net_pnl"] == (Decimal("50"))
...
def test_terminal_accounting_headline_exposes_open_mark_dependency() -> None:
    ...
    assert value["closed_cycle_realized_return_on_initial_cash"] == (Decimal("-0.01"))
    assert value["open_cycle_marked_return_on_initial_cash"] == Decimal("0.06")
...
def test_terminal_accounting_headline_rejects_rehashed_inconsistent_identity() -> None:
    ...
    malformed["pnl_decomposition"]["open_cycle_marked_net_pnl"] = Decimal("59")
...
def _performance_research_result() -> dict[str, object]:
    return {
        ...
        "performance_reason": ("STRATEGIC_SAMPLE_BELOW_100+TACTICAL_SAMPLE_BELOW_200"),
        "daily_metrics": {
...
def test_performance_headline_exposes_holdout_benchmark_and_annualization() -> None:
    value = FULL_MARKET._performance_headline(
        ...
        terminal_headline={
            "return_dependency_status": ("POSITIVE_TOTAL_DEPENDS_ON_OPEN_CYCLE_MARKS")
        },
    )

    assert value["status"] == "EVALUATED_SAMPLE_INSUFFICIENT"
    assert value["relative_performance_status"] == (
        "POSITIVE_FULL_SAMPLE_BUT_NEGATIVE_HOLDOUT_AND_UNDERPERFORMS_ALL_BENCHMARKS"
    )
    ...
    assert value["benchmarks"][0]["strategy_excess_net_return"] == (Decimal("-0.05"))
    assert value["benchmarks"][1]["strategy_excess_net_return"] == (Decimal("-0.12"))
...
def test_human_review_mode_requires_approximation_and_rejects_removed_state_args(
    tmp_path,
) -> None:
...
def test_nonempty_historical_human_review_alert_carries_sector_gate() -> None:
        ...
        candidate_audit=(
            {
                ...
                "sector_ordinal": 1,
                "sector_rank_score": 5,
                "sector_rank_components": {"neutral_access": 5},
    ...
    assert alert.sector_ranking_evidence.rank_components == (("neutral_access", 5),)
