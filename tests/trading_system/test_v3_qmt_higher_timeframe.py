...
from tests.trading_system.test_v3_qmt_same_base_stream import _native_session
...
def test_sector_coverage_proves_physical_qmt_cache_left_boundary() -> None:
    ...
    physical_stable = {
        "schema": "chanlun-qmt-current-sector-physical-5m-coverage/v1",
    ...
    assert document["contract_id"].endswith("/v3")
    ...
    physical_legacy = dict(document)
    physical_legacy["contract_id"] = (
        "chanlun-qmt-sector-same-5m-source-coverage/v2"
    )
    physical_legacy.pop("physical_source_requested_start_at")
    parsed_physical_legacy = QmtSectorSameBaseCoverageEvidence.from_document(
        physical_legacy
    )
    assert parsed_physical_legacy.physical_source_requested_start_at is None

    legacy = dict(document)
    legacy["contract_id"] = "chanlun-qmt-sector-same-5m-source-coverage/v1"
    for name in tuple(legacy):
        if name.startswith("physical_source_"):
            legacy.pop(name)
    parsed_legacy = QmtSectorSameBaseCoverageEvidence.from_document(legacy)
    assert parsed_legacy.physical_source_boundary_status == "NOT_RECORDED_LEGACY"
