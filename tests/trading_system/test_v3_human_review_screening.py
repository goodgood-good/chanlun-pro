...
def _period_diagnostic(
    ...
    *,
    ...
    return HigherTimeframePeriodDiagnostic(
        ...
        mapping_unique=True,
        mapped_center_id=(f"{period}-center" if formed else None),
        mapping_candidate_ids=((f"{period}-center",) if formed else ()),
        blocker_codes=(),
        warning_codes=(),
...
def test_sector_source_evidence_is_portable_hash_bound_and_legacy_compatible(
) -> None:
    legacy = _alert()
    assert "sector_higher_timeframe_evidence" not in (
        human_review_alert_document(legacy)
    )
    ...
    alert = replace(
        ...
        source_fact_ids=(*legacy.source_fact_ids, source.evidence_id),
    )
    ...
    assert source.schema != SECTOR_HIGHER_TIMEFRAME_REVIEW_EVIDENCE_LEGACY_SCHEMA
...
def test_sector_v4_preserves_multi_prefix_convergence_and_rejects_relabel(
) -> None:
    source = _sector_evidence(with_convergence=True)

    assert source.schema.endswith("/v4")
...
def test_market_symbol_mwd_evidence_is_causal_hash_bound_and_legacy_compatible(
) -> None:
    legacy = _alert()
    legacy_id = legacy.candidate_id
    assert "market_symbol_higher_timeframe_evidence" not in (
        human_review_alert_document(legacy)
    )
    assert legacy.candidate_id == legacy_id

    evidence = _market_symbol_evidence()
    alert = replace(
        legacy,
        market_symbol_higher_timeframe_evidence=evidence,
        source_fact_ids=(*legacy.source_fact_ids, evidence.evidence_id),
    )
...
def test_market_symbol_source_support_preserves_session_warmup_and_native_daily(
    ...
    risk: dict[str, object] = {
        ...
        "warmup_evidence_contract_id": (
            "chanlun-qmt-mwd-warmup-evidence/v1"
        ),
...
def test_sector_ranking_evidence_is_portable_explicit_and_hash_bound() -> None:
    ...
    assert evidence.source_profile == "LIVE_FULL_RANKING"
    ...
    with pytest.raises(ValueError, match="cannot invent components"):
        replace(
            evidence,
...
def test_pre_sector_gate_archive_reloads_with_exact_identity_and_fails_closed(
) -> None:
    """The real July v1 profile omitted the sector gate from portable rows."""
    ...
    [restored] = validate_human_review_screen_document(report)
    assert restored.candidate_id == legacy_candidate_id
    assert restored.signal_lifecycle_id == current.signal_lifecycle_id
    assert restored.sector_risk_gate == "UNRESOLVED"
    assert restored.sector_risk_gate_attested is False
    assert human_review_alert_document(restored) == legacy_identity

    # The compatibility profile is deliberately exact.  A newer boundary or
    # source-evidence shape cannot omit its sector gate and call itself legacy.
    forged = json.loads(json.dumps(report))
    forged_row = forged["review_queue"][0]
    for field in (
        "entry_confirmation_bar_closed_at",
        "entry_price_cap",
        "entry_valid_until",
        ...
        forged_row[field] = None
    outer = {key: forged[key] for key in forged if key != "content_sha256"}
    forged["content_sha256"] = sha256_json(outer)
    with pytest.raises(ValueError, match="candidate_malformed"):
        validate_human_review_screen_document(forged)
...
def test_sector_source_evidence_from_risk_rejects_partial_extension() -> None:
    risk = {
        "sector_higher_timeframe_source_mode": QMT_SECTOR_SAME_BASE_SOURCE_MODE,
        "sector_strict_same_5m_warmup_evidence": (
            _warmup(sufficient=True).document()
        ),
        "sector_research_bridge_parameter_set_id": None,
    }
    source = sector_higher_timeframe_review_evidence_from_risk(risk)
    assert source is not None
    assert source.schema == SECTOR_HIGHER_TIMEFRAME_REVIEW_EVIDENCE_LEGACY_SCHEMA
    assert parse_sector_higher_timeframe_review_evidence(source.document()) == source
    assert sector_higher_timeframe_review_evidence_from_risk(risk) == source
    assert sector_higher_timeframe_review_evidence_from_risk({}) is None
    risk.pop("sector_research_bridge_parameter_set_id")
    with pytest.raises(ValueError, match="fields are incomplete"):
        sector_higher_timeframe_review_evidence_from_risk(risk)
...
def test_legacy_feedback_ledger_remains_readable(tmp_path) -> None:
    path = tmp_path / "legacy-feedback.json"
    feedback = HumanReviewFeedback(
    ...
    legacy_identity_values = {
        field: getattr(feedback, field) for field in legacy_fields
    }
    stable_entry = {
        **{
            **asdict(feedback),
        ...
        "feedback_id": sha256_json(legacy_identity_values),
    ...
    stable_document = {
        "schema": "chanlun-v3-human-review-feedback-ledger/v1",
        "entries": [entry],
        "automated_order_authorized": False,
        "live_status": "LIVE_DISABLED",
    }
    ...
    assert load_human_review_feedback_ledger(path)["entries"] == [entry]
