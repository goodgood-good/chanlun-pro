        ...
        "structure_coverage": (
            {
                ...
                "rule_coverage": {
                    rule: "COMPLETE" for rule in V3_REQUIRED_STRUCTURE_RULES
                },
            },
        ),
...
def market_bars(entry_at: datetime, exit_at: datetime) -> tuple[HistoricalMinuteExecutionBar, ...]:
...
def test_real_facts_generate_entry_cash_action_and_frozen_exit_then_replay() -> None:
    ...
    built = build_v3_replay_payload(
        prescreen_artifacts=(
            prescreen(chains=(chain_document(entry_at),)),
        ),
...
def test_persistent_exit_keeps_working_across_later_sessions() -> None:
    ...
    events = [
        event
        for batch in built.payload["batches"]
...
def test_chain_with_missing_fact_ledger_is_empty_and_explicitly_unresolved() -> None:
    ...
    built = build_v3_replay_payload(
        prescreen_artifacts=(
            prescreen(chains=(chain_document(decision),)),
        ),
        fact_ledger=None,
        bars_by_symbol={},
        corporate_action_ledger=None,
...
def test_prescreen_and_fact_ledgers_require_exact_content_hashes() -> None:
    ...
    rejected_prescreen = build_v3_replay_payload(
    ...
    rejected_facts = build_v3_replay_payload(
        prescreen_artifacts=(
            prescreen(chains=(chain_document(entry_at),)),
        ),
