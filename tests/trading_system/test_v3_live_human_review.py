...
def _attach_strength_evidence(
    ...
    *,
    ...
    sector_ids = tuple(
        sorted(str(value["sector_id"]) for value in snapshot["sectors"])
    )
    ...
    by_id = {
        str(value["sector_id"]): value for value in snapshot["sectors"]
    }
    ...
    epoch_id = screening_coverage_epoch_id(
        ...
        structure_version=str(snapshot["structure_version"]),
        parameter_version=str(snapshot["parameter_version"]),
    )
...
def _decisions():
    ...
    sell_bundle = replace(
        ...
        one_points=(
            confirmed_point("1sell", frequency="1m", minutes_after=1),
        ),
        opposite_points=(),
    )
    return core, (
...
def live_snapshot() -> dict[str, object]:
            ...
            "by_frequency": [
                {
                    "frequency": frequency,
                    "converged": True,
                    "full_bar_count": SCREENING_WARMUP_REQUIRED_BARS[
                        frequency
                    ],
                    "suffix_bar_count": (
                        expected_screening_warmup_suffix_bar_count(
    ...
    screening_policy = {"schema": "test-screening-policy/v1"}
    screening_policy_id = sha256_json(screening_policy)
    universe_revision = sha256_json(
        {"schema": "test-screening-universe/v1", "codes": codes}
    )
    ...
    return {
        **stable,
        "snapshot_content_sha256": live_screening_snapshot_content_sha256(
            stable
        ),
    }
...
def _with_monitor_instrument_exclusion(
    ...
    *,
    ...
    snapshot["snapshot_content_sha256"] = (
        live_screening_snapshot_content_sha256(snapshot)
    )
    return snapshot
...
def test_monitor_instrument_exclusion_contract_accepts_canonical_diagnostic(
) -> None:
...
def test_monitor_instrument_exclusion_contract_preserves_unresolved_type(
) -> None:
...
def test_market_watermark_rejects_rehashed_monitor_exclusion_forgery() -> None:
    ...
    forged_scope["monitor_instrument_exclusions"][0][
        "selection_sources"
    ] = ["QMT_SECTOR_TRIGGER"]

    forged_tradable_type = copy.deepcopy(canonical)
    forged_tradable_type["monitor_instrument_exclusions"][0][
        "qmt_instrument_type"
    ] = "stock_cn"
    ...
    rediscovered_count = len(
        rediscovered["coverage_manifest"]["discovered_codes"]
    )
    ...
    for forged in (
        ...
        forged["snapshot_content_sha256"] = (
            live_screening_snapshot_content_sha256(forged)
        )
        with pytest.raises(
            ValueError,
...
def test_live_snapshot_preserves_and_recomputes_mwd_multi_prefix_evidence(
) -> None:
    snapshot = live_snapshot()
    as_of = deterministic_bundle().as_of
    ...
    risk.update(
        {
            ...
            "market_warmup_convergence_evidence": copy.deepcopy(
                convergence
            ),
            "sector_warmup_convergence_evidence": copy.deepcopy(
                convergence
            ),
            "symbol_warmup_convergence_evidence": copy.deepcopy(
                convergence
            ),
        }
    )
...
def test_live_snapshot_authenticates_warmup_semantic_diagnostic() -> None:
    ...
    def semantic(weekly_state: str) -> WarmupSemanticSnapshot:
    ...
    baseline = semantic("NONE")
    middle = semantic("FORMED")
    ...
    envelope = bind_warmup_mapping_supply_diagnostic(
        envelope,
        snapshots=(empty_supply, empty_supply, empty_supply),
    )
...
def test_session_gap_diagnostic_is_recomputed_and_cannot_claim_suspension() -> None:
    ...
    # A legacy v6 row remains valid only while it does not claim a session
    # problem that would require exact affected dates.
                ...
                "issues": [
                    {
                        ...
                        "detail": (
                            "trading-calendar session is absent from the QMT 1m "
                            "prefix"
                        ),
    ...
    forged["symbol_session_evidence"]["issues"][0][
    ...
    assert evidence.market.source_support.session_evidence.document() == (
        exact_empty
    )
    assert (
    ...
    assert evidence.market.source_support.support_id == (
...
def test_displayed_decision_recomputes_provisional_setup_reason() -> None:
    ...
    signal["decision_reasons"] = [
        value
        for value in signal["decision_reasons"]
        if value != "setup_not_confirmed"
    ]
...
def test_live_adapter_preserves_30m_5m_1m_roles() -> None:
    ...
    assert ranking["source_profile"] == "LIVE_FULL_RANKING"
    ...
    assert ranking["strength_evidence_revision"] == snapshot[
        "sector_strength_evidence_revision"
    ]
    assert ranking["sector_catalog_revision"] == snapshot[
        "coverage_manifest"
...
def test_live_alert_retains_structured_sector_source_evidence() -> None:
    ...
    assert strict.sector_higher_timeframe_evidence.sector_id == (
    ...
    assert strict.sector_higher_timeframe_evidence.gate == (
        signal["higher_timeframe_risk"]["sector_gate"]
    )
    assert (
        strict.sector_higher_timeframe_evidence.evidence_id
        in strict.source_fact_ids
    )
    ...
    risk["sector_period_diagnostics"] = [
    ...
    assert research.sector_higher_timeframe_evidence.period_diagnostics == (
        diagnostics
    )
        ...
        snapshot["sectors"][0].update(
            {
                ...
                "horizontal_rank": 1,
                "strength_anchor_session": deterministic_bundle().as_of.date().isoformat(),
...
def test_live_alert_lifecycle_is_stable_across_snapshot_revisions() -> None:
    ...
    assert dict(evidence.market.states) == signals[0]["higher_timeframe_risk"][
        "market_states"
    ]
    assert dict(evidence.symbol_evidence.states) == signals[0][
...
def test_live_alert_explains_watchlist_monitor_origin() -> None:
    ...
    signal["decision_reasons"].append(
        "current_qmt_sector_trigger_required"
    )
    signal["decision_document_id"] = signal_decision_document_id(signal)
    snapshot["snapshot_content_sha256"] = (
        live_screening_snapshot_content_sha256(snapshot)
...
def test_live_adapter_rejects_incomplete_coverage_or_wrong_locator() -> None:
    ...
    tampered["snapshot_content_sha256"] = (
        live_screening_snapshot_content_sha256(tampered)
    )
    ...
    malformed_basis["snapshot_content_sha256"] = (
        live_screening_snapshot_content_sha256(malformed_basis)
    )
    with pytest.raises(ValueError, match="boundary is malformed"):
        validate_live_review_snapshot(malformed_basis)


def test_live_adapter_rejects_legacy_or_mixed_signal_contract() -> None:
    legacy = live_snapshot()
    legacy.pop("signal_document_contract_id")
    legacy["coverage_manifest"].pop("signal_document_contract_id")
    legacy["snapshot_content_sha256"] = (
        live_screening_snapshot_content_sha256(legacy)
    )
    with pytest.raises(ValueError, match="boundary is incomplete"):
        validate_live_review_snapshot(legacy)

    mixed = live_snapshot()
    mixed["coverage_manifest"]["signal_document_contract_id"] = (
        "chanlun-human-assisted-signal-document/v1"
    )
    mixed["snapshot_content_sha256"] = (
        live_screening_snapshot_content_sha256(mixed)
    )
    with pytest.raises(ValueError, match="boundary is incomplete"):
        validate_live_review_snapshot(mixed)

    forged_v2 = live_snapshot()
    forged_v2["signals"][0]["higher_timeframe_risk"].pop(
        "market_reason_codes"
    )
    forged_v2["snapshot_content_sha256"] = (
        live_screening_snapshot_content_sha256(forged_v2)
    )
    with pytest.raises(ValueError, match="risk evidence"):
        validate_live_review_snapshot(forged_v2)
...
def test_live_adapter_rejects_tampered_identity_and_low_stock_coverage() -> None:
    ...
    insufficient["snapshot_content_sha256"] = (
        live_screening_snapshot_content_sha256(insufficient)
    )
    with pytest.raises(ValueError, match="boundary is incomplete"):
...
def test_live_adapter_recomputes_partial_universe_coverage_claims() -> None:
    ...
    completed_codes = sorted(
        (*signal_codes, "SH.600001", "SH.600002")
    )
    ...
    partial["snapshot_content_sha256"] = (
        live_screening_snapshot_content_sha256(partial)
    )
    ...
    missing_error["snapshot_content_sha256"] = (
        live_screening_snapshot_content_sha256(missing_error)
    )
    with pytest.raises(ValueError, match="boundary is incomplete"):
...
def test_live_adapter_authenticates_minimum_history_exclusions() -> None:
    ...
    completed_codes = sorted(
        (*signal_codes, "SH.600001", "SH.600002", "SH.600003")
    )
...
def test_live_adapter_recomputes_decision_core_identity() -> None:
    ...
    self_asserted["snapshot_content_sha256"] = (
        live_screening_snapshot_content_sha256(self_asserted)
    )
    with pytest.raises(ValueError, match="boundary is incomplete"):
