...
from tools.backtest_multisymbol_events import (
    INPUT_SCHEMA,
    _jsonable,
    run_payload,
)
...
def chain(symbol: str, decision: datetime, *, boundary: str = "10") -> AlignedEntryChain:
...
class CountingCore(DecisionCore):
    def __init__(self) -> None:
        self.calls = 0

    def decide(self, facts: DecisionInput):  # type: ignore[no-untyped-def]
        self.calls += 1
        return super().decide(facts)


def engine(started_at: datetime, core: DecisionCore | None = None):  # type: ignore[no-untyped-def]
...
def test_contract_binds_parent_etf_override_alignment_and_tactical() -> None:
    contract = strict_replay_contract()
    ...
    assert contract.effective_alignment_contract_id == ALIGNMENT_CONTRACT_ID
...
def test_direct_recursive_contract_uses_same_decision_and_execution_core() -> None:
    decision = at(D0, 10, 0)
    symbol = "SH.510300"
    contract = strict_direct_replay_contract()
    ...
    core = CountingCore()
    result = StrictMultiSymbolReplayEngine(
        initial_cash=Decimal("1000000"),
...
def test_research_individual_contract_uses_stock_order_and_shared_core() -> None:
    ...
    candidate_value = CandidateDecision(
        ...
        checks=tuple(
            ...
            for gate in sorted(INDIVIDUAL_REQUIRED_CANDIDATE_GATES)
        ),
...
def test_entry_scheduler_shrinks_to_decision_time_slot_cap() -> None:
    ...
    decision = at(D0, 10, 0)
    symbol = "SH.510300"
    replay = StrictMultiSymbolReplayEngine(
...
def test_missing_alignment_fact_fails_closed_after_shared_core_decision() -> None:
    ...
    assert not result.metrics.ledger_valid
...
def test_strategic_exit_is_t1_blocked_then_persists_to_next_session() -> None:
    ...
    persistent_intent_id = "persistent-exit:SH.510880:point-1"
    entry_event = event(
        "entry:t1",
...
def test_verified_flat_persistent_exit_is_an_idempotent_noop() -> None:
    ...
    flat_exit = event(
        ...
        bars=(),
        optional=False,
        persistent_intent_id="persistent-exit:already-flat",
    )
    ...
    assert result.resolved_persistent_intent_ids == (
        "persistent-exit:already-flat",
    )
    assert result.suppressed_persistent_event_counts == (
        ("persistent-exit:already-flat", 1),
    )
...
def test_zero_lot_tactical_inventory_retires_persistent_sell_retries() -> None:
    ...
    persistent_id = "persistent-tactical-sell:zero-lot"
    entry_event = event(
...
def frozen_builder_contract() -> dict[str, str]:
    ...
    return {
        "alignment_contract_id": AlignmentContract().contract_id,
        "alignment_parameter_set_id": AlignmentContract().parameter_set_id,
        "timeframe_override_parameter_set_id": (
...
def test_json_artifact_entry_rejects_foreign_builder_contract() -> None:
    ...
    for key, bad in (
        ("alignment_contract_id", "SOMETHING_ELSE"),
