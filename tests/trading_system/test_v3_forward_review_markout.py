...
def _report(
    ...
    *,
    ...
    if decision_core_id is not None:
        input_hashes["decision_core_id"] = decision_core_id
    if decision_source_snapshot is not None:
        input_hashes["decision_source_snapshot_id"] = (
            decision_source_snapshot_id(decision_source_snapshot)
        )
    stable = {
        ...
        **(
            ...
            else {"decision_source_snapshot": decision_source_snapshot}
        ),
    }
    return {**stable, "content_sha256": sha256_json(stable)}
...
def test_markout_names_the_three_independent_green_gates_exactly() -> None:
    ...
    assert report["schema"] == "chanlun-v3-forward-review-markout/v7"
...
def test_markout_separates_screening_policy_cohorts() -> None:
    ...
    assert "MIXED_SCREENING_POLICY_COHORTS_MUST_NOT_BE_POOLED" in report[
        "reason_codes"
    ]
...
def test_markout_replays_embedded_decision_source_snapshot() -> None:
    ...
    report["decision_source_snapshot"]["files"][0]["sha256"] = (
        "sha256:" + "5" * 64
    )
...
def test_sample_minimum_requires_attested_cohort_and_price_source() -> None:
    ...
    assert missing_price_audit["sample"]["sample_sufficient_by_horizon"][
        "5"
    ] is False
    ...
    legacy_samples = tuple(
        replace(
            sample,
            source_decision_core_id="LEGACY_UNATTESTED_DECISION_CORE",
    ...
    legacy = build_forward_review_markout(
        legacy_samples,
        through_session=sessions[-1],
        trading_sessions=sessions,
        bars_by_symbol={base_alert.symbol: bars},
    ...
    assert legacy["sample"]["sample_sufficient_by_horizon"]["5"] is False
    assert legacy["sample"]["source_identity_status"] == "LEGACY_UNATTESTED"
    assert "DECISION_SOURCE_COHORT_UNATTESTED" in legacy["reason_codes"]
...
def test_current_markout_rejects_rehashed_internal_accounting_forgery() -> None:
    ...
    wrong_cohort["observations"][0]["source_cohort_id"] = (
        "sha256:" + "0" * 64
    )
...
def test_markout_never_shifts_past_an_incomplete_market_session() -> None:
    ...
    bars = [
        ...
        for bar in bars
        if bar.observed_at
        != a_share_completed_one_minute_closes(sessions[0])[100]
    ]
    ...
    five = next(
        row for row in report["observations"] if row["horizon_sessions"] == 5
    )
...
def test_markout_rejects_a_gapped_intraday_reference_prefix() -> None:
    ...
    assert {
        row["reason_code"] for row in report["observations"]
    } == {
