...
from tools import run_forward_paper as subject
...
def _ready_implementation_continuity() -> dict[str, object]:
    return {
        "schema": "chanlun-forward-implementation-continuity",
        "ready": True,
        "status": "ready",
        "reason_code": "READY",
...
def _paper_review_source(
    *,
    ...
    ranking = SectorRankingReviewEvidence(
        ...
        reason_codes=("TEST_FULL_RANKING",),
    ...
    alert = HumanReviewAlert(
        ...
        source_point_id=subject.sha256_json(
            {"schema": "test-source-point", "symbol": symbol}
        ),
        structure_snapshot_id=subject.sha256_json(
            {
                "schema": "test-structure-snapshot",
                "symbol": symbol,
    ...
    frame.attrs.update(
        {
            ...
            "price_basis_adjustment": "causal-forward-ex-date",
...
def test_human_review_evaluate_uses_frozen_evaluated_status(
    ...
    def fake_status_capture(**kwargs):
        ...
        return {
            "schema": "chanlun-qmt-instrument-status-snapshot",
            "all_complete": True,
...
def test_forward_attempt_without_declared_policy_is_rejected(
    tmp_path: Path,
) -> None:
    session_root = tmp_path / "session"
    live_object = session_root / "objects" / "unattested.json"
    snapshot = {
        "screening_policy": {
            "selection_universe_source": "qmt_gics3_current_components"
        }
    }
    source_identity = subject.sha256_json(snapshot)
    stable = {
        "schema": "chanlun-forward-live-screening-snapshot",
    ...
    with pytest.raises(RuntimeError, match="policy identity is invalid"):
        subject._attempt_screening_policy_id(session_root, attempt)
...
def test_evaluate_never_refreshes_pit_after_decision_close() -> None:
...
def test_application_source_revision_tracks_only_forward_runtime_sources(
    tmp_path: Path,
) -> None:
    ...
    original = calculate_forward_application_source_revision(repo)
    assert original.startswith(f"{head}.tree.")
    assert len(original.rsplit(".tree.", maxsplit=1)[1]) == 24

    source.write_text("VALUE = 2\n", encoding="utf-8")
    tracked_change = calculate_forward_application_source_revision(repo)
    assert tracked_change != original

    historical_tool.write_text("PIPELINE_ID = 'changed'\n", encoding="utf-8")
    historical_change = calculate_forward_application_source_revision(repo)
    assert historical_change == tracked_change

    forward_tool.write_text("FORWARD_ID = 'changed'\n", encoding="utf-8")
    forward_change = calculate_forward_application_source_revision(repo)
    assert forward_change != historical_change
    ...
    untracked_change = calculate_forward_application_source_revision(repo)
    assert untracked_change != forward_change
...
def test_implementation_provenance_is_content_addressed_and_safe() -> None:
    ...
    assert provenance["schema"] == FORWARD_IMPLEMENTATION_PROVENANCE_SCHEMA
    assert content_sha256 == subject.sha256_json(stable)
    assert provenance["forward_scheduler_module_sha256"] == subject.sha256_file(
