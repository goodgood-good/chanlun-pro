...
from tools.build_v3_recent_year_current_sector_triggers import (
    _catalog_scope,
    _pit_snapshot_available_at,
)
from tools.extract_v3_sector_first_direct_facts import _load_query_plan
from tools.prescreen_v3_sector_first_research_candidates import (
    _COMPATIBLE_SUPERSET_PLANNER_REVISIONS,
    _checkpoint_path,
    _checkpoint_payload,
    ...
    _structural_hash_transition_is_compatible,
)
...
def test_historical_pit_availability_is_invariant_to_release_copy_mtime(
    tmp_path: Path,
) -> None:
    ...
    path.write_text("same immutable bytes", encoding="utf-8")
    later = datetime(2026, 8, 2, 2, 52, tzinfo=CN).timestamp()
    ...
    forward, forward_basis = _pit_snapshot_available_at(
        path=path,
        snapshot=snapshot,
        forward_session=date(2026, 8, 2),
    )

    assert historical == captured
    assert historical_basis == "CONTENT_CAPTURED_AT_HISTORICAL"
    assert forward == datetime.fromtimestamp(later, tz=CN)
    assert forward_basis == "MAX_CONTENT_CAPTURED_AT_AND_FILE_MTIME_FORWARD"
...
def test_structural_compatibility_allows_only_exact_audited_transitions(
    path: str,
    old: str,
    current: str,
) -> None:
    assert _structural_hash_transition_is_compatible(path, current, current)
    assert _structural_hash_transition_is_compatible(path, old, current)
    assert not _structural_hash_transition_is_compatible(
        path,
        "sha256:" + "0" * 64,
        current,
    )
    assert not _structural_hash_transition_is_compatible(
        "src/chanlun/core/another.py",
        old,
        current,
    )


def test_formal_v20_planner_is_an_explicit_superset_source_only() -> None:
    assert (
        "sha256:92092c427c3d1fc760cab5705ec488f438dfd5bb8e4e405e131b39a770ac9784"
        in _COMPATIBLE_SUPERSET_PLANNER_REVISIONS
    )


def test_recent_year_algorithm_scope_is_relevant_and_deterministic() -> None:
    paths = set(RECENT_YEAR_RESEARCH_ALGORITHM_PATHS)
    assert RECENT_YEAR_RESEARCH_ALGORITHM_SCOPE.endswith("/v1")
    assert (
