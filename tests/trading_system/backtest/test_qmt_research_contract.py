...
def test_symbol_fact_algorithm_boundary_is_strict_dependency_subset() -> None:
    ...
    assert "src/chanlun/decision_support/trading_system/backtest/fixed_year.py" in fact
    ...
    assert (
        "src/chanlun/decision_support/trading_system/live_human_review.py" not in fact
    )
    assert "src/chanlun/decision_support/trading_system/backtest/report.py" not in fact
    assert (
        "src/chanlun/decision_support/trading_system/backtest/portfolio.py" not in fact
    )
    assert "src/chanlun/decision_support/trading_system/portfolio_risk.py" not in fact
