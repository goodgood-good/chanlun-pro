...
from tools.prescreen_v31_cached_symbols import (
...
def test_seed_requires_hash_match_and_completed_trend_evidence() -> None:
    seed = {
        ...
        "l0_first_center_third_buy_count": 2,
        "l1_completed_trend_count": 3,
        "l2_first_or_second_buy_count": 4,
        "aligned_entry_chain_count": 1,
        "l1_completed_trend_ledger": (
            {"direction": "up", "price_basis_revision": "basis"},
            {"direction": "down", "price_basis_revision": "basis"},
        ),
    }
    report = _seed_report(seed, market_hash="market", pit_hash="pit")
    assert report is not None
    assert report["l1_evidence_kind"] == "COMPLETED_TREND"
    assert report["l1_constituent_unit_accepted"] is False
    assert report["structurally_legal_chain_count"] == 1
    assert _seed_report(seed, market_hash="changed", pit_hash="pit") is None
