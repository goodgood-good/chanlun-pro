...
from tools.build_chanlun_v3_replay_payload import (
...
def test_strict_replay_defaults_are_the_fixed_eight_csi300_etfs() -> None:
    symbols = tuple(
        path.stem.removeprefix("v31_cached_symbol_prescreen_")
...
def test_strict_replay_defaults_use_expanded_actions_and_consolidated_cache() -> None:
    assert DEFAULT_CORPORATE_ACTIONS.name == "qmt_csi300_etf_corporate_actions_v1.json"
    assert DEFAULT_MARKET_DATABASE.as_posix().endswith(
        "chanlun_v31_csi300_broad_pool/financial_data_query_bars.sqlite3"
    )
