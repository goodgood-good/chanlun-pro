...
from tools import promote_v3_sector_release as subject
...
def _setup(tmp_path: Path) -> SimpleNamespace:
    ...
    stable_manifest = tmp_path / "published/v3_release_manifest.json"
    candidate_artifact = tmp_path / "candidate/result.json"
    candidate_manifest = tmp_path / "candidate/v3_release_manifest.json"
    economic = {"schema": "research/v1", "parameters": {"slots": 5}}
    _write_json(
        stable_artifact,
        {
            **economic,
    ...
    _write_json(stable_manifest, {"version": "old"})
...
def test_promotion_archives_and_atomically_replaces_both_files(
    ...
    monkeypatch.setattr(
        ...
        lambda **_: {"schema": "release/v1", "content_sha256": NEW_SHA},
    )
    args.execute = True
