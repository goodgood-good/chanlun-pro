...
def test_second_point_keeps_internal_pullback_extreme_and_confirmation(direction):
    engine = strict_engine(direction=direction)
    parent = only_point(engine.first_class_points())
    old = only_point(engine.second_class_points((parent,)))
    pullback = next(u for u in engine.structure.levels[0].units if u.unit_id == old.anchor_unit_id)
    moment = pullback.market_end - timedelta(minutes=2)
    pullback = replace(pullback, **(
        {"end_tick":pullback.low_tick + 2, "low_market_time":moment}
        if parent.side == "buy" else {"end_tick":pullback.high_tick - 2, "high_market_time":moment}
    ))
    point = engine._second_class_point(parent, pullback, variant=StrictPointVariant.STRICT,
                                      divergence=None, related_point_ids=())
    assert point.anchor_at == moment < pullback.market_end
    assert point.anchor_tick == old.anchor_tick
    assert point.invalidation_tick == parent.anchor_tick
    assert point.confirmed_at == old.confirmed_at
    assert point.available_at == old.available_at
    ...
    lower = replace(
        ...
        divergence=replace(
            ...
            divergence_id=stable_structure_id(
                "chanlun-strict-divergence", TEST_PRICE_BASIS, 0,
                SourceKind.SEGMENT.value, "trend", "down",
                lower.divergence.compare_leg_unit_ids, (lower_anchor.unit_id,),
            ),
            signal_unit_id=lower_anchor.unit_id,
            signal_leg_unit_ids=(lower_anchor.unit_id,),
            price_anchor_unit_id=lower_anchor.unit_id,
            anchor_at=signal.market_end,
            anchor_tick=signal.end_tick,
    ...
    parent = replace(
        ...
        divergence=replace(
            ...
            divergence_id=stable_structure_id(
                "chanlun-strict-divergence", TEST_PRICE_BASIS, 0,
                SourceKind.SEGMENT.value, "trend", "down",
                parent.divergence.compare_leg_unit_ids, (lower_anchor.unit_id,),
            ),
            signal_unit_id=lower_anchor.unit_id,
            signal_leg_unit_ids=(lower_anchor.unit_id,),
            price_anchor_unit_id=lower_anchor.unit_id,
