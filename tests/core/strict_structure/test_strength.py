...
def test_divergence_price_and_time_refer_to_the_same_internal_extreme(direction):
    sign = -1 if direction == "down" else 1
    provider = fake_strength_provider(native_hist=[sign * v for v in (6, 5, 4, 1, 1, 1)])
    before = unit_covering_indexes(0, 2, direction=direction, unit_id="before")
    after = unit_covering_indexes(3, 5, direction=direction, unit_id="after")
    moment = source_dates(6)[4]
    after = replace(after, **(
        {"low_tick":95, "low_market_time":moment}
        if direction == "down" else {"high_tick":115, "high_market_time":moment}
    ))
    divergence = compare_divergence(before, after, provider, kind="consolidation")
    assert divergence.anchor_at == moment < after.market_end
    assert divergence.anchor_tick == (95 if direction == "down" else 115)
    assert divergence.confirmed_at == after.confirmed_at
