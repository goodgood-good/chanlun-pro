...
def _trend_units():
    return (
        ...
        # The first return must stay strictly above the upper boundary.
        unit(
            5,
            "down",
            130,
...
def test_recursive_preview_return_touching_boundary_is_not_a_third_point(return_tick, expected):
    ...
    returning = unit(5, "down", 130, return_tick, locked=False,
                     source_kind=SourceKind.TREND_TYPE, structural_level=1)
    projected = _advance_center_preview_lifecycle(preview, (values[0], *core), (values[4], returning))
    assert projected is not None and projected.state is expected
