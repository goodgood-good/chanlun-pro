...
def test_five_segment_window_preserves_middle_three_core() -> None:
...
def test_four_segments_are_not_formal_and_five_establish() -> None:
    values = valid_five_up_exit()
...
def test_entry_without_positive_core_overlap_is_rejected() -> None:
    values = (
        unit(-1, "up", 80, 80),
        unit(0, "down", 80, 60),
        unit(1, "up", 60, 75),
    ...
    assert _establish(values) is None


@pytest.mark.parametrize("bad_role", ("entry", "leave"))
def test_entry_and_leave_must_positively_overlap_frozen_core(bad_role) -> None:
    ...
    if bad_role == "entry":
        ...
        values[4] = replace(values[4], start_tick=115, low_tick=115)

    assert _establish(tuple(values)) is None


def test_unlocked_fifth_role_is_preview_only() -> None:
    values = valid_five_up_exit()
    active = values[:-1] + (
...
def test_center_identity_includes_price_basis_revision() -> None:
    ...
    rebased = _establish(
        tuple(replace(item, price_basis_revision="post-action") for item in values)
    )
...
def test_seed_rejects_mixed_basis_in_entry_or_body() -> None:
    values = valid_five_up_exit()
    mixed = values[:-1] + (
