...
def _sample_df():
    return pd.DataFrame(
        {
            "date": pd.to_datetime(["2026-05-18 09:31", "2026-05-18 09:32"]),
            "code": ["SH.600000", "SH.600000"],
            "open": [12.3456, 12.3454],
            "high": [12.9999, 13.0001],
            "low": [12.0005, 12.0004],
            "close": [12.5 + 4e-16, 12.5 - 4e-16],
            "volume": [1000.0, 2000.0],
        }
    )


def test_normalize_rounds_ohlc_to_market_precision():
    # A股 股票 → 2 位
    out = normalize_kline_precision(_sample_df(), "a", "SH.600000")
    assert list(out["open"]) == [12.35, 12.35]
    assert list(out["high"]) == [13.0, 13.0]
    assert list(out["low"]) == [12.0, 12.0]
    assert list(out["close"]) == [12.5, 12.5]


def test_normalize_leaves_volume_untouched():
    out = normalize_kline_precision(_sample_df(), "a", "SH.600000")
    assert list(out["volume"]) == [1000.0, 2000.0]


def test_normalize_a_share_fund_uses_3_decimals():
    df = _sample_df()
    df["code"] = "SH.513100"
    out = normalize_kline_precision(df, "a", "SH.513100")
    assert list(out["open"]) == [12.346, 12.345]


def test_normalize_does_not_mutate_input():
    df = _sample_df()
    before = df["open"].tolist()
    normalize_kline_precision(df, "a", "SH.600000")
    assert df["open"].tolist() == before  # 原 df 未被改动


def test_normalize_empty_or_none_returns_input():
    assert normalize_kline_precision(None, "a", "SH.600000") is None
    empty = pd.DataFrame()
    assert normalize_kline_precision(empty, "a", "SH.600000") is empty


def test_normalize_unknown_market_returns_unchanged():
    df = _sample_df()
    out = normalize_kline_precision(df, "mars", "X")
    assert list(out["open"]) == list(df["open"])  # 未知市场不改数据


def test_normalize_missing_ohlc_column_is_safe():
    df = _sample_df().drop(columns=["high"])
    out = normalize_kline_precision(df, "a", "SH.600000")
    assert "high" not in out.columns
    assert list(out["open"]) == [12.35, 12.35]
