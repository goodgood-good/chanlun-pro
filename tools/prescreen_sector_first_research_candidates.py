...
must subsequently be rebuilt by ``extract_sector_first_direct_facts.py``;
...
QUERY_ROW_CHECKPOINT_SCHEMA = "chanlun-terminal-query-row-checkpoint"
...
def _conservative_superset_negative_rows(
    *,
    ...
    if (
        prior.get("schema") != "chanlun-sector-first-terminal-query-plan"
        ...
        or prior.get("producer_source_sha256")
    ...
    prior_structural_hashes = {
        path: digest
        for path, digest in prior_hashes.items()
        if _is_structural_superset_path(path)
    }
    current_structural_hashes = {
        path: digest
        for path, digest in current_hash_map.items()
        if _is_structural_superset_path(path)
    }
    if not current_structural_hashes or (
        prior_structural_hashes != current_structural_hashes
    ):
    ...
    proof = {
        ...
        "structural_hash_path_count": len(current_structural_hashes),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.no_three_program:
        ...
        if (
            ...
            raise ValueError("current-sector query must use frozen research dates")
        if trigger.selection_order[:2] != (
            "QMT_CURRENT_SECTOR_TRIGGER",
            "QMT_CURRENT_MEMBERS_BACKFILLED_USER_AUTHORIZED",
        ):
            raise ValueError("current-sector query requires historical backfill trigger")
        by_code = {
            code: str(row["sector_id"])
        ...
        scope_start = parameters.requested_start
        scope_end = parameters.requested_end
        codes = tuple(
            sorted(
                code
                for code in by_code
