#!/usr/bin/env python3
"""Evaluate one exact-PIT V3 ETF_PROXY candidate without trading.
...
CSI300 market risk at the same decision time, then calls the shared V3
``evaluate_candidate`` core.  Optional ledger fields can be passed through to
produce ``chanlun-v3-frozen-decision-fact-ledger/v1`` for the replay builder.
"""
...
REQUEST_SCHEMA = "chanlun-v3-etf-proxy-candidate-request/v1"
RESULT_SCHEMA = "chanlun-v3-etf-proxy-candidate-evaluation/v1"
...
def evaluate_request(
    ...
    *,
    ...
    payload: dict[str, object] = {
        "schema": "chanlun-v3-etf-proxy-candidate-cli-output/v1",
        "generated_at": datetime.now(CN).isoformat(),
