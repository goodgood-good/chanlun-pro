#!/usr/bin/env python3
"""Causal sector-first direct-recursive V3 research replay.
...
from chanlun.decision_support.trading_system.v3_decision import (  # noqa: E402
...
from chanlun.decision_support.trading_system.v3_human_review_screening import (  # noqa: E402
    ...
    review_priority,
    sector_ranking_review_evidence_from_historical_audit,
...
from chanlun.decision_support.trading_system.v3_multisymbol_replay import (  # noqa: E402
    ...
    V3_SECTOR_TECHNICAL_REQUIRED_CANDIDATE_GATES,
...
from chanlun.decision_support.trading_system.v3_qmt_native_daily_bridge import (  # noqa: E402
...
from chanlun.decision_support.trading_system.v3_selection import (  # noqa: E402
    CandidateDecision,
    GateCheck,
)
...
# initializers can pull database, exchange and forward-paper adapters while
...
DEFAULT_ROOT = Path("audit/chanlun_trading_system_backtest/sector_first_full_market_v2")
...
_ZERO = Decimal("0")
FEE_SCHEDULE_ID = "A_SHARE_RESEARCH_2025_V1"
...
def parser() -> argparse.ArgumentParser:
    ...
    value.add_argument(
        ...
        help="evaluate one same-session forward-paper observation",
    )
    value.add_argument(
    ...
    value.add_argument(
        "--holding-state",
        type=Path,
        help="prior cumulative paper state whose open symbols require exit monitoring",
    )
    value.add_argument(
        "--batch-output",
        type=Path,
        help="persist the exact session ReplayBatch tuple for cumulative paper replay",
    )
    value.add_argument(
        "--active-signal-state",
        type=Path,
        help="prior persistent strategic/tactical paper signal checkpoint",
    )
    value.add_argument(
        "--signal-state-output",
        type=Path,
        help="write persistent signal state after this session's grids",
    )
    return value
...
def _decision_source_snapshot_matches_current(
    ...
    """Return False for legacy, malformed, forged or stale result metadata."""
...
def _loaded_replay_source_paths(
    ...
    for name, module in tuple(sys.modules.items()):
        ...
        if relative.endswith(".py") and (
            ...
            or relative == "tools/backtest_v3_sector_first_full_market.py"
        ):
            paths.add(relative)
...
def _direct_checkpoint_binding(
    ...
    if manifest.get("schema") != "chanlun-v3-sector-first-direct-extract/v3":
...
def _load_active_signal_state(path: Path) -> dict[str, dict[str, Signal]]:
    try:
        value = pickle.loads(path.read_bytes())
    except (OSError, EOFError, pickle.PickleError, ValueError, AttributeError) as exc:
        raise ValueError("persistent paper signal checkpoint cannot be read") from exc
    if not isinstance(value, dict) or set(value) != {"strategic", "tactical"}:
        raise ValueError("persistent paper signal checkpoint is invalid")
    output: dict[str, dict[str, Signal]] = {}
    for name in ("strategic", "tactical"):
        rows = value[name]
        if not isinstance(rows, dict) or any(
            not isinstance(code, str)
            or not isinstance(signal, Signal)
            or signal.symbol != code
            for code, signal in rows.items()
        ):
            raise ValueError("persistent paper signal rows are invalid")
        output[name] = dict(rows)
    return output
...
def _holding_contexts(
    *,
    ...
    try:
        ...
        raise ValueError("cumulative forward holding state cannot be read") from exc
    if (
        not isinstance(state, Mapping)
        or state.get("schema") != "chanlun-v3-forward-cumulative-state/v1"
        or state.get("strategy_parameter_set_id")
        != "sha256:7c7f7f0fe638110ad891b5f98f87f6f4b784bfd15980239261c964f80d06cf0b"
        or state.get("live_status") != "LIVE_DISABLED"
        or state.get("real_account_accessed") is not False
        or state.get("real_order_transport_enabled") is not False
    ):
        raise ValueError("cumulative forward holding state is invalid")
    stable_state = {key: value for key, value in state.items() if key != "content_sha256"}
    if state.get("content_sha256") != sha256_json(stable_state):
        raise ValueError("cumulative forward holding state hash changed")
    if date.fromisoformat(str(state.get("through_session"))) >= session:
        raise ValueError("cumulative forward holding state is not from a prior session")
    raw_positions = state.get("positions")
    if not isinstance(raw_positions, list):
        raise ValueError("cumulative forward positions are unavailable")
    ...
    output: dict[str, SymbolContext] = {}
    for raw in raw_positions:
        if not isinstance(raw, Mapping):
            raise ValueError("cumulative forward position row is invalid")
        code = str(raw.get("symbol") or "")
        opened_at = datetime.fromisoformat(str(raw.get("opened_at") or ""))
        if not code or opened_at.tzinfo is None or opened_at > end_at:
            raise ValueError("cumulative forward position identity/time is invalid")
        ...
        frame = load_qmt_frame(
            ...
            end_at=end_at,
            factors=qmt_factor_frame(factors),
        )
        if frame.empty:
            raise ValueError(f"held symbol one-minute data is unavailable: {code}")
        evidence = final_confirmed_structure_events(
            code,
            "1m",
            frame,
            visibility_windows=((opened_at, end_at),),
        )
        points = tuple(
            point for point in evidence.points if point.available_at >= opened_at
        )
        point_ids = {point.point_id for point in points}
        anchors = tuple(
            row for row in evidence.point_anchor_unit_ids if row[0] in point_ids
        )
        source_revision = sha256_json(
            {
                "schema": "chanlun-v3-forward-held-symbol-source/v1",
                "code": code,
                "opened_at": opened_at,
        ...
        counts = Counter(
            f"level{point.recursive_level}:{point.point_type}" for point in points
        )
        facts = SectorFirstDirectSymbolFacts(
            schema="chanlun-v3-sector-first-direct-symbol-facts/v1",
            ...
            source_start=pd.Timestamp(frame["date"].iloc[0]).to_pydatetime(),
            source_end=pd.Timestamp(frame["date"].iloc[-1]).to_pydatetime(),
            one_minute_row_count=len(frame),
            sector_trigger_windows=(),
            direct_decisions=(),
            strategic_sell_points=tuple(
                point
                for point in points
                if point.recursive_level == 2
                and point.point_type == "3sell"
                and point.side == "sell"
            ),
            structural_points=points,
            completed_units=evidence.completed_units,
            completed_trends=evidence.completed_trends,
            point_anchor_unit_ids=anchors,
            point_counts=tuple(sorted(counts.items())),
            security_master=security,
            memberships=memberships,
            factors=factors,
        )
        output[code] = _load_context(facts)
    return output
    ...
    try:
        ...
        return evidence.gate, evidence.snapshot_id, evidence.reason_codes, {
            ...
            **(
                ...
                if hasattr(evidence, "warmup_convergence_evidence")
                else {}
            ),
        ...
        source = sha256_json(
            {
                "schema": "v3-native-daily-risk-error/v2",
                "symbol": symbol,
                "at": observed_at,
        ...
        return "UNRESOLVED", sha256_json(
            {"schema": "v3-risk-error/v1", "symbol": symbol, "at": observed_at}
...
def _sector_risk_gate(
    *,
    ...
    unresolved_identity = sha256_json(
        {
            "schema": "v3-sector-risk-error/v1",
            "sector_id": sector_id,
            "at": observed_at,
        }
    )
    ...
    try:
        ...
        return evidence.gate, evidence.snapshot_id, evidence.reason_codes, {
            ...
            **(
                ...
                if hasattr(resolution, "strict_warmup_convergence_evidence")
                else {}
            ),
...
def _reconciled_market_calendar(
    ...
    revision = sha256_json(
        {
            "schema": "chanlun-qmt-two-index-realized-calendar/v1",
    ...
    return left, {
        "contract_id": "chanlun-qmt-two-index-realized-calendar/v1",
...
def _candidate(
    *,
    ...
    forward_pit = trigger.selection_order[:2] == (
    ...
    required_gates = (
        V3_SECTOR_TECHNICAL_REQUIRED_CANDIDATE_GATES
        if current_mode
        else V3_INDIVIDUAL_REQUIRED_CANDIDATE_GATES
    )
    ...
    base.update(
        {
            "sector_trigger": (
                ...
                (
                    ...
                    if current_mode and triggered and not forward_pit
                    else (
    ...
    audit = {
        ...
        "sector_ranking_source_profile": (
            None if sector_rank is None else "HISTORICAL_TRIGGER_SUMMARY"
        ),
        "sector_name": (
            ...
            for entry_dispatch, chain, strategic in accepted_approximate_entries:
                ...
                if exact_sell is not None and exact_sell.available_at == exit_at:
                    ...
                    identity = sha256_json(
                        {
                            "schema": "v3-approximate-invalidation-exit/v1",
                            "symbol": code,
...
def _signal_event_id(signal: Signal, decision_at: datetime) -> str:
    ...
    return (
        f"V3FM:{decision_at.isoformat()}:{signal.kind}:"
...
def _build_batches_with_state(
    *,
    ...
    for ordinal, (decision_at, valuation_at) in enumerate(grids):
        # Forward paper persists the active dictionary through the whole
        # trading session and reconciles it only at the end-of-day checkpoint.
        # Historical replay must use the identical boundary: a lifecycle
        # resolved on D remains eligible for idempotent suppression on D, then
        # disappears before the first grid of the next session.
        retired = _retire_active_before_session(
            {
        ...
        strategic_active = retired["strategic"]
        tactical_active = retired["tactical"]
        new = sorted(
        ...
        batches.append(
            ReplayBatch(
                batch_id=f"V3FM:{ordinal:06d}:{decision_at.isoformat()}",
                decision_at=decision_at,
                valuation_at=valuation_at,
                events=events,
...
def _retire_active_before_session(
    ...
    *,
    ...
    daily forward checkpoint and prevents a stale strategic exit from masking
...
def _build_batches(
    *,
    ...
    batches, _active = _build_batches_with_state(
        contexts=contexts,
        grids=grids,
        signals=signals,
        technical_approximation=technical_approximation,
...
def _retire_resolved_active_signals(
    active_state: Mapping[str, Mapping[str, Signal]],
    replay_result: object,
) -> dict[str, dict[str, Signal]]:
    """Remove completed/no-op exits from the next forward checkpoint.

    Batch construction must preserve a persistent exit until the event-sourced
    replay proves its lifecycle outcome.  Persisting the pre-replay dictionary
    used to carry already-filled exits and zero-lot tactical sells into every
    later paper session.  The replay result is the sole account authority, so
    only identities explicitly adjudicated there may be retired here.
    """

    resolved = set(
        getattr(replay_result, "resolved_persistent_intent_ids", ())
    )
    return {
        name: {
            symbol: signal
            for symbol, signal in values.items()
            if _persistent_signal_id(signal) not in resolved
        }
        for name, values in active_state.items()
    }
...
def _fee_model(start: date) -> V3FeeModel:
    return V3FeeModel(
...
def _run_historical_session_checkpoint_replay(
    *,
    ...
    """Converge a full-history schedule to daily forward checkpoints.
...
def _single_session_scheduler_audit(
    batches: Sequence[ReplayBatch],
    replay_result: object,
) -> dict[str, object]:
    """Describe the ordinary forward single-session checkpoint contract."""

    signature = _scheduler_event_signature(batches)
    event_count = sum(len(batch.events) for batch in batches)
    resolution_sessions = _persistent_resolution_sessions(batches, replay_result)
    return {
        "schema": "chanlun-v3-session-checkpoint-scheduler-audit/v1",
        "mode": "FORWARD_SINGLE_SESSION_CHECKPOINT",
        "converged": True,
        "build_pass_count": 1,
        "replay_pass_count": 1,
        ...
        "removed_stale_retry_event_count": 0,
        "resolved_persistent_signal_count": len(resolution_sessions),
        "resolution_sessions": tuple(
            {
                "persistent_intent_id": identity,
            ...
            for identity, resolved_on in sorted(resolution_sessions.items())
        ),
        "event_schedule_sha256": sha256_json({"batch_events": signature}),
        "retirement_boundary": "RESOLUTION_SESSION_END_EFFECTIVE_NEXT_SESSION",
        "forward_checkpoint_equivalent": True,
        "parameters_changed": False,
        "live_status": "LIVE_DISABLED",
    }
...
def _filter_batches(
    ...
    return tuple(
        replace(batch, events=tuple(event for event in batch.events if predicate(event)))
        for batch in batches
    )
...
def _human_review_report(
    *,
    ...
    report: dict[str, object] = {
        ...
        "result_label": (
            "FORWARD_HUMAN_REVIEW_QUEUE"
            if args.forward_paper_session is not None
            else "RECENT_YEAR_HUMAN_REVIEW_SCREEN_EVENT_STUDY"
        ),
        ...
        "data_caveats": (
            ...
            "forward runs must capture and hash the QMT sector catalog on the same session",
        ),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.human_review_only:
        ...
        incompatible = {
            "holding_state": args.holding_state,
            "batch_output": args.batch_output,
            "active_signal_state": args.active_signal_state,
            "signal_state_output": args.signal_state_output,
        }
        supplied = tuple(name for name, value in incompatible.items() if value is not None)
        if supplied:
            raise ValueError(
                "human review mode cannot consume or write replay state: "
                + ", ".join(supplied)
            )
    ...
    if args.no_three_program:
        ...
        if (
            ...
            != parameters.parameter_set_id
            or query_plan.get("forward_paper_session")
            != (
                None
                if forward_session is None
                else forward_session.isoformat()
            )
            or manifest.get("inputs", {}).get("forward_paper_session")
            != (
                None
                if forward_session is None
                else forward_session.isoformat()
        ...
        if forward_session is not None:
            if (
                ...
                or
                date.fromisoformat(str(entry["captured_at"])[:10])
                != forward_session
                ...
                raise RuntimeError("forward paper catalog/trigger is not same-session PIT")
    ...
    if args.forward_paper_session is not None and args.holding_state is not None:
        holding_contexts = _holding_contexts(
            state_path=args.holding_state.resolve(),
            pit_path=args.pit_snapshot.resolve(),
            trigger_sha256=_sha256_file(trigger_path),
            algorithm_revision=trigger.algorithm_revision,
            session=args.forward_paper_session,
        )
        holding_codes = tuple(sorted(holding_contexts))
        # Existing holdings are monitored for exits/tactical events.  They may
        # not be treated as fresh entries even if their sector also triggered.
        contexts.update(holding_contexts)
    if not contexts and args.forward_paper_session is None:
        raise RuntimeError("causal extraction supplied no auditable symbol")
    effective_start = (
        args.forward_paper_session
        if not contexts
        else min(value.facts.effective_start for value in contexts.values())
    )
    requested_end = (
        args.forward_paper_session
        if not contexts
        else max(value.facts.requested_end for value in contexts.values())
    )
    if effective_start is None or requested_end is None:
        raise RuntimeError("forward paper date is unavailable")
    ...
    active_state = (
        {"strategic": {}, "tactical": {}}
        if args.active_signal_state is None
        else _load_active_signal_state(args.active_signal_state.resolve())
    )
    if args.forward_paper_session is None:
        (
            batches,
        ...
        batches, next_active_state = _build_batches_with_state(
            contexts=contexts,
            grids=grids,
            signals=signals,
            initial_strategic_active=active_state["strategic"],
        ...
        scheduler_causality_audit = _single_session_scheduler_audit(
            batches,
            result,
        )
    batch_output = args.batch_output
    if args.forward_paper_session is not None and batch_output is None:
        batch_output = root / "forward_replay_batches.pkl"
    batch_sha256: str | None = None
    if batch_output is not None:
        batch_path = batch_output.resolve()
        _atomic_bytes(
            batch_path,
    ...
    signal_state_output = args.signal_state_output
    if args.forward_paper_session is not None and signal_state_output is None:
        signal_state_output = root / "forward_active_signals.pkl"
    signal_state_sha256: str | None = None
    # The event-sourced replay, not the pre-run scheduler, owns account truth.
    # Reconcile completed and proven-no-op persistent signals before writing a
    # forward checkpoint so they cannot leak into the next paper session.
    next_active_state = _retire_resolved_active_signals(next_active_state, result)
    if signal_state_output is not None:
        signal_state_path = signal_state_output.resolve()
        _atomic_bytes(
            signal_state_path,
            pickle.dumps(next_active_state, protocol=pickle.HIGHEST_PROTOCOL),
        )
        signal_state_sha256 = _sha256_file(signal_state_path)
    ...
    if args.forward_paper_session is None:
        ...
        exact_green = ablation_results[
            "EXACT_GREEN_HIGHER_TIMEFRAME_ONLY"
        ]
    else:
        # A single forward session is an observation checkpoint, not a
        # historical ablation study.  Keep the legacy batch projection only
        # for its local diagnostic and do not attest it as independently
        # rescheduled.
        no_tactical_batches = _filter_batches(
            batches,
            lambda event: ":TACTICAL_" not in event.event_id,
        )
        no_tactical = _run(
            batches=no_tactical_batches,
            research=research,
            initial_cash=args.initial_cash,
            technical_approximation=technical_approximation,
        )
        exact_green_ids = {
            f"{row['symbol']}|{row['decision_at']}"
            for row in candidate_audit
            if row.get("accepted") and row.get("exact_green")
        }
        exact_green_batches = _filter_batches(
            batches,
            lambda event: (
                ":ENTRY:" not in event.event_id
                or f"{event.facts.symbol}|{event.facts.decision_time}"
                in exact_green_ids
            ),
        )
        exact_green = _run(
            batches=exact_green_batches,
            research=research,
            initial_cash=args.initial_cash,
            technical_approximation=technical_approximation,
        )
    ...
    strict_empty = StrictV3MultiSymbolReplayEngine(
        initial_cash=args.initial_cash,
    ...
    report: dict[str, object] = {
        ...
        "result_label": (
            "FORWARD_PAPER_CURRENT_SECTOR_TECHNICAL_OBSERVATION"
            if args.forward_paper_session is not None
            else (
        ...
        "scope": {
            ...
            "carried_holding_symbols": holding_codes,
            "sector_count": len(trigger.sector_source_revisions),
            "selection_order": trigger.selection_order,
        },
        ...
        "input_hashes": {
            ...
            "holding_state": (
                None
                if args.holding_state is None
                else _sha256_file(args.holding_state.resolve())
            ),
            "session_replay_batches": batch_sha256,
            "prior_active_signal_state": (
                None
                if args.active_signal_state is None
                else _sha256_file(args.active_signal_state.resolve())
            ),
            "session_active_signal_state": signal_state_sha256,
        },
        "decision_source_snapshot": run_decision_source_snapshot,
        "parameter_snapshots": {
        ...
        "approximation_disclosures": (
            (*technical_disclosures,
                "QMT GICS3 membership was captured on the evaluated session and is used only after capture",
                "the individual three-program remains disabled under the frozen user-authorized parameter snapshot",
                "completed 1m bars are the minimum execution observation; no tick or order-book fill is invented",
                "when sector 5m warmup is short, native-daily current-member M/W/D is paired with 5m-derived 30m only as an unreconciled research advisory and any GREEN is capped to AMBER",
                "this is paper observation only and cannot enable a broker/account/order transport",
            )
            if args.forward_paper_session is not None
            else
            (*technical_disclosures,
                ...
                "this result is not formal V3, not survivor-bias-free, and can never enable live trading",
            )
            if current_mode
    ...
    risk_execution_headline = (
        ...
        if risk_execution_attribution is None
            ...
            "schema": (
                "chanlun-v3-higher-timeframe-execution-headline/v1"
            ),
