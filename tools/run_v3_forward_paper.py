#!/usr/bin/env python3
"""Run the frozen V3 human-review forward control plane without broker access.

``capture`` records the current QMT GICS3 catalog.  In the default
human-review mode, ``evaluate`` checks same-session sector evidence and
...
this current-QMT/manual-confirmation mode and remain hard gates in stricter
decision modes.  A read-only virtual cash/fee book may be reconstructed, but
...
from chanlun.decision_support.trading_system.v3_forward_paper import (  # noqa: E402
...
from chanlun.decision_support.trading_system.v3_trading_session import (  # noqa: E402
...
from chanlun.decision_support.trading_system.v3_forward_review_markout import (  # noqa: E402
...
from chanlun.decision_support.trading_system.v3_qmt_instrument_status_snapshot import (  # noqa: E402
...
from chanlun.decision_support.trading_system.v3_live_human_review import (  # noqa: E402
    LEGACY_UNATTESTED_SCREENING_POLICY_ID,
...
from chanlun.decision_support.trading_system.v3_technical_approximation import (  # noqa: E402
...
DEFAULT_ROOT = Path(".cache/chanlun_v3_human_review_forward")
...
DEFAULT_HUMAN_PAPER_LEDGER = Path(
    ".cache/chanlun_v3_human_review/paper_ledger.json"
)
DEFAULT_HUMAN_FEEDBACK_LEDGER = Path(
    ".cache/chanlun_v3_human_review/feedback_ledger.json"
)
FORWARD_SESSION_MANIFEST_SCHEMA = "chanlun-v3-forward-session-manifest/v1"
FORWARD_ATTEMPT_RECEIPT_SCHEMA = "chanlun-v3-forward-evaluation-attempt/v1"
LEGACY_UNATTESTED_DECISION_CORE_ID = "LEGACY_UNATTESTED_DECISION_CORE"
...
def _is_screening_policy_identity(value: object) -> bool:
    return _is_sha256_identity(value) or (
        value == LEGACY_UNATTESTED_SCREENING_POLICY_ID
    )


def _is_decision_core_identity(value: object) -> bool:
    return _is_sha256_identity(value) or (
        value == LEGACY_UNATTESTED_DECISION_CORE_ID
    )


def _calculate_application_source_revision(root: Path) -> str:
    """Compatibility wrapper around the shared forward source identity."""

    return calculate_forward_application_source_revision(Path(root))


@lru_cache(maxsize=4)
def _application_source_revision(root: Path = PROJECT_ROOT) -> str:
    return _calculate_application_source_revision(root)


def _implementation_provenance_document(
    *,
...
def _forward_attempt_identity_document(
    ...
    """Return the exact v1 identity fields, excluding later attestations."""
    ...
    if set(attempt) - allowed or not identity_fields.issubset(attempt):
...
def _load_session_manifest(
    ...
    *,
    ...
    if any(
        "screening_policy_id" in item
        and not _is_screening_policy_identity(item.get("screening_policy_id"))
        for item in attempts
        if isinstance(item, Mapping)
    ):
        raise RuntimeError("forward session screening policy identity is invalid")
    if any(
        "decision_core_id" in item
        and not _is_decision_core_identity(item.get("decision_core_id"))
        for item in attempts
        if isinstance(item, Mapping)
    ):
    ...
    if promoted_policy is not None and not _is_screening_policy_identity(
        promoted_policy
    ):
        raise RuntimeError("forward session promoted policy identity is invalid")
    if promoted_policy is not None:
    ...
    if promoted_core is not None and not _is_decision_core_identity(promoted_core):
        raise RuntimeError("forward session promoted decision core identity is invalid")
    if promoted_core is not None:
        promoted_attempt = next(
            item
            for item in attempts
            if isinstance(item, Mapping) and item.get("attempt_id") == promoted
...
def _attempt_live_snapshot(
    ...
    # v1 attempts written before the page/forward identity unification used
    # the hash of the complete mutable JSON payload.  Current attempts use the
    # same semantic snapshot identity as the page, while retaining the complete
    # payload hash as an independent outer attestation.  Accept exactly those
    # two profiles; a re-hashed document may not silently switch between them.
    ...
    if payload_identity is None:
        source_valid = source_identity == exact_payload_identity
    else:
        source_valid = (
...
def _attempt_screening_policy_id(
    ...
    if declared is not None and not _is_screening_policy_identity(declared):
    ...
    if snapshot_policy_id is None and top_policy_id is None:
        # Older immutable screens carried the policy document but did not
        # declare its identity.  Preserve the repository-wide legacy label
        # instead of retroactively asserting an identity that was not signed at
        # capture time.
        derived = LEGACY_UNATTESTED_SCREENING_POLICY_ID
    elif (
...
def _attempt_decision_core_id(
    ...
    if declared is not None and not _is_decision_core_identity(declared):
    ...
    if (
        isinstance(core_document, Mapping)
        and "policy" not in core_document
        and top_core_id is None
    ):
        # Immutable screens written before the portable contract included its
        # TradingPolicy cannot be independently re-attested after the fact.
        derived = LEGACY_UNATTESTED_DECISION_CORE_ID
    elif isinstance(core_document, Mapping):
        try:
...
def _record_forward_attempt(
    *,
    ...
    # Keep the v1 attempt identity field set byte-compatible.  Older attempts
    # placed the complete-payload hash in ``source_content_sha256``; current
    # attempts place the page-parity semantic identity there and bind the exact
    # payload through the immutable live object plus its explicit
    # ``source_payload_sha256`` attestation.  The loader verifies both profiles.
    # The explicit policy field remains attested metadata, so adding it did not
    # retroactively turn an otherwise identical legacy evaluation into another
    # attempt identity.
...
def _archive_live_screening_snapshot(
    *,
    ...
    boundaries_valid = (
        payload.get("schema_version") == "chanlun-trading-screening/v3"
    ...
    # Event evidence points only at content-addressed objects.  The two legacy
    # filenames remain latest-view aliases for the page and public interface;
    # replacing an alias can no longer invalidate an earlier ledger event.
...
def _human_paper_entry_selection_settlement_gate(
    *,
    ...
    source_report_archive = {
        "schema": "chanlun-human-paper-entry-source-report-archive/v1",
        "status": (
            "COMPLETE"
    ...
    return {
        "schema": "chanlun-human-paper-entry-selection-settlement-gate/v2",
        "status": status,
...
def _archive_human_paper_ledger_prefix(
    *,
    ...
    return {
        "schema": "chanlun-human-paper-ledger-prefix-archive/v1",
        "status": "COMPLETE",
...
def _settle_human_paper(
    *,
    ...
    prior_capital_rejection_evidence = (
        audit_human_paper_capital_rejection_evidence(
            tuple(before["events"]),
            forward_root=_paths(args)[0],
        )
    ...
    if (
        any(
            event.get("kind") in {"CAPITAL_REJECT", "PORTFOLIO_REJECT"}
            for event in before["events"]
        )
        and prior_capital_rejection_evidence.get("status") != "COMPLETE"
    ):
        raise RuntimeError(
            "existing virtual capital rejections have incomplete immutable "
            "execution evidence: "
            f"{prior_capital_rejection_evidence.get('status') or 'INVALID'}"
        )
    prior_capital_decision_audit = audit_human_paper_capital_decisions(
        tuple(before["events"]),
        parameters=accounting_parameters,
    )
    if (
        any(event.get("kind") == "CAPITAL_REJECT" for event in before["events"])
        and prior_capital_decision_audit.get("status") != "COMPLETE"
    ):
        raise RuntimeError(
            "existing virtual capital rejection decisions cannot be reconstructed: "
            f"{prior_capital_decision_audit.get('status') or 'INVALID'}"
        )
    ...
    if not pending:
        ...
        return {
            ...
            "capital_rejection_evidence": prior_capital_rejection_evidence,
            "capital_decision_audit": prior_capital_decision_audit,
    ...
    capital_rejection_evidence = audit_human_paper_capital_rejection_evidence(
        tuple(after["events"]),
        forward_root=_paths(args)[0],
    )
    if (
        ...
        and capital_rejection_evidence.get("status") != "COMPLETE"
    ):
        raise RuntimeError(
            "virtual capital rejection immutable execution evidence is incomplete: "
            f"{capital_rejection_evidence.get('status') or 'INVALID'}"
        )
    capital_decision_audit = audit_human_paper_capital_decisions(
        tuple(after["events"]),
        parameters=accounting_parameters,
    )
    if (
        any(event.get("kind") == "CAPITAL_REJECT" for event in after["events"])
        and capital_decision_audit.get("status") != "COMPLETE"
    ):
        raise RuntimeError(
            "virtual capital rejection decision reconstruction failed: "
            f"{capital_decision_audit.get('status') or 'INVALID'}"
        )
    ...
    return {
        ...
        "capital_rejection_evidence": capital_rejection_evidence,
        "capital_decision_audit": capital_decision_audit,
        ...
        "capital_rejection_count": sum(
            value.get("result") in {"CAPITAL_REJECTED", "PORTFOLIO_REJECTED"}
            for value in capital_evaluations
        ),
    ...
    for symbol in symbols:
        ...
        try:
            ...
            raw_bar_revision = sha256_json(
                {
                    "schema": "chanlun-forward-markout-raw-1m-bars/v1",
                    "symbol": symbol,
                    "through_session": through_session.isoformat(),
                    "rows": tuple(
                        {
    ...
    *,
    ...
    return {
        ...
        "warmup_structure_lineage_rollup": {
            ...
            "legacy_session_count": lineage_rollup["legacy_session_count"],
    ...
    _document, event, reused = _append(
        ...
        evidence={
            ...
            "human_confirmation_required": (
                contract.technical_mode == "HUMAN_REVIEW_SCREENING"
            ),
...
def _session_pit_snapshot(
    *,
    ...
    """Use a current session snapshot; refresh only in the pre-decision phase."""
    ...
    command = (
        sys.executable,
        str(PROJECT_ROOT / "tools" / "snapshot_qmt_pit_metadata.py"),
        "--start",
        recent_year_research_parameters().warmup_start.isoformat(),
        "--end",
        session.isoformat(),
        "--output",
        str(daily),
    ...
    result = _run_logged(
        command=command,
        log_path=session_dir / "pit_snapshot.log",
    )
    if result.returncode != 0 or not daily.is_file():
        return (
            daily,
            {
                "pit_snapshot_refreshed": False,
                "pit_snapshot_log": str(session_dir / "pit_snapshot.log"),
                "pit_snapshot_exit_code": result.returncode,
            },
            "PIT_METADATA_REFRESH_FAILED",
        )
    try:
        snapshot = load_snapshot(daily)
    except Exception as exc:
        return daily, {"pit_error": f"{type(exc).__name__}: {exc}"}, "PIT_METADATA_INVALID"
    if snapshot.source_end < session:
        return (
            daily,
            {"pit_source_end": snapshot.source_end.isoformat()},
            "PIT_SECURITY_AND_ACTION_METADATA_NOT_CURRENT",
        )
    completed_at = datetime.fromtimestamp(daily.stat().st_mtime, tz=CN)
    if snapshot.captured_at.date() != session or completed_at > datetime.combine(
        session, time(15), tzinfo=CN
    ):
        return (
            daily,
            {
            ...
            "PIT_METADATA_NOT_CAPTURED_BEFORE_SESSION_CLOSE",
        )
    return (
        daily,
        {
            ...
            "pit_captured_at": snapshot.captured_at.isoformat(),
            "pit_completed_at": completed_at.isoformat(),
            "pit_snapshot_sha256": sha256_file(daily),
            "pit_snapshot_refreshed": True,
            "pit_snapshot_log": str(session_dir / "pit_snapshot.log"),
        },
        None,
    )
...
def _forward_pipeline(
    *,
    ...
    assert args.qmt_local_data_dir is not None
    contract = load_frozen_forward_contract(args.parameter_snapshot.resolve())
    human_review_mode = contract.technical_mode == "HUMAN_REVIEW_SCREENING"
    common = {
        "pit": str(pit_path.resolve()),
        "qmt": str(args.qmt_local_data_dir.resolve()),
        "catalog": str(args.sector_ledger.resolve()),
        "root": str(session_dir),
        "session": session.isoformat(),
    }
    cumulative_state = root / "cumulative_state.json"
    holding_args: tuple[str, ...] = (
        ("--holding-state", str(cumulative_state))
        if not human_review_mode and cumulative_state.is_file()
        else ()
    )
    active_state = root / "active_signal_state.pkl"
    active_args: tuple[str, ...] = (
        ("--active-signal-state", str(active_state))
        if not human_review_mode and active_state.is_file()
        else ()
    )
    result_path = session_dir / (
        "forward_human_review_screen.json"
        if human_review_mode
        else "forward_paper_result.json"
    )
    decision_outputs: tuple[str, ...] = (
        ...
        if human_review_mode
            ...
            "--batch-output",
            str(session_dir / "forward_replay_batches.pkl"),
            "--signal-state-output",
            str(session_dir / "forward_active_signals.pkl"),
            *holding_args,
            *active_args,
        )
    )
    steps: tuple[tuple[str, tuple[str, ...]], ...] = (
        (
            ...
            (
                sys.executable,
                str(PROJECT_ROOT / "tools" / "build_v3_recent_year_current_sector_triggers.py"),
                "--pit-snapshot",
                common["pit"],
                "--catalog-ledger",
                common["catalog"],
        ...
        (
            ...
            (
                sys.executable,
                str(PROJECT_ROOT / "tools" / "prescreen_v3_sector_first_research_candidates.py"),
                "--warmup-start",
                recent_year_research_parameters().warmup_start.isoformat(),
                "--effective-start",
                common["session"],
                ...
                common["pit"],
                "--trigger-ledger",
                str(session_dir / "sector_first_trigger_ledger.pkl"),
                "--no-three-program",
                "--current-catalog-ledger",
                common["catalog"],
                "--output",
                str(session_dir / "terminal_query_plan.json"),
                "--qmt-local-data-dir",
                common["qmt"],
                "--forward-paper-session",
                common["session"],
        ...
        (
            ...
            (
                sys.executable,
                str(PROJECT_ROOT / "tools" / "extract_v3_sector_first_direct_facts.py"),
                "--warmup-start",
                recent_year_research_parameters().warmup_start.isoformat(),
                "--start",
                common["session"],
        ...
        (
            "human_review_screen" if human_review_mode else "decision_replay",
            (
                sys.executable,
                str(PROJECT_ROOT / "tools" / "backtest_v3_sector_first_full_market.py"),
                "--root",
                common["root"],
                "--pit-snapshot",
                common["pit"],
    ...
    for name, command in steps:
        ...
        result = _run_logged(command=command, log_path=log_path)
        row = {
            "step": name,
            "exit_code": result.returncode,
            "log": str(log_path),
    ...
    try:
        ...
        return None, {
            "failed_step": "decision_report_validation",
            "error": f"{type(exc).__name__}: {exc}",
            "steps": tuple(step_rows),
        }
    if not isinstance(report, Mapping):
        return None, {
            "failed_step": "decision_report_binding",
            "steps": tuple(step_rows),
        }
    common_binding_failed = (
        report.get("forward_paper_session") != session.isoformat()
        or report.get("live_status") != "LIVE_DISABLED"
    )
    human_binding_failed = human_review_mode and (
        report.get("schema") != "chanlun-v3-human-review-screen/v1"
        or report.get("data_grade") != "HUMAN_REVIEW_SCREENING"
        or report.get("highest_status") != "REVIEW_REQUIRED"
        or report.get("portfolio_backtest_performed") is not False
        or report.get("orders_created") != 0
        or report.get("fills_created") != 0
        or report.get("automated_order_authorized") is not False
        or report.get("human_confirmation_required") is not True
    )
    replay_binding_failed = not human_review_mode and (
        report.get("data_grade") != "RESEARCH_APPROXIMATION"
    )
    if common_binding_failed or human_binding_failed or replay_binding_failed:
        return None, {
            "failed_step": "decision_report_binding",
            "steps": tuple(step_rows),
        }
    if human_review_mode:
        evidence = {
            ...
            "result_content_sha256": report.get("content_sha256"),
            "result_label": report.get("result_label"),
            "candidate_funnel": report.get("candidate_funnel"),
            "signal_counts": report.get("signal_counts"),
            "event_study": report.get("event_study"),
            "review_queue_count": len(report.get("review_queue") or ()),
            "portfolio_backtest_performed": False,
            "orders_created": 0,
            "fills_created": 0,
            "automated_order_authorized": False,
    ...
    try:
        cumulative = _cumulative_replay(root=root, through_session=session)
    except Exception as exc:
        return None, {
            "failed_step": "cumulative_paper_replay",
            "error": f"{type(exc).__name__}: {exc}",
            "steps": tuple(step_rows),
        }
    evidence = {
        ...
        "candidate_funnel": report.get("candidate_funnel"),
        "signal_counts": report.get("signal_counts"),
        "session_replay_metrics": report.get("research_variant_result", {})
        .get("replay", {})
        .get("metrics"),
        "cumulative_state": str(root / "cumulative_state.json"),
        "cumulative_state_sha256": sha256_file(root / "cumulative_state.json"),
        "cumulative_metrics": cumulative.get("metrics"),
        "cumulative_positions": cumulative.get("positions"),
        "steps": tuple(step_rows),
        "real_account_accessed": False,
        "real_order_transport_enabled": False,
    }
...
def _cumulative_replay(*, root: Path, through_session: date) -> dict[str, object]:
    """Re-run every immutable daily batch so cash, T+1 and positions carry."""

    from tools.backtest_v3_sector_first_full_market import _run
        ...
        sorted(
            (
                ...
                for path in (root / "sessions").glob(
                    "*/forward_replay_batches.pkl"
                )
                if date.fromisoformat(path.parent.name) <= through_session
            ),
            key=lambda value: value.parent.name,
        )
    )
    if not batch_files or batch_files[-1].parent.name != through_session.isoformat():
        raise RuntimeError("current forward session replay batches are unavailable")
    batches: list[ReplayBatch] = []
    identities: list[dict[str, object]] = []
    for path in batch_files:
        values = pickle.loads(path.read_bytes())
        if not isinstance(values, tuple) or any(
            not isinstance(value, ReplayBatch) for value in values
        ):
            raise ValueError(f"invalid forward replay batch checkpoint: {path}")
    ...
    if len({row.batch_id for row in ordered}) != len(ordered):
        raise ValueError("forward cumulative replay contains duplicate batch ids")
    first_session = date.fromisoformat(batch_files[0].parent.name)
    market = load_qmt_frame(
        "SH.000001",
        "1m",
        start_at=datetime.combine(first_session, time(9, 30), tzinfo=CN),
        end_at=datetime.combine(through_session, time(15), tzinfo=CN),
    )
    market_sessions = {
        pd.Timestamp(value).date() for value in market["date"]
    }
    replay_sessions = {date.fromisoformat(path.parent.name) for path in batch_files}
    missing_sessions = tuple(sorted(market_sessions - replay_sessions))
    if missing_sessions:
        raise RuntimeError(
            "forward cumulative replay has unobserved market sessions: "
    ...
    result = _run(
        ...
        technical_approximation=technical,
    )
    session_signal_path = (
        root
        / "sessions"
        / through_session.isoformat()
        / "forward_active_signals.pkl"
    )
    if not session_signal_path.is_file():
        raise RuntimeError("current forward active-signal checkpoint is unavailable")
    signal_state = pickle.loads(session_signal_path.read_bytes())
    if not isinstance(signal_state, dict) or set(signal_state) != {
        "strategic",
        "tactical",
    }:
        raise ValueError("current forward active-signal checkpoint is invalid")
    open_symbols = {value.symbol for value in result.positions}
    pruned_signal_state: dict[str, dict[str, object]] = {}
    for name in ("strategic", "tactical"):
        rows = signal_state[name]
        if not isinstance(rows, dict):
            raise ValueError("current forward active-signal rows are invalid")
        pruned_signal_state[name] = {
            code: signal for code, signal in rows.items() if code in open_symbols
        }
    active_state_path = root / "active_signal_state.pkl"
    _atomic_bytes(
        active_state_path,
        pickle.dumps(pruned_signal_state, protocol=pickle.HIGHEST_PROTOCOL),
    )
    stable: dict[str, object] = {
        "schema": "chanlun-v3-forward-cumulative-state/v1",
        "through_session": through_session.isoformat(),
        "strategy_parameter_set_id": (
            "sha256:7c7f7f0fe638110ad891b5f98f87f6f4b784bfd15980239261c964f80d06cf0b"
        ),
        "technical_mode": "RESEARCH_APPROXIMATION",
        "technical_approximation_parameter_set_id": technical.parameter_set_id,
        "batch_files": tuple(identities),
        "initial_cash": result.initial_cash,
        "final_cash": result.final_cash,
        "positions": result.positions,
        "closed_cycles": result.closed_cycles,
        "metrics": result.metrics,
        "unresolved_reason_codes": result.unresolved_reason_codes,
        "active_signal_state_sha256": sha256_file(active_state_path),
        "active_strategic_signal_count": len(pruned_signal_state["strategic"]),
        "active_tactical_signal_count": len(pruned_signal_state["tactical"]),
    ...
    document = {
        **stable,
        "content_sha256": sha256_json(_jsonable(stable)),
    }
    _atomic_json(root / "cumulative_state.json", document)
    return dict(_jsonable(document))
...
def _evaluate(args: argparse.Namespace) -> int:
    ...
    if contract.technical_mode == "HUMAN_REVIEW_SCREENING":
        ...
        _document, evaluated, evaluated_reused = _append(
            ...
            # would make otherwise valid ledgers unreadable by the v1 loader.
            status="EVALUATED",
            evidence=archive_evidence,
        )
        _print(
        ...
        return 0
    report, pipeline_evidence = _forward_pipeline(
        args=args,
        session=session,
        pit_path=pit_path,
    )
    ...
    _print(
        {
            ...
            "result": {
                ...
                "candidate_funnel": report.get("candidate_funnel"),
                "signal_counts": report.get("signal_counts"),
                "orders_created": report.get("orders_created"),
                "fills_created": report.get("fills_created"),
                "highest_status": report.get("highest_status"),
    ...
    _print(
        {
            ...
            "paper_capital_controls": {
                "cash_and_slot_pretrade_enforced": True,
                "capital_rejected_intent_count": len(
                    human_paper_capital_rejected_intent_ids(
                        tuple(paper_ledger["events"])
                    )
                    | human_paper_portfolio_rejected_intent_ids(
