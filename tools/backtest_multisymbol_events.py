#!/usr/bin/env python3
"""Replay a strict strategy multi-symbol point-in-time event artifact.
...
derives nor repairs a trading signal.  The shared strict strategy decision core and shared
...
INPUT_SCHEMA = "chanlun-strict-multisymbol-replay-input"
OUTPUT_SCHEMA = "chanlun-strict-multisymbol-replay-result"
...
def _parse_fee_model(value: object) -> FeeModel:
...
def _parse_candidate(value: object) -> CandidateDecision | None:
    ...
    return CandidateDecision(
        ...
        higher_timeframe_risk_buyable=_boolean(
            _required(row, "higher_timeframe_risk_buyable"),
            "candidate.higher_timeframe_risk_buyable",
        ),
    )


def _parse_chain(value: object) -> AlignedEntryChain | None:
    if value is None:
        return None
...
def _parse_facts(value: object) -> DecisionInput:
...
def _validate_builder_contract(payload: Mapping[str, object]) -> Mapping[str, object]:
    ...
    produced by a different alignment or override identity would still be
...
def run_payload(
    ...
    *,
    ...
    engine = StrictMultiSymbolReplayEngine(
