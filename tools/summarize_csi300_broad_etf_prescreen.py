#!/usr/bin/env python3
"""Summarize the fixed eight-symbol CSI300 broad-ETF strict strategy prescreen."""
...
DEFAULT_OUTPUT = Path(
    "audit/chanlun_live_integration/csi300_broad_etf_prescreen_summary.json"
)
DEFAULT_MARKDOWN = Path(
    "audit/chanlun_live_integration/csi300_broad_etf_prescreen_summary.md"
)
...
def _load_verified_universe(path: Path) -> tuple[Mapping[str, object], tuple[str, ...]]:
    ...
    if (
        payload.get("schema") != "chanlun-csi300-broad-etf-universe"
...
def _artifact_path(directory: Path, symbol: str) -> Path:
    return directory / f"cached_symbol_prescreen_{symbol[:6]}.json"
...
def build_summary(
    *,
    ...
    result: dict[str, object] = {
        "schema": "chanlun-csi300-broad-etf-prescreen-summary",
...
def render_markdown(summary: Mapping[str, object]) -> str:
    ...
    return "\n".join(
        (
            "# 统一策略固定沪深300宽基 ETF 池严格结构预筛",
            "",
            "结论：固定 8 只池没有形成一条合法 L0→L1→L2 入场链，"
            "因此本产物不能进入组合收益评价，也不能给出完整统一策略收益结论。",
            "",
