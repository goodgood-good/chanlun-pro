...
from chanlun.decision_support.trading_system.v3_etf_proxy_facts import (  # noqa: E402
    DailyMarketBar,
)
from chanlun.decision_support.trading_system.v3_sector_strength_replay import (  # noqa: E402
...
def parser() -> argparse.ArgumentParser:
    ...
    value.add_argument(
        ...
        help="build only same-session forward-paper events after the PIT capture",
    )
    value.add_argument(
        "--reclassify-existing",
        action="store_true",
...
def _catalog_scope(
    *,
    ...
    selected_start = scope_start or parameters.requested_start
    selected_end = scope_end or parameters.requested_end
...
def _pit_snapshot_available_at(
    *,
    path: Path,
    snapshot: PITMetadataSnapshot,
    forward_session: date | None,
) -> tuple[datetime, str]:
    """Return byte-stable availability for historical release copies.
    ...
    research therefore uses the content-bound ``captured_at``.  A same-session
    forward capture still keeps the stricter in-place publication-time check.
    """

    if forward_session is None:
        return snapshot.captured_at, "CONTENT_CAPTURED_AT_HISTORICAL"
    file_available_at = datetime.fromtimestamp(path.stat().st_mtime, tz=CN)
    return (
        max(snapshot.captured_at, file_available_at),
        "MAX_CONTENT_CAPTURED_AT_AND_FILE_MTIME_FORWARD",
    )
...
def _build_member_daily(code: str) -> ReplaySectorMemberDailySeries:
    ...
    source_revision = sha256_json(
        {
            "schema": "chanlun-v3-sector-strength-member-daily-source/v1",
            "symbol": code,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    forward_session = args.forward_paper_session
    catalog_captured = datetime.fromisoformat(str(catalog_entry["captured_at"]))
    if forward_session is not None and catalog_captured.date() != forward_session:
        raise RuntimeError("forward paper requires a same-session QMT catalog capture")
    members, names, excluded = _catalog_scope(
        catalog_entry=catalog_entry,
        snapshot=snapshot,
        scope_start=forward_session,
        scope_end=forward_session,
    )
    start_at = datetime.combine(parameters.warmup_start, time(9, 30), tzinfo=CN)
    end_date = forward_session or parameters.requested_end
    end_at = datetime.combine(end_date, time(15), tzinfo=CN)
    ...
    effective_at = datetime.combine(
        forward_session or parameters.effective_start,
        time(9, 30),
        tzinfo=CN,
    )
    observed_times = tuple(
        ...
        and (
            ...
            or (
                value.date() == forward_session
                and value
                >= max(catalog_captured, snapshot.captured_at, pit_available_at)
            )
        )
    )
    ...
    member_source_stable = {
        "schema": "chanlun-v3-sector-strength-member-source-manifest/v1",
    ...
    strength_manifest_stable = {
        "schema": "chanlun-v3-sector-strength-replay-manifest/v1",
        "benchmark_symbol": "SH.000300",
        "catalog_entry_sha256": catalog_entry["entry_sha256"],
    ...
    trigger = build_current_sector_trigger_ledger(
        ...
        membership_mode=(
            ...
            else "CURRENT_CAPTURE_BACKFILLED_USER_AUTHORIZED"
        ),
    ...
    report.update(
        {
            ...
            "current_membership_backfilled": forward_session is None,
            "same_session_membership_capture": forward_session is not None,
            "survivorship_bias_accepted_for_research": forward_session is None,
