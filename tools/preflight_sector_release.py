#!/usr/bin/env python3
"""Run the bounded strict strategy sector-release gates without launching a full replay.
...
SCHEMA = "chanlun-sector-release-preflight"
...
def parser() -> argparse.ArgumentParser:
    ...
    value.add_argument(
        ...
        help=(
            ...
            "release_manifest.json beside --artifact"
        ),
    )
...
def _coverage_candidate(
    ...
    *,
    ...
    for raw in _sequence(artifact.get("candidate_audit"), "candidate_audit"):
        ...
        rows.append(row)
    if not rows:
        raise ValueError("no current strict strategy sector-coverage candidate matches the sample")
    rows.sort(
        key=lambda row: (
...
def _validate_physical_projection(
    ...
    if parsed.document() != dict(coverage):
        raise ValueError("sector coverage is not a canonical strict strategy document")
    expected = {
...
def run(args: argparse.Namespace) -> dict[str, object]:
    ...
    release_path = (
        artifact_path.parent / "release_manifest.json"
    ...
    if release_path.is_file():
        ...
        raise ValueError(
            "current release artifact requires release_manifest.json"
        )
    else:
