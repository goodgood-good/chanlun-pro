#!/usr/bin/env python3
"""Read and normalize point-in-time data for the strict Chanlun v3 audit.
...
def _merge_rows(rows: pd.DataFrame, *, completed_at: pd.Timestamp) -> dict[str, object]:
    ...
    return {
        ...
        "previous_close": (
            None if pd.isna(first["previous_close"]) else float(first["previous_close"])
        ),
        "volume": float(ordered["volume"].sum()),
        "amount": float(ordered["amount"].fillna(0).sum()),
        "source_row_count": len(ordered),
        "source_first_time": ordered.iloc[0]["source_time"],
        "source_last_time": ordered.iloc[-1]["source_time"],
    }
...
def aggregate_completed_bars(
    ...
    *,
    ...
    if minutes not in {5, 30}:
        raise ValueError("strict v3 intraday aggregation only permits 5 or 30 minutes")
    if one_minute.empty:
        return pd.DataFrame(
