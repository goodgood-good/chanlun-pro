...
PIT_DATABASE = Path(".cache/chanlun_v3_external_pit/etf_proxy_pit.sqlite3")
...
ETF_DATABASES = {
    ...
    "SZ.159919": (
        Path(".cache/chanlun_v31_159919/financial_data_query_bars.sqlite3"),
        "159919.SZ",
    ),
}
...
OUTPUT = Path("audit/chanlun_live_integration/v3_etf_proxy_fact_audit.json")
...
def audit(
    *,
    ...
    for session in sessions:
        ...
        for symbol, mapping in mappings.items():
            selection = repository.build_selection_facts(
                mapping,
                decision_time=decision,
                reviewer="v3-etf-proxy-research-adapter",
    ...
    payload: dict[str, object] = {
        "schema": "chanlun-v3-etf-proxy-fact-audit/v1",
