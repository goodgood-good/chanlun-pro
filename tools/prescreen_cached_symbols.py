#!/usr/bin/env python3
"""Batch-prescreen cached minute symbols for strict strategy entry evidence.
...
DEFAULT_OUTPUT = Path(
    "audit/chanlun_live_integration/cached_symbol_prescreen.json"
)
...
_IMPLEMENTATION_DEPENDENCIES = (
    ...
    (PROJECT_ROOT / "tools/research_data.py").resolve(),
    (SOURCE_ROOT / "chanlun/core/cl.py").resolve(),
    (SOURCE_ROOT / "chanlun/core/macd_htf.py").resolve(),
    *tuple(
        sorted(
            (SOURCE_ROOT / "chanlun/core/strict_structure").glob("*.py"),
    ...
    (
        SOURCE_ROOT
        / "chanlun/decision_support/trading_system/timeframe_alignment.py"
    ).resolve(),
    (
        SOURCE_ROOT
        / "chanlun/decision_support/trading_system/timeframe_alignment.py"
...
def _candidate_alignment_decision_documents(
    ...
    *,
    ...
    if len(decision_ids) != len(set(decision_ids)):
        raise RuntimeError("strict strategy alignment returned duplicate L0 decisions")
    if set(decision_ids) != set(l0_available_at):
        raise RuntimeError("strict strategy alignment decisions do not cover every L0 candidate")
    ...
    *,
    ...
    for frequency in ("30m", "5m"):
        print(
            json.dumps(
                {
                    "stage": "CACHED_SYMBOL_CAUSAL_REPLAY",
