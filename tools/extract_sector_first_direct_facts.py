...
def _worker(request: WorkerRequest) -> dict[str, object]:
    ...
    if (
        ...
        or _sha256_file(
            SOURCE_ROOT
            / "chanlun/decision_support/trading_system/sector_first_direct_facts.py"
        )
...
def _manifest(
    *,
    ...
    return {
        "schema": "chanlun-sector-first-direct-extract",
        ...
        "research_variant_gate": {
            ...
            "current_membership_backfilled": current_mode,
            "technical_replay_may_run": True,
            "formal_or_survivor_bias_free_claim_allowed": False,
        },
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.no_three_program:
        ...
        if trigger.selection_order[:2] != (
            ...
            raise ValueError("current-sector extraction requires historical backfill trigger")
        ...
        if (
            query.get("schema") != "chanlun-sector-first-terminal-query-plan"
    ...
    # An empty terminal query plan is a valid historical result.  Always
    # persist the final manifest even when no worker future existed.
    _atomic_json(
        manifest_path,
        _manifest(
            args=args,
