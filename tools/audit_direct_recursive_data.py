#!/usr/bin/env python3
"""Audit data eligibility for the direct-recursive strict strategy research replay.
...
from chanlun.decision_support.trading_system.data_audit import (  # noqa: E402
...
DEFAULT_PRESCREEN = Path(
    "audit/chanlun_live_integration/direct_recursive_etf_prescreen.json"
)
DEFAULT_BACKTEST = Path(
    "audit/chanlun_live_integration/direct_recursive_component_backtest.json"
)
DEFAULT_OUTPUT = Path(
    "audit/chanlun_live_integration/direct_recursive_data_acceptance.json"
)
...
def build_audit(
    ...
    *,
    ...
    if prescreen.get("schema") != "chanlun-direct-recursive-etf-prescreen":
        raise ValueError("unsupported direct prescreen schema")
    if backtest.get("schema") != "chanlun-direct-recursive-component-backtest":
    ...
    document: dict[str, object] = {
        "schema": "chanlun-direct-recursive-data-acceptance",
        "prescreen": {
