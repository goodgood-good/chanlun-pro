#!/usr/bin/env python3
"""Produce the final go/no-go audit for a complete V3 historical backtest.
...
DEFAULT_PRESCREEN = INTEGRATION / "direct_recursive_v3_etf_prescreen.json"
DEFAULT_BACKTEST = INTEGRATION / "direct_recursive_v3_component_backtest.json"
DEFAULT_DATA_AUDIT = INTEGRATION / "direct_recursive_v3_data_acceptance.json"
DEFAULT_QMT_AUDIT = INTEGRATION / "qmt_v3_history_source_audit.json"
...
DEFAULT_OUTPUT = INTEGRATION / "v3_complete_backtest_readiness.json"
DEFAULT_MARKDOWN = INTEGRATION / "v3_complete_backtest_readiness.md"
...
def build_readiness(
    *,
        ...
        blockers.append(
            {
                ...
                "detail": (
                    ...
                    "the immutable V3 specification requires independent signed "
    ...
    stable: dict[str, object] = {
        "schema": "chanlun-v3-complete-backtest-readiness/v1",
...
def _markdown(report: Mapping[str, object]) -> str:
    ...
    lines.extend(
        (
            ...
            "## 本次新增与修改",
            "",
            "- 新增 `v3_qmt_sector_ledger.py`：QMT 板块目录不可变捕获、哈希链、同日可见性与历史回填审计。",
            "- 新增 `snapshot_qmt_gics3_sector_ledger.py`：已保存首个 66 板块点时快照，只能用于捕获之后的决策。",
            "- 新增 `audit_qmt_v3_history_sources.py`：自动检查未来上市成员与历史 tick 覆盖。",
            "- 修改 `prescreen_v3_direct_recursive.py`：每个 30m 战略点按其真实决策时刻绑定同源 M/W/D 风险；坏数据保留为 UNRESOLVED 拒绝。",
...
def main() -> int:
    ...
    prescreen = _verified_json(
        args.prescreen, "chanlun-v3-direct-recursive-etf-prescreen/v1"
    )
    backtest = _verified_json(
        args.backtest, "chanlun-v3-direct-recursive-component-backtest/v1"
    )
    data_audit = _verified_json(
        args.data_audit, "chanlun-v3-direct-recursive-data-acceptance/v1"
    )
    qmt_audit = _verified_json(
        args.qmt_audit, "chanlun-v3-qmt-history-source-audit/v1"
    )
