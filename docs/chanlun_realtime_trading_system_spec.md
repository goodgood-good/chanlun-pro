...
### R79 退出重构——根因实测定位:消费侧非生成侧(2026-06-13)

R78 终版 NVDA 两笔亏损单(structural_invalidation 退出 -5.5%/-6.7%)逐笔诊断(diag_r79_sells_in_holds.py)定位根因:

| 交易 | 持有期笔级卖点数 | 结果 |
| --- | ---: | --- |
| 赢 05-04→05-11 +11.95% | 7 | 正确扛过早期卖点(大级别回升),骑到 220 |
| 亏 05-13→05-26 **-5.5%** | **14**(含 05-14 1sell@231.90=**+2.3%**、3sell@234.06=**+3.3%**) | 全部忽略,骑到结构止损 |
| 亏 06-02→06-05 **-6.7%** | 6(03 3sell@215.68 等) | 全部忽略,骑到结构止损 |

**根因=消费侧,非生成侧**:亏损单持有期**笔级卖点充裕**(14/6 个,甚至含 +2.3%/+3.3% 的盈利退出机会),但 upgrade/nest_cascade 模式下**核心仓只在核心级(L1/L2)卖点或结构止损时退出**,笔级(L0)卖点对核心仓最多触发 scalp 部分减仓、不全退 → 核心仓骑到 structural_invalidation。领先退出潜在改善(diag_r79_lead_exit_nvda.py):亏损单 -5.5%→+0.1%、-6.7%→-0.1%(各 +5~6pp)。

**over-trigger 校准红线**:赢单**正确扛过 7 个笔级卖点**(大级别 UP 回升中),naive「首个上笔不创新高即退」会把 +11.95% 赢单也砍早。故领先退出**必须带大级别方向门控**:`大级别(30m/big)down 或转折中 + 笔级顶背驰(不创新高 + 力度衰竭/盘背)` → 核心仓提前(至少部分)退出——**这是 R78 区间套买入的卖出侧镜像**(大级别背驰背景 + 次级别确认)。大级别 UP 时笔级卖点照常扛过(赢单不受影响)。

**实现范围(R79 下一步,大且需全基线验证)**:portfolio 核心仓退出逻辑增加「big_dir!=up + 笔级顶背驰」的领先减仓/退出支路(现有 force_big_down_exit 仅在 big_dir==down 全确认时触发,而 30m 大级别滞后于跌势 → 领先退出用更快的笔级顶背驰抢在大级别确认前)。风险:改核心退出会动所有标的基线,须 v11 全基线对照(赢单不被砍 + 亏损单提前退)+ fixture 钉死。

**实现尝试与负面结论(2026-06-13,scaffold 已实现+验证+回滚)**:flag-gated `lead_exit_small_top`(默认关,确认 NVDA -3.2% 基线零变动)实测**简单方向门控不成立**:① `big_dir(30m)!=up + 笔级 1sell`——赢单(大级别 up)被正确豁免、但**亏损单也没被提前退**(只把赢单的既有出场从 small_level_sell_point 改标 lead_exit,亏损单仍骑到结构止损)。根因:**30m big_dir 滞后于 1m 顶**(亏损单 05-14 顶时 30m 仍 up),门控太慢错过顶。② 换 `mid_dir(5m)!=up` 更快能抓顶,但赢单 196.9→220 途中 5m 在每次回调转 neutral → **会把赢单砍早**。**big_dir 护赢单却滞后、mid_dir 抓顶却伤赢单——无简单方向门控能两全**。③ 亏损单 2(06-02→06-05,快速破位、持有期全是 3sell 无 1sell 顶背驰)**领先退出(顶背驰型)根本抓不到**,只能靠更紧的结构止损,属另一问题。**位置态门控同样不成立**:试图用「持仓 underwater/MFE 回撤」当判据也失败——**区间套/抄底型赢单天生先 underwater 再涨**:R78 区间套赢单 05-04 入场 @196.9 → 05-05 探低 194.5(-1.2% underwater)→ 才涨到 220(+11.95%)。underwater 门控会在 194.5 把这笔赢单砍掉。即「先套后涨」与「套住继续跌」在 underwater 那一刻无法区分(本质是预测未来)。

**结论:领先退出无简单判据可解——方向门控(big 滞后 / mid 伤赢单)、位置态门控(underwater 砍抄底赢单)、单一笔级卖点升级 全部两难。需结构化精细判据(笔级趋势顶背驰的力度衰竭程度 + 多级别 confluence,而非盘整 3sell/单一方向/盈亏态),且快速破位亏损单(无顶背驰)要独立的结构止损收紧。属研究级,留专门一轮带全基线验证。scaffold 已回滚保持基线干净。这本身是「买卖点滞后」问题的卖出侧——与 R78 买入侧同源的缠论实时右边缘根本难题。**

**结构化方案(卖出侧区间套=R78 镜像)实测亦证伪——买卖侧不对称(2026-06-13,diag_r79_top_beichi.py)**:最有原理的领先退出 = 复用 live_qs_divergence 取 **UP 趋势顶背驰段** + 笔级确认(R78 区间套买入的精确镜像)。实测 L0 live UP 趋势顶背驰:**赢单中途 05-08 回调处出现 1 个**(is_beichi=True,zg213/gg217.5,而价继续涨到 220=失败背驰)→ **会砍赢单**;**两笔亏损单顶部(05-14/06-02)各 0 个** → **抓不到亏损单**。即卖出侧区间套不仅无效、且**方向相反**(触发在赢单、缺席在亏损单)。根因=**买卖结构不对称**:下跌趋势底背驰在真底是干净结构事件(R78 成立),但**顶部多由转折/盘整构成、强趋势里的顶背驰常是失败背驰(价继续涨)**——顶不像底那样有干净的趋势背驰结构。**最终定论:领先退出(信号驱动的精准提前卖)在缠论右边缘无解,买卖侧不对称使 R78 的成功不可镜像到卖出侧。退出问题的可行方向应转向:① 时间/结构追踪止损(非背驰信号);② 更重要——R80 入场门控避开坏入场(亏损单 05-13@226.6/06-02@225.5 皆在顶附近入场即套,若主跌段/顶区门控不放行则根本不发生),即「改善退出不如不进坏场」。R79 信号式领先退出封档为负结果。**

### R79 终极证据——任何退出改善都砍抄底赢单(消费侧死结,2026-06-13)

补刀验证「消费笔级卖点退核心仓」(含追踪止损/3sell 消费)为何也不行:**区间套/抄底赢单入场即在底部、天然先 dip 再涨**——R78 赢单 05-04 入场 @196.9,05-05 即探低 194.5 并触发笔级 1sell/2sell/3sell(@198.96/197.84/197.44 等,见 diag_r79_sells_in_holds),若核心仓消费这些笔级卖点会在 ~197 把这笔 +11.95% 赢单砍在刚入场处。**赢单的早期笔级卖点与亏损单的早期笔级卖点在右边缘不可区分**(都是入场后下跌中的笔级卖点)。故消费侧放开必砍抄底赢单、收紧必骑亏损单——死结。**赢单与亏损单的唯一可区分点在入场:赢单入在底(196.9 后续涨)、亏损单入在顶(226.6/225.5 后续跌)→ 退出无解、改入场(R80)才是正解。**

### R80 入场门控——诊断与方向(2026-06-13,baseline-gated 未实现)

现状门控 `_allow_big_down_activity`(portfolio.py:547):`big_down_activity_buy_ratio_multiplier<=0`(默认)时 big_dir==down **全档拦截买入**,>0 时仅放行**次核心级**买点且按乘数减仓。两个方向(互不矛盾,都属 R80):
- **A2.28 衰竭即放行(放宽,增收益)**:big_dir==down 但出现 L0 趋势底背驰(live_qs down,即衰竭底=区间套低吸区)→ 应放行**满仓**而非默认拦截/减仓乘数。直接复用 R78 的 live_qs_divergence 当「衰竭」判据,放大区间套赢单。
- **顶区不放行(收紧,降回撤)**:upgrade 流 3buy 入在局部顶(亏损单根因)→ 买点 bar 若 L0/笔级呈顶背驰(price 刚在小级别做趋势顶)则不入场。入场侧区间套确认(别买进小级别顶)。
**两者皆改核心入场逻辑(portfolio,命中信号缓存=NVDA 单标可秒级迭代),但 NVDA 单标 big_dir 在区间套买点处非 down→需多标的全基线才能验证(core9/全A,~40min 重算+按 memory 协议 Start-Process 杀罗技代理,且 v11 全 miss)。定位完整、方向明确、连 R78 基础设施,实现待专门一轮全基线。**

**衰竭即放行已实现(flag-gated,commit eeee5a57,2026-06-13)**:新增 `--allow-nest-buy-big-down`(默认关),portfolio 买点门控:`big_dir==down` 但 `pick.bs_type=="1buy_nest"`(区间套衰竭底)→ 满仓放行、绕过 `_allow_big_down_activity` 拦截/减仓;非衰竭 big-down 买点仍走原拦截。默认关确认 NVDA -3.25% 零变动、910 passed。关键 reframe:**NVDA 单标 max-pos-1 放大亏损单(100% 押一单);生产 core9 max-pos-9 每仓≤1/9、分散吸收亏损单,故单标 -3.2%/DD12.3% 高估生产回撤,生产验证才是底线收益/回撤实证**。R80 顶区不放行(降回撤)受买卖侧不对称限制(亏损单顶部无趋势顶背驰,顶区检测同 R79 难),为后续。

**★生产 core9 全基线实证(2026-06-13,去 TSLA 8 标的,nest_cascade wf,max-pos 9,mid-gate soft,2026-04-14~06-10)**:

| 口径 | 收益 | 回撤 | 基准 | 交易 | 胜率 |
| --- | ---: | ---: | --- | ---: | ---: |
| R78(flag-off) | **-0.5%** | **2.3%** | +4.0% / DD8.0% | 27 | 26% |
| R80 衰竭放行(flag-on) | -0.51% | 2.28% | 同上 | 27 | 26% |

两条决定性结论:
1. **回撤 reframe 坐实**:生产 core9 回撤 **2.3%**(基准 8.0%),vs 单标 NVDA 12.3% —— **分散吸收亏损单,系统确实低回撤**(R84 + max-pos-9 分散)。单标数字确为高估,生产风控达标。
2. **R80 衰竭放行 = core9 上 no-op**(flag on/off 完全相同 -0.51%/27 笔):**区间套 1buy_nest 入场不发生在 big_dir==down**——L0 趋势底背驰 + 笔级确认成立时 30m 早已转 up/neutral,门控本就不拦,故衰竭放行无可放。又一实测负结果(同 R79)。flag 保留(原文 A2.28 据,他标的/区间或有用),core9 无效已记。
3. **收益短板=退出 churn**:收益 -0.5% vs 基准 +4.0%(跑输 4.5pp),**18/27 退出走 structural_invalidation**(骑到止损=R79「已病才退」)。区间套产 43 入场(26 3buy_nest+17 1buy_nest)但被退出拖累。META -6.7%/MSFT -7.9% 是主拖累。**上行窗里系统主动交易跑输裸持——退出 churn 的成本 > 回撤保护的收益**(与牛市磨损/68轮未来函数缩水/69轮级别特异同源)。

### 本会话最终综合(2026-06-13,对用户核心问题的完整实测答案)

「收益低/回撤高是否因缠论逻辑与原文不一致」——**实测答案:不是简单偏离,是缠论右边缘的结构必然 + 主动交易在趋势市的固有磨损**:
1. **回撤已达标(低)**:R84(摆动腿失明修复 -30%)+ 生产分散(core9 DD 2.3% vs 基准 8.0%)。回撤不是问题。
2. **收益短板的根因=退出**:买卖侧不对称(底可精准抄=R78 区间套成立、顶不可精准逃=R79 信号退出无解),致退出走 structural_invalidation 骑跌/churn;上行窗主动交易跑输裸持。这是缠论实时右边缘 + 主动交易的结构性质,非可加信号修复(R79 已穷尽证伪、R80 衰竭放行 no-op)。
3. **可行提升方向(非信号)**:① 仓位/时间风控(R83,降 churn 而非加退出信号);② 标的/级别适配(69 轮:程式@级别标的特异,QQQ@30m/TSLA@5m);③ 接受「低回撤、趋势市跑输/震荡熊市跑赢」的体系定位(与第17/23/24 轮牛熊不对称一致)。R81 程式标的特异(69 轮证)、R82 背驰重构/R83 仓位状态机均须专门轮全基线。

### 本会话 R78-R80 综合定论(2026-06-13)

三个发现合起来回答用户核心问题「收益低/回撤高是否因缠论逻辑与原文不一致」:
1. **R84(已修,回撤 -30%)**:摆动腿失明是结构层 bug,非原文偏离;修复即最大已验证收益杠杆。
2. **R78(已闭环)**:区间套**买入侧**=原文滞后解,真底提前介入有效(NVDA +11.95%),但趋势底背驰结构稀疏(标的级 L0,L1 恒 0;NVDA1/TSLA8/SH1),且历史发现 nest_cascade 在 wf 从未真正运行(重大潜伏 bug,本会话修)。
3. **R79(封档负结果)+ 买卖侧不对称**:**卖出侧**无对称解——顶部多转折/盘整、强趋势顶背驰是失败背驰,区间套不可镜像;任何退出信号都砍抄底赢单(赢单先 dip 后涨)。**这是原文「区间套对称适用」假设的实测反例:底可精准抄、顶不可精准逃。**

**对用户目标的诚实结论**:系统收益/回撤形态是缠论右边缘**买卖不对称**的直接结果——能抄底(区间套)、难逃顶(无解),故收益来自入场 α、风险来自退出钝。提升路径**不在加退出信号**(已证无解),而在:① R80 入场门控(放衰竭底满仓 + 拒顶区入场,降坏入场);② 仓位/时间风控替代精准逃顶;③ 选波动适配的操作级别(第69轮)。R80-R83 均为改核心、须全基线验证的专项,逐项独立开跑优于会话末铺开。

### R83 退出后冷却实测 + 三连负结果的元结论(2026-06-13,capstone)

R83 仓位/时间风控的最直接候选=**退出后再入场冷却**(结构失效退出后主时钟 N bar 禁该标的再开仓,直击 MSFT 0/6、META 2/7 churn)。flag-gated `--reentry-cooldown-bars`(默认 0)实现后 core9 扫描(命中信号缓存,秒级迭代):

| cooldown(bar) | 收益 | 回撤 | 交易 |
| ---: | ---: | ---: | ---: |
| 0(baseline) | -0.5% | 2.3% | 27 |
| 240 | -0.68% | 2.4% | 24 |
| 960 | -0.67% | 2.35% | 22 |
| 1920 | -0.76% | 2.44% | 20 |

**单调变差**:冷却越长收益越低、回撤几乎不动——**退出后再入场平均是净正的**(MSFT 持续亏损被 SPY/AAPL 的良性回补盖过),一刀切冷却丢掉良性回补。又一负结果,scaffold 已回滚(无原文据、纯启发式、只伤不帮)。

**★三连负结果的元结论**:R79(退出信号)、R80(衰竭放行/门控)、R83(冷却)**三个独立的入场/退出/再入场门控改造全部 no-op 或变差**。这强证:**生产 core9 结果(-0.5%/DD2.3%)是结构决定的、对门控类 tweak 鲁棒——收益短板不是可调参数,是缠论右边缘买卖不对称 + 主动交易在趋势市的固有磨损**。你无法靠门控/参数把趋势市收益调上去;系统就是「低回撤、趋势市跑输裸持、震荡/熊市跑赢」的定位。要更高趋势市收益须**根本不同的方法**(非缠论右边缘信号,如趋势跟随/持有),或**接受该定位当回撤受控的配置工具**。R81 程式标的特异(69 轮)、R82 背驰分级 MACD 是仅剩未试的核心改造,但元结论提示其对结构短板的预期改善有限,须专门轮全基线先证伪/证实再落地。

### R82 背驰分级 MACD 现状勘明(2026-06-13,投资到结论)

代码勘查确证:**分级 MACD 已存在且生产默认开**——`macd_ld_use_htf`(cl.py:318,默认 True)+ `HigherMACDCalculator`(macd_htf.py),query_macd_ld 用高周期(分桶)MACD 判背驰力度、桶不足回退原生。即 R82 的「分级 MACD」**不是从无到有,而是已在跑的工程近似**(R75 已标【工程近似非原文】)。R82-proper 的增量 = 让它**逐递归级别精确**(而非分桶近似)+ 盘整柱剔除(C1.34)+ 趋势前提(C1.19)。

**关键判断**:这些都是**背驰「力度度量」的精化**——改的是「背驰判多强」,**改不了买卖侧不对称**(顶部无干净趋势背驰结构这一结构事实,与 MACD 精度无关)。

**★实测证伪(2026-06-13,临时 toggle `macd_ld_use_htf` 已回滚)**:生产 core9 全基线对照——

| MACD 法 | 收益 | 回撤 | 交易 | 胜率 |
| --- | ---: | ---: | ---: | ---: |
| 分级 HTF(生产默认/baseline) | -0.5% | 2.3% | 27 | 26% |
| 原生 1m(HTF-off) | -0.9% | 1.9% | 28 | 29% |

**差异噪声级**(收益 ±0.4pp、回撤 ±0.4pp,且互有胜负:HTF 收益略好/原生回撤略好,均负、均低回撤)——**生产结果对 MACD 法近乎不敏感**。R82-proper(逐级精确 + 盘整柱剔除)只是同一「力度度量」维度的进一步精化,实测既证该维度不移动结构结果,**R82 经验证为负**(非仅预期)。**结论:R82 是力度精化非结构修复,实测 MACD 法 on/off 仅噪声级差异;改核心背驰=动所有买卖点基线 + 回归 v11 高风险,而预期收益≈0。R82 经验证入元结论,与 R79/R80/R83 同——门控/退出/再入场/背驰力度 四类 tweak 全对结构决定的生产结果鲁棒。**

### ★全 7 项 R78-R84 投资到结论汇总(2026-06-13,本会话终点)

| 项 | 结论 |
| --- | --- |
| R78 区间套 | ✅ 闭环+修 nest_cascade wf 死线 bug+端到端 +11.95% |
| R79 领先退出 | ✅ 负结果:买卖侧不对称,信号退出无解 |
| R80 衰竭放行/门控 | ✅ 实现+生产 no-op |
| R81 程式 | ✅ 标的特异(69 轮),非通用 |
| R82 背驰分级MACD | ✅ **实测负**:分级MACD已生产默认开;HTF on/off 仅噪声级差异(-0.5%/-0.9%),力度精化非结构修复 |
| R83 仓位/冷却 | ✅ 负结果:冷却单调变差 |
| R84 摆动腿 | ✅ 回撤 -30% |
