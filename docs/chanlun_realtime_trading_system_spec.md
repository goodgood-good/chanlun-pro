...
**减亏 6.56pp、退出提前 6 天、退出价 +14.8**。机理坐实：v8 摆动腿失明使 NVDA 的 L1/L2 中枢结构缺失 → 退出信号（小级别卖点/结构失效）缺位 → 被迫持有到远端结构边界 201.68；R84 恢复中枢结构后 06-03 即出现退出信号。这是「先修数据再调参」纪律的直接回报——R77 矩阵诊断的「退出杀跌/已病才退」病症，部分根源在结构层失明而非交易层规则。v9 NVDA 单标的整体 -6.69%（15 trades/30 signals）。完整 core3 v9 组合对照 v8（-4.04%/98 信号）待 TSLA 一年预热完成回填。
