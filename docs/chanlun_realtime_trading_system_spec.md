...
**实现范围(R79 下一步,大且需全基线验证)**:portfolio 核心仓退出逻辑增加「big_dir!=up + 笔级顶背驰」的领先减仓/退出支路(现有 force_big_down_exit 仅在 big_dir==down 全确认时触发,而 30m 大级别滞后于跌势 → 领先退出用更快的笔级顶背驰抢在大级别确认前)。风险:改核心退出会动所有标的基线,须 v11 全基线对照(赢单不被砍 + 亏损单提前退)+ fixture 钉死。诊断完整、设计明确,实现待专门一轮。
