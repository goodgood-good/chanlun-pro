...
**实现尝试与负面结论(2026-06-13,scaffold 已实现+验证+回滚)**:flag-gated `lead_exit_small_top`(默认关,确认 NVDA -3.2% 基线零变动)实测**简单方向门控不成立**:① `big_dir(30m)!=up + 笔级 1sell`——赢单(大级别 up)被正确豁免、但**亏损单也没被提前退**(只把赢单的既有出场从 small_level_sell_point 改标 lead_exit,亏损单仍骑到结构止损)。根因:**30m big_dir 滞后于 1m 顶**(亏损单 05-14 顶时 30m 仍 up),门控太慢错过顶。② 换 `mid_dir(5m)!=up` 更快能抓顶,但赢单 196.9→220 途中 5m 在每次回调转 neutral → **会把赢单砍早**。**big_dir 护赢单却滞后、mid_dir 抓顶却伤赢单——无简单方向门控能两全**。③ 亏损单 2(06-02→06-05,快速破位、持有期全是 3sell 无 1sell 顶背驰)**领先退出(顶背驰型)根本抓不到**,只能靠更紧的结构止损,属另一问题。**结论:领先退出不是「方向门控 + 笔级卖点升级」能解,需更精细判据(力度衰竭程度阈值 / 持仓 MFE 回撤 / 多级别 confluence),且快速破位亏损单要独立的止损收紧;属研究级,留专门一轮带全基线验证。scaffold 已回滚保持基线干净。**
