...
def advance_top_risk_state(
    ...
    """Apply only the top-fractal transitions frozen in specification v3."""
...
class TechnicalEntrySnapshot:
    ...
    def __post_init__(self) -> None:
        ...
        if self.level_relation_mode == "DIRECT_RECURSIVE":
            ...
            raise ValueError("unsupported V3 level relation mode")
...
class CandidateDecision:
    ...
    # ``None`` 仅用于兼容读取字段尚不存在的历史 payload，并按未知、最低优先级处理。
    higher_timeframe_risk_buyable: bool | None = None

    def __post_init__(self) -> None:
        if (
            self.higher_timeframe_risk_buyable is not None
            and type(self.higher_timeframe_risk_buyable) is not bool
        ):
            raise TypeError("higher-timeframe risk buyability must be bool or None")
...
def candidate_decision_sort_key(
    ...
    return (
        (
            0
            if decision.higher_timeframe_risk_buyable is True
            else 1
            if decision.higher_timeframe_risk_buyable is False
            else 2
        ),
