...
V3Level = Literal["L0", "L1", "L2"]
USER_OVERRIDE_VARIANT_ID = (
    "CL-HIER-30M5M1M-v3.0-USER-OVERRIDE-INDEPENDENT-TIMEFRAMES-v1"
)
ENTRY_ALIGNMENT_CONTRACT_ID = (
    "CL-HIER-30M5M1M-v3.0-INDEPENDENT-CAUSAL-ALIGNMENT-v1"
)
...
class IndependentTimeframeOverride:
    ...
    only the source of V3 level facts and never changes the frozen Chanlun
    ...
    base_strategy_id: str = STRATEGY_V3_ID
    ...
    def __post_init__(self) -> None:
        ...
        if self.base_strategy_id != STRATEGY_V3_ID:
            raise ValueError("override must reference the frozen v3 base strategy")
        if (
    ...
    def frequency_for(self, level: V3Level) -> str:
        return {
    ...
    def validate_point(
        ...
        *,
        ...
        if point.source_frequency != self.frequency_for(level):
            raise ValueError("point frequency does not match the V3 level mapping")
