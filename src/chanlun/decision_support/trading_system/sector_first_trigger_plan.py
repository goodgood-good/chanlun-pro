"""Causal sector-first scheduling plan for the strict strategy individual-stock path.
...
SECTOR_FIRST_TRIGGER_SCHEMA = "chanlun-sector-first-trigger-ledger"
...
class SectorTriggerRankFact:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            or sum(value for _name, value in components) != self.rank_score
        ):
            raise ValueError("ranked sector components are invalid")
...
class SectorFirstTriggerLedger:
    ...
    def __post_init__(self) -> None:
        ...
        elif self.selection_order[:2] != _CURRENT_BACKFILL_PREFIX:
            raise ValueError("sector trigger must precede stock expansion")
    ...
    *,
    ...
    facts = tuple(
        SectorTriggerRankFact(
            ...
            rank_components=tuple(sorted(row.assessment.rank_components)),
        ...
        events.append(
            SectorFirstTriggerEvent(
                ...
                candidate_symbols_sha256=sha256_json(
                    {
                        "schema": "chanlun-sector-first-candidates",
                        "observed_at": observed_at,
                        "symbols": candidates,
                    }
                ),
...
def build_current_sector_trigger_ledger(
    *,
    ...
    """Rank QMT GICS3 sectors for the authorized historical backfill.

    Current captured membership is intentionally backfilled across the frozen
    research window.  Realtime screening has its own same-session QMT path and
    cannot enter this historical builder.
    """
    ...
    for raw_time in sorted(set(observed_times)):
        ...
        events.append(
            SectorFirstTriggerEvent(
                ...
                candidate_symbols_sha256=sha256_json(
                    {
                        "schema": "chanlun-current-sector-candidates",
                        "observed_at": observed_at,
    ...
    return SectorFirstTriggerLedger(
        ...
        taxonomy="QMT_GICS3_CURRENT_BACKFILL",
        source="QMT_GICS3_CURRENT_CAPTURE_AND_LOCAL_BARS",
        selection_order=(
            *_CURRENT_BACKFILL_PREFIX,
            "MARKET_SECTOR_SYMBOL_HIGHER_TIMEFRAME_RISK",
            "DIRECT_RECURSIVE_30M_5M_1M_TECHNICAL_ENTRY",
