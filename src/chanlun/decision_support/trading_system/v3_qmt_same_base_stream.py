"""Derive causal V3 timeframes from one completed QMT one-minute stream.
...
auction bars (09:31..11:30 and 13:01..15:00).  Treating all 241 rows as regular
minutes shifts every 30-minute boundary and double-counts the session shape.
...
def normalize_qmt_opening_event_for_completed_minutes(
    ...
    way.  Other grid validation remains the caller's responsibility.
    """
    ...
    if len(rows) == 1:
        ...
        result.attrs = snapshot_attrs
        return result
    if pd.Timestamp(rows.iloc[1]["date"]).time() != time(9, 31):
        raise ValueError("QMT 09:30 opening event is not followed by 09:31")
    opening = rows.iloc[0]
    ...
    if float(opening["volume"]) > 0:
        first_bar["open"] = opening["open"]
        first_bar["high"] = max(float(opening["high"]), float(first_bar["high"]))
        first_bar["low"] = min(float(opening["low"]), float(first_bar["low"]))
        first_bar["volume"] = float(opening["volume"]) + float(
            first_bar["volume"]
        )
    merged = pd.concat(
...
def build_qmt_same_base_stream_frames(
    *,
    ...
    base_revision = sha256_json(
        {
            "schema": "chanlun-v3-qmt-same-base-stream/v1",
            "symbol": symbol,
