...
argument is not itself historical evidence.  The V3 system therefore treats
...
QMT_SECTOR_LEDGER_SCHEMA = "chanlun-v3-qmt-sector-capture-ledger/v1"
QMT_SECTOR_ENTRY_SCHEMA = "chanlun-v3-qmt-sector-capture/v1"
QMT_HISTORICAL_AUDIT_SCHEMA = "chanlun-v3-qmt-sector-history-audit/v1"
QMT_SECTOR_RECEIPT_AUDIT_SCHEMA = "chanlun-v3-qmt-sector-receipt-audit/v1"
...
def audit_sector_capture_receipts(
    *,
    ...
    """Audit immutable daily receipts against every hash-chained capture.

    A ledger entry can predate the receipt mechanism.  Such an entry remains
    valid point-in-time evidence, but it must be reported as a legacy receipt
    gap instead of receiving a fabricated retrospective receipt.
    """
    ...
    receipt_required_from = (
        min(covered_capture_dates) if covered_capture_dates else None
    )
    legacy_missing = tuple(
        value
        for value in missing
        if receipt_required_from is not None
        and datetime.fromisoformat(str(value["captured_at"])).date()
        < receipt_required_from
    )
    ...
    status = (
        ...
        if required_missing
        else "RECEIPT_COVERAGE_UNPROVEN"
        if missing and receipt_required_from is None
        else "COMPLETE"
        if not missing
        else "LEGACY_RECEIPT_GAPS"
    )
    return {
        ...
        "receipt_required_from_session": (
            None if receipt_required_from is None else receipt_required_from.isoformat()
        ),
        "legacy_missing_capture_sessions": tuple(
            str(value["captured_at"])[:10] for value in legacy_missing
        ),
        "required_missing_capture_sessions": tuple(
            str(value["captured_at"])[:10] for value in required_missing
        ),
