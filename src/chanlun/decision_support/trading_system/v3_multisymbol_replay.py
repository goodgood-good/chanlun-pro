...
V3_ETF_REQUIRED_CANDIDATE_GATES = frozenset(
    {
        "selection_path",
        "research_visibility",
        "etf_proxy",
...
class StrictV3ReplayContract:
    """Immutable identities for the user-authorized strict V3 replay path."""

    strategy_parameter_set_id: str
    timeframe_override_parameter_set_id: str
    legacy_override_alignment_contract_id: str
    ...
    alignment_precedence: str = (
        "V2_EFFECTIVE_ALIGNMENT_SUPERSEDES_LEGACY_OVERRIDE_ENTRY_RULE_TEXT"
    )

    def __post_init__(self) -> None:
        parent = etf_parameter_snapshot()
        override = independent_timeframe_override()
        alignment = v31_alignment_contract()
        ...
        expected = {
            ...
            "legacy_override_alignment_contract_id": (
                override.entry_alignment_contract_id
            ),
            "effective_alignment_contract_id": V31_ALIGNMENT_CONTRACT_ID,
        ...
        if changed:
            raise ValueError(
                "strict V3 replay contract identity changed: " + ",".join(changed)
            )
        if (
            self.selection_path,
            ...
            raise ValueError("strict V3 replay timeframe/path mapping changed")
        frozen = (
        ...
        if not frozen:
            raise ValueError("strict V3 replay portfolio parameters changed")
        if self.result_status != "RESEARCH_ONLY" or self.live_status != LIVE_STATUS:
            raise ValueError("strict V3 replay cannot enable live trading")
...
def strict_v3_replay_contract() -> StrictV3ReplayContract:
    parent = etf_parameter_snapshot()
    override = independent_timeframe_override()
    alignment = v31_alignment_contract()
    proxy = bar_proxy_parameter_snapshot(parent)
    return StrictV3ReplayContract(
        strategy_parameter_set_id=parent.parameter_set_id,
        timeframe_override_parameter_set_id=override.parameter_set_id,
        legacy_override_alignment_contract_id=(
            override.entry_alignment_contract_id
...
@dataclass(frozen=True, slots=True)
class StrictV3DirectReplayContract:
    """Replay identities for the one-graph direct-recursive V3 path."""

    strategy_parameter_set_id: str
    timeframe_override_parameter_set_id: None
    legacy_override_alignment_contract_id: None
    effective_alignment_contract_id: str
    effective_alignment_parameter_set_id: str
    execution_parameter_set_id: str
    selection_path: str = "ETF_PROXY"
    ...
    def __post_init__(self) -> None:
        ...
        identity = (
            ...
            self.legacy_override_alignment_contract_id,
        ...
        if identity != (
            ...
            raise ValueError("strict direct V3 replay contract identity changed")
        mapping = (
            self.selection_path,
            self.l0_source_frequency,
            self.l1_source_frequency,
        ...
        if mapping != (
            ...
            raise ValueError("strict direct V3 replay mapping changed")
        if not (
            self.slot_count == parent.slot_count == 5
            and self.slot_fraction == parent.slot_fraction == Decimal("0.18")
            and self.account_exposure_cap
            ...
            raise ValueError("strict direct V3 portfolio parameters changed")
        if self.result_status != "RESEARCH_ONLY" or self.live_status != LIVE_STATUS:
            raise ValueError("strict direct V3 replay cannot enable live trading")
...
def strict_v3_direct_replay_contract() -> StrictV3DirectReplayContract:
    parent = etf_parameter_snapshot()
    alignment = direct_recursive_alignment_contract()
    proxy = bar_proxy_parameter_snapshot(parent)
    return StrictV3DirectReplayContract(
        strategy_parameter_set_id=parent.parameter_set_id,
        timeframe_override_parameter_set_id=None,
        legacy_override_alignment_contract_id=None,
        effective_alignment_contract_id=alignment.contract_id,
...
class ResearchIndividualDirectReplayContract:
    ...
    legacy_override_alignment_contract_id: None = None
...
V3ReplayContract = (
    StrictV3ReplayContract
    | StrictV3DirectReplayContract
    | ResearchIndividualDirectReplayContract
    | ResearchSectorTechnicalDirectReplayContract
    | ResearchSectorTechnicalApproxReplayContract
)
...
class ReplayFactBindings:
    """Point-in-time provenance attached to one V3 decision input."""
...
class _OrderSchedule:
    scheduled_intent: V3DecisionIntent | None
...
def _prepare_order(
    ...
    *,
    ...
    if intent.action not in _ORDER_ACTIONS:
        raise ValueError("non-order V3 intent cannot be prepared")
    ...
    if intent.persistence == "OPTIONAL" and event.expires_at is None:
        raise ValueError("optional V3 replay order requires an explicit expiry")
    if intent.persistence == "PERSISTENT_EXIT" and event.expires_at is not None:
        raise ValueError("persistent V3 replay exit cannot expire")
    ...
    identity = {
        "schema": "strict-v3-multisymbol-order/v1",
...
class StrictV3MultiSymbolReplayEngine:
    """Causal five-slot V3 replay over externally frozen decision facts.
    ...
    :class:`V3DecisionCore` exactly once per event, and delegates every fill
    ...
    def __init__(
        ...
        *,
        ...
        decision_core: V3DecisionCore | None = None,
        contract: V3ReplayContract | None = None,
    ) -> None:
        self.initial_cash = Decimal(initial_cash)
        self.started_at = normalize_datetime(started_at, "started_at")
        if self.initial_cash <= 0:
            raise ValueError("strict V3 replay initial cash must be positive")
        self.fee_model = fee_model
        self.decision_core = decision_core or V3DecisionCore()
        stock_paths = {
            "INDIVIDUAL_THREE_PROGRAM",
            RECENT_YEAR_SELECTION_PATH,
        }
        ...
        self.contract = contract or strict_v3_replay_contract()
    ...
    def _process_batch(self, batch: ReplayBatch, state: _ReplayState) -> None:
        ...
        prepared: list[
            tuple[ReplayDecisionEvent, V3DecisionIntent, tuple[str, ...], int]
        ] = []
    ...
    def _fact_gate(
        ...
        if intent.action == "ENTRY_INTENT":
            ...
            if self.contract.selection_path == "INDIVIDUAL_THREE_PROGRAM":
                required_candidate_gates = V3_INDIVIDUAL_REQUIRED_CANDIDATE_GATES
            elif self.contract.selection_path == RECENT_YEAR_SELECTION_PATH:
                required_candidate_gates = (
                    V3_SECTOR_TECHNICAL_REQUIRED_CANDIDATE_GATES
                )
            else:
                required_candidate_gates = V3_ETF_REQUIRED_CANDIDATE_GATES
            if (
                candidate is None
                ...
                reasons.append("UNRESOLVED_PARENT_V3_CANDIDATE_BINDING")
            elif (
            ...
            if (
                ...
                reasons.append("UNRESOLVED_EFFECTIVE_ALIGNMENT_V2_BINDING")
            if chain is None:
                reasons.append("UNRESOLVED_ALIGNED_30M_5M_1M_ENTRY_CHAIN")
            elif isinstance(chain, V31AlignedEntryChain):
                required = {
                    chain.l0_point_id,
                    chain.l0_center_id,
    ...
    def _schedule_order_at_decision(
        ...
        sizing = size_v3_strategic_entry(
            EntrySizingInput(
    ...
    def _apply_fills(
        ...
        *,
        ...
        if intent.action in _BUY_ACTIONS:
            ...
            state.cash += gross - match.total_fees
        if state.cash < 0:
            raise RuntimeError("strict V3 replay cash became negative")
        ...
        if intent.action == "ENTRY_INTENT":
            ...
            cycle_id = f"v3-cycle:{intent.symbol}:{first_fill.execution_id}"
    ...
    @staticmethod
    def _committed_exposure(state: _ReplayState) -> Decimal | None:
        exposure = _ZERO
        for symbol, position in state.positions.items():
            mark = state.last_marks.get(symbol)
            if mark is None or not mark.complete:
            ...
            exposure += Decimal(committed_quantity) * mark.raw_close
        return exposure
