"""Append-only control ledger for the frozen V3 forward paper observation.
...
the same shared V3 decision/replay core; this ledger only proves what was (or
...
FORWARD_PAPER_CONTRACT_SCHEMA = "chanlun-v3-forward-paper-contract/v1"
FORWARD_PAPER_APPROX_CONTRACT_SCHEMA = (
    "chanlun-v3-forward-paper-technical-approximation-contract/v2"
)
FORWARD_PAPER_HUMAN_REVIEW_CONTRACT_SCHEMA = (
    "chanlun-v3-forward-paper-human-review-contract/v3"
)
FORWARD_PAPER_LEDGER_SCHEMA = "chanlun-v3-forward-paper-ledger/v1"
FORWARD_PAPER_EVENT_SCHEMA = "chanlun-v3-forward-paper-event/v1"
FORWARD_PAPER_SESSION_DELIVERY_SCHEMA = (
    "chanlun-v3-forward-paper-session-delivery/v2"
)
...
FORWARD_IMPLEMENTATION_CONTINUITY_SCHEMA = (
    "chanlun-v3-forward-implementation-continuity/v1"
)
...
_FORWARD_SESSION_MANIFEST_SCHEMA = "chanlun-v3-forward-session-manifest/v1"
_FORWARD_ATTEMPT_RECEIPT_SCHEMA = (
    "chanlun-v3-forward-evaluation-attempt/v1"
)
_FORWARD_LIVE_OBJECT_SCHEMA = (
    "chanlun-v3-forward-live-screening-snapshot/v1"
)
_SOURCE_REVISION = re.compile(
    r"^[0-9a-f]{40,64}\.tree\.[0-9a-f]{24}$"
)
_IMPLEMENTATION_PROVENANCE_FIELDS = frozenset(
    {
        "schema",
        "application_source_revision",
        "forward_runner_script_sha256",
...
def _capture_delivery_evidence_proven(
    ...
    *,
    ...
    catalog_captured_at = _aware_local_datetime(
        readiness.get("catalog_captured_at")
    )
    recorded_at = _aware_local_datetime(event.get("recorded_at"))
    entry_sha256 = str(receipt.get("entry_sha256") or "")
    return bool(
        readiness.get("schema")
        == "chanlun-v3-forward-sector-capture-readiness/v1"
        ...
        and _SHA256.fullmatch(str(evidence.get("receipt_sha256") or ""))
        is not None
        and _SHA256.fullmatch(
            str(evidence.get("sector_ledger_sha256") or "")
        )
        is not None
...
def _data_ready_delivery_evidence_proven(
    ...
    *,
    ...
    return bool(
        ...
        and evidence.get("sector_catalog_entry_sha256")
...
def _forward_artifact_path(
    ...
    candidate = (
        ...
        else (session_root / value).resolve()
    )
    try:
...
def _selection_attestation_audit_proven(
    ...
    if (
        any(left & right for index, left in enumerate(outcomes) for right in outcomes[index + 1 :])
    ...
    if (
        ...
        or audit.get("exact_qmt_revision_name_and_membership_verified")
...
def _selection_source_audit_proven(
    ...
    if (
        ...
        or set(audit) != expected_fields
        or audit.get("schema")
        != "chanlun-human-paper-entry-selection-source-audit/v1"
        or audit.get("status")
        not in {
            "COMPLETE",
            "INCOMPLETE_SOURCE_ARCHIVE",
            "LEGACY_UNATTESTED_LIVE_RANKED_BUY",
            "INVALID",
    ...
    legacy = _selection_audit_identities(
        audit,
        "legacy_unattested_intent_ids",
        sorted_required=False,
    )
    ...
    if any(value is None for value in (verified, legacy, unavailable, invalid)):
        return False, ()
    assert verified is not None
    assert legacy is not None
    assert unavailable is not None
    assert invalid is not None
    outcomes = (set(verified), set(legacy), set(unavailable), set(invalid))
    minimum_required = len(verified) + len(legacy)
    if (
        any(left & right for index, left in enumerate(outcomes) for right in outcomes[index + 1 :])
        or verified_count != len(verified)
    ...
    expected_status = (
        ...
        if unavailable
        else "LEGACY_UNATTESTED_LIVE_RANKED_BUY"
        if legacy
        else "COMPLETE"
        if required
    ...
    if (
        audit.get("status") != expected_status
        or audit.get("immutable_source_ranking_resolved")
...
def _selection_source_report_archive_proven(
    ...
    *,
    ...
    if (
        ...
        != "chanlun-human-paper-entry-source-report-archive/v1"
        or archive.get("status")
        not in {"COMPLETE", "NO_REQUIRED_SOURCE_REPORTS"}
    ...
    try:
        for identity in archive.get("objects", ()):
            ...
            if (
                ...
                != session_root
                / "objects"
                / "paper_entry_selection_source_report"
            ...
            if (
                ...
                or sorted(value.candidate_id for value in source_alerts)
...
def _selection_paper_ledger_prefix_archive_proven(
    ...
    *,
    ...
    if (
        archive.get("schema")
        != "chanlun-human-paper-ledger-prefix-archive/v1"
        ...
        or (
            event_count > 0
            and _SHA256.fullmatch(str(last_event_id or "")) is None
        )
        or archive.get("broker_transport_available") is not False
...
def _selection_paper_ledger_prefix_archive_file(
    ...
    *,
    ...
    try:
        ...
        if (
            ledger_path.parent
            != session_root / "objects" / "human_paper_ledger_prefix"
...
def _human_paper_entry_selection_gate_proven(
    ...
    """Validate the execution-time QMT sector-admission gate when present.

    Forward artifacts created before this contract remain readable without
    the field.  Every new runner result carries it; once present, counts and
    intent identities must be internally complete and the settlement status
    must agree with the gate outcome.
    """
    ...
    if (
        ...
        or gate.get("schema")
        != "chanlun-human-paper-entry-selection-settlement-gate/v2"
        ...
        or gate.get(
            "exact_qmt_sector_admission_required_before_virtual_buy_fill"
        )
        is not True
        or gate.get(
            "immutable_source_ranking_required_before_virtual_buy_fill"
        )
        is not True
        or gate.get(
            "paper_ledger_prefix_required_for_independent_replay"
        )
        is not True
    ...
    if gate.get("status") != expected_gate_status:
        return False
    attestation_valid, audited_verified = (
        _selection_attestation_audit_proven(gate.get("attestation_audit"))
    )
    ...
    if (
        ...
        and _SHA256.fullmatch(str(catalog_hash or "")) is None
    ...
    if not source_valid:
        return False
    expected_verified = (
        set(pending) & set(audited_verified) & set(source_verified)
    )
    ...
    if (
        "ENTRY_SELECTION_EVIDENCE" in settlement_status
        and (not blocked or gate.get("status") != "BLOCKED")
    ):
        return False
...
def _qmt_instrument_status_snapshot_evidence_proven(
    ...
    *,
    ...
    """Verify an optional future-only status snapshot bound to one screen."""
    ...
    # Sessions captured before this diagnostic was introduced remain readable.
    if value is None:
        return True
    ...
    if value.get("status") == "CAPTURE_INCOMPLETE":
        return bool(
            ...
            == "QMT_INSTRUMENT_STATUS_SNAPSHOT_UNAVAILABLE"
            and isinstance(value.get("error"), str)
            and bool(str(value.get("error") or "").strip())
            and value.get("coverage_scope")
            == "SCREENING_SIGNAL_SYMBOLS_ONLY"
    ...
    try:
        ...
        validated_status = QmtInstrumentStatusSnapshot.from_document(
            status_document
        )
        expected = tuple(sorted(set(expected_symbols)))
        return bool(
            validated_status.session == session
            and value.get("captured_at")
            == validated_status.captured_at.isoformat()
            ...
            and value.get("point_in_time_scope")
...
def _evaluation_artifacts_proof(
    ...
    *,
    ...
    try:
        ...
        if receipt_path.parent != session_root / "objects" / "forward_evaluation_attempt":
            raise ValueError("forward evaluation receipt path changed")
        receipt_file_sha256 = str(
            evidence.get("attempt_receipt_sha256") or ""
        )
        if (
        ...
        if (
            ...
            or manifest.get("promoted_screening_policy_id")
            != screening_policy_id
            or manifest.get("promoted_decision_core_id") != decision_core_id
            or manifest.get("promotion_policy")
            != "FIRST_VALID_EVALUATION_ONLY"
        ...
        if (
            ...
            or live_path.name
            != f"{str(live_identity.get('file_sha256'))[7:]}.json"
            ...
            or review_path.name
            != f"{str(review_identity.get('file_sha256'))[7:]}.json"
            or _forward_artifact_path(session_root, evidence.get("result"))
            != live_path
        ...
        if (
            ...
            != event.get("strategy_parameter_set_id")
            or live.get("source_content_sha256")
        ...
        if (
            ...
            != receipt.get("source_content_sha256")
            or review_content_sha256
            != review_identity.get("content_sha256")
            ...
            or evidence.get("human_review_content_sha256")
        ...
        valuation_content_sha256 = str(
            valuation.get("valuation_content_sha256") or ""
        )
        if (
            _SHA256.fullmatch(valuation_content_sha256) is None
            or valuation_path.parent
            != session_root / "objects" / "paper_valuation"
            or valuation_path.name
            != f"{valuation_content_sha256[7:]}.json"
        ):
        ...
        if (
            ...
            or _SHA256.fullmatch(str(settlement.get("content_sha256") or ""))
            is None
            ...
            or validated_valuation.get("content_sha256")
            != valuation_content_sha256
        ...
        if selection_gate is not None:
            ...
            ledger_archive = selection_gate.get(
...
def audit_forward_paper_session_delivery(
    ...
    *,
    ...
    capture_implementation_provenance_id = (
        _event_implementation_provenance_identity(captured)
    )
    data_ready_implementation_provenance_id = (
        _event_implementation_provenance_identity(data_ready)
    )
    evaluation_implementation_provenance_id = (
        _event_implementation_provenance_identity(evaluated)
    )
    ...
    sequence_valid = bool(
        ...
        and event_indexes[id(captured)] < event_indexes[id(data_ready)]
        ...
        latest_gate_before_evaluation = next(
            (
                ...
                for event in reversed(
                    session_events[: event_indexes[id(evaluated)]]
                )
    ...
    terminal = evaluated or blocked or (
        capture_failed if not capture_event_present else None
    )
    return {
...
class ForwardPaperContract:
    ...
    tactical_ratio: str
    technical_mode: str = "STRICT_DIRECT_RECURSION"
    technical_approximation_parameter_set_id: str | None = None
    technical_alignment_parameter_set_id: str | None = None
    human_review_screening_parameter_set_id: str | None = None
    ...
    highest_status: str = "PAPER_OBSERVATION"
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            or self.highest_status
            != (
                "REVIEW_REQUIRED"
                if self.technical_mode == "HUMAN_REVIEW_SCREENING"
                else "PAPER_OBSERVATION"
            )
            or self.live_status != "LIVE_DISABLED"
        ):
            raise ValueError("forward paper safety contract changed")
        if self.technical_mode == "STRICT_DIRECT_RECURSION":
            if (
                self.technical_approximation_parameter_set_id is not None
                or self.technical_alignment_parameter_set_id is not None
                or self.human_review_screening_parameter_set_id is not None
                or self.schema != FORWARD_PAPER_CONTRACT_SCHEMA
            ):
                raise ValueError("legacy forward paper technical contract changed")
        elif self.technical_mode == "RESEARCH_APPROXIMATION":
            if (
                ...
                != FROZEN_TECHNICAL_ALIGNMENT_PARAMETER_SET_ID
                or self.human_review_screening_parameter_set_id is not None
                or self.schema != FORWARD_PAPER_APPROX_CONTRACT_SCHEMA
            ):
                raise ValueError("approximate forward paper technical contract changed")
        elif self.technical_mode == "HUMAN_REVIEW_SCREENING":
            if (
                self.technical_approximation_parameter_set_id
                != FROZEN_TECHNICAL_APPROXIMATION_PARAMETER_SET_ID
                or self.technical_alignment_parameter_set_id
                ...
                != FROZEN_HUMAN_REVIEW_SCREENING_PARAMETER_SET_ID
                or self.schema != FORWARD_PAPER_HUMAN_REVIEW_CONTRACT_SCHEMA
            ):
                raise ValueError("human review forward paper contract changed")
        else:
            raise ValueError("unsupported forward paper technical mode")
    ...
    def _stable_document(self) -> dict[str, object]:
        ...
        # Preserve the byte-level v1 contract representation for the existing
        # strict paper ledger.  Approximation starts a separate v2 ledger and
        # binds both of its technical parameter identities explicitly.
        if self.technical_mode == "STRICT_DIRECT_RECURSION":
            values.pop("technical_mode")
            values.pop("technical_approximation_parameter_set_id")
            values.pop("technical_alignment_parameter_set_id")
            values.pop("human_review_screening_parameter_set_id")
        elif self.technical_mode == "RESEARCH_APPROXIMATION":
            # Preserve the existing v2 identity after adding the v3 field.
            values.pop("human_review_screening_parameter_set_id")
...
def load_frozen_forward_contract(parameter_snapshot_path: Path) -> ForwardPaperContract:
    try:
        ...
        raise ValueError("frozen recent-year parameter snapshot cannot be read") from exc
    if not isinstance(payload, Mapping):
    ...
    technical_mode = str(
        payload.get("technical_mode", "STRICT_DIRECT_RECURSION")
    )
    if technical_mode == "HUMAN_REVIEW_SCREENING":
        human_required = {
        ...
        human_changed = tuple(
            key
            for key, expected in human_required.items()
            if payload.get(key) != expected
        )
        if human_changed:
            raise ValueError(
                "frozen human review parameter snapshot changed: "
                + ", ".join(human_changed)
            )
    return ForwardPaperContract(
        ...
        technical_approximation_parameter_set_id=(
            None
            if technical_mode == "STRICT_DIRECT_RECURSION"
            else str(payload.get("technical_approximation_parameter_set_id"))
        ),
        technical_alignment_parameter_set_id=(
            None
            if technical_mode == "STRICT_DIRECT_RECURSION"
            else str(payload.get("technical_alignment_parameter_set_id"))
        ),
        human_review_screening_parameter_set_id=(
            str(payload.get("human_review_screening_parameter_set_id"))
            if technical_mode == "HUMAN_REVIEW_SCREENING"
            else None
        ),
        highest_status=(
        ...
        schema=(
            ...
            if technical_mode == "STRICT_DIRECT_RECURSION"
            else FORWARD_PAPER_HUMAN_REVIEW_CONTRACT_SCHEMA
            if technical_mode == "HUMAN_REVIEW_SCREENING"
            else FORWARD_PAPER_APPROX_CONTRACT_SCHEMA
        ),
    )
