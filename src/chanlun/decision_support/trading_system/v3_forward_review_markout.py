...
FORWARD_REVIEW_MARKOUT_SCHEMA = "chanlun-v3-forward-review-markout/v7"
FORWARD_REVIEW_SESSION_QUALIFICATION_SCHEMA = (
    "chanlun-v3-forward-review-session-qualification/v2"
)
LEGACY_FORWARD_REVIEW_MARKOUT_SCHEMAS = frozenset(
    {
        "chanlun-v3-forward-review-markout/v1",
        "chanlun-v3-forward-review-markout/v2",
        "chanlun-v3-forward-review-markout/v3",
        "chanlun-v3-forward-review-markout/v4",
        "chanlun-v3-forward-review-markout/v5",
        "chanlun-v3-forward-review-markout/v6",
    }
)
FORWARD_REVIEW_SOURCE_AUDIT_CONTRACT_ID = (
    "QMT_FORWARD_MARKOUT_RAW_OPENING_FACTOR_ADJUSTED_BARS_V1"
)
FORWARD_REVIEW_SAMPLE_COHORT_CONTRACT_ID = (
    "SCREENING_POLICY_DECISION_CORE_AND_SOURCE_SNAPSHOT_V1"
)
_HUMAN_REVIEW_SCREEN_SCHEMA = "chanlun-v3-human-review-screen/v1"
_STRATEGIC_SAMPLE_MINIMUM = 100
_SHA256_ID = re.compile(r"^sha256:[0-9a-f]{64}$")
_LEGACY_UNATTESTED_DECISION_CORE_ID = "LEGACY_UNATTESTED_DECISION_CORE"
_LEGACY_UNATTESTED_DECISION_SOURCE_ID = (
    "LEGACY_UNATTESTED_DECISION_SOURCE_SNAPSHOT"
)
    ...
    *,
    ...
    if (
        ...
        or any(
            ...
            or _SHA256_ID.fullmatch(identity) is None
...
def qualified_forward_review_session_dates(
    ...
    *,
    ...
    if (
        ...
        != FORWARD_REVIEW_SESSION_QUALIFICATION_SCHEMA
        or qualification.get("through_session")
        ...
        or qualification.get("qualified_session_count")
        != len(raw_qualified)
    ...
    try:
        ...
        qualified = tuple(
            date.fromisoformat(str(value)) for value in raw_qualified
        )
    ...
    if observed_at.tzinfo is None or observed_at.date() != through_session:
        raise ValueError(
            "forward review session qualification observation is invalid"
        )
    if (
    ...
    if (
        tuple(excluded_sessions) != tuple(sorted(set(excluded_sessions)))
        or set(excluded_sessions).intersection(qualified)
    ):
...
def review_price_bars_revision(
    *,
    ...
    return sha256_json(
        {
            "schema": "chanlun-forward-markout-adjusted-1m-bars/v1",
            "symbol": symbol,
    ...
    *,
    ...
    try:
        ...
        return (
            ...
            == QMT_CAUSAL_FACTOR_ADJUSTMENT_CONTRACT_ID
            and audit.get("factor_known_through")
            == through_session.isoformat()
            ...
            and _SHA256_ID.fullmatch(str(audit.get("raw_bar_revision")))
            is not None
...
class ForwardReviewSample:
    ...
    source_decision_source_snapshot: Mapping[str, object] | None = None
...
def _report_screening_policy_id(report: Mapping[str, object]) -> str:
    ...
    if value is None:
        return LEGACY_UNATTESTED_SCREENING_POLICY_ID
    if value == LEGACY_UNATTESTED_SCREENING_POLICY_ID:
        return value
...
def _report_decision_core_id(report: Mapping[str, object]) -> str:
    input_hashes = report.get("input_hashes")
    if not isinstance(input_hashes, Mapping):
        return _LEGACY_UNATTESTED_DECISION_CORE_ID
...
def _report_decision_source_snapshot(
    ...
    if snapshot is None:
        if declared is not None:
            raise ValueError(
                "forward human-review decision source snapshot is missing"
            )
        return _LEGACY_UNATTESTED_DECISION_SOURCE_ID, None
    identity = decision_source_snapshot_id(snapshot)
    if declared != identity:
        raise ValueError(
            "forward human-review decision source identity changed"
        )
    if not isinstance(snapshot, Mapping):
        raise ValueError("forward human-review decision source snapshot changed")
...
def select_first_strategic_buy_samples(
    ...
    *,
    ...
    for source_session, source_sha256, report in sorted(
        ...
        decision_source_id, decision_source_snapshot = (
            _report_decision_source_snapshot(report)
        )
        for raw in queue:
            if not isinstance(raw, Mapping):
...
def build_forward_review_markout(
    ...
    *,
    ...
    calendar_document = {
        "contract": "EXPLICIT_SORTED_MARKET_TRADING_SESSIONS_V1",
        "through_session": through_session.isoformat(),
    ...
    for sample in samples:
        ...
        source_cohort_attestation[source_cohort_id] = (
            source_cohort_attestation.get(source_cohort_id, True)
        ...
        if sample.source_decision_source_snapshot is not None:
            ...
            if snapshot_id == sample.source_decision_source_snapshot_id:
                ...
                if existing_snapshot != snapshot_document:
                    raise ValueError(
                        "forward review decision source snapshot collided"
                    )
    ...
    for policy_id in policy_ids:
        ...
        policy_cohorts[policy_id] = {
            ...
            "mixed_decision_source_cohorts": (
                len(policy_source_cohort_ids) > 1
            ),
    ...
    for cohort_id in source_cohort_ids:
        policy_id, decision_core_id, source_snapshot_id = (
            source_cohort_metadata[cohort_id]
        )
        cohort_summary = _jsonable(
            summarize_event_study(by_source_cohort[cohort_id])
        )
        cohort_eligible = {
            horizon: int(values["eligible_count"])
            for horizon, values in cohort_summary.items()
    ...
    stable: dict[str, object] = {
        ...
        "source_session_qualification": _jsonable(
            dict(source_session_qualification)
        ),
        ...
        "sample": {
            ...
            "source_identity_status": (
                ...
                if not samples
                    ...
                    "ATTESTED"
                    if all_source_cohorts_attested
        ...
        "reason_codes": [
            ...
            *(
                ("DECISION_SOURCE_COHORT_UNATTESTED",)
                if samples and not all_source_cohorts_attested
                else ()
            ),
            *(
                ()
...
def _markout_summary_counts(
    ...
    for horizon, (eligible, pending) in expected.items():
        ...
        if (
            row.get("eligible_count") != eligible
            or row.get("pending_count") != pending
        ):
...
def _markout_observation(
    ...
    *,
    ...
    expected_end_session = (
        ...
        else future_sessions[horizon - 1]
    )
    ...
    if (
        (invalidation is True and first_invalidation is None)
        or (invalidation is not True and first_invalidation is not None)
    ):
    ...
    if (
        ...
        and expected_end_session is not None
    ):
...
def validate_forward_review_markout_document(
    ...
    """Replay the v7 report's safety and sample-accounting invariants.
    ...
    if (
        calendar.get("contract")
        != "EXPLICIT_SORTED_MARKET_TRADING_SESSIONS_V1"
    ...
    observation_objects: list[ReviewEventStudyObservation] = []
    risk_observation_objects: dict[
        str, list[ReviewEventStudyObservation]
    ] = {"ALL_GREEN": [], "BLOCKED": []}
    cohort_observation_objects: dict[
        str, list[ReviewEventStudyObservation]
    ] = {}
    policy_observation_objects: dict[
        str, list[ReviewEventStudyObservation]
    ] = {}
    for raw in observations:
        ...
        if any(
            value not in {"GREEN", "AMBER", "RED", "UNRESOLVED"}
            for value in gates
        ):
    ...
    expected_all = {
        key: (values[0], values[1]) for key, values in all_counts.items()
    }
    if not observations:
        expected_all = {}
    _markout_summary_counts(payload.get("summary"), expected_all, "summary")
    if payload.get("summary") != _jsonable(
        summarize_event_study(observation_objects)
    ):
    ...
    implementations = {
        ...
        for metadata in observation_cohort_metadata.values()
    }
    ...
    if (
        ...
        or sample.get("feedback_linked_lifecycle_count")
        != len(feedback_lifecycles)
        ...
        or sample.get("mixed_decision_source_cohorts")
        is not (len(implementations) > 1)
        or sample.get("minimum_strategic_observations")
        != _STRATEGIC_SAMPLE_MINIMUM
    ):
        raise ValueError("forward markout global sample status changed")
    expected_identity_status = (
        "NOT_APPLICABLE"
        if not observations
        else "ATTESTED" if all_attested else "LEGACY_UNATTESTED"
    )
    ...
    conditional_reasons = {
        ...
        "DECISION_SOURCE_COHORT_UNATTESTED": (
            bool(observations) and not all_attested
        ),
        ...
        "STRATEGIC_MARKOUT_SAMPLE_INSUFFICIENT": not (
            ...
            and all(
                count >= _STRATEGIC_SAMPLE_MINIMUM
                for count in global_eligible.values()
            )
        ),
    }
