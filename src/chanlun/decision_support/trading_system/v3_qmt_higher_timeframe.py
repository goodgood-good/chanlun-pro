"""QMT adapters for V3 M/W/D risk with the explicit D -> 30m mapping.
...
QMT_HIGHER_TIMEFRAME_WARMUP_CONVERGENCE_PARAMETER_SET_ID = sha256_json(
    {
        "contract": "chanlun-qmt-mwd-multi-prefix-convergence/v1",
        "frequency": "d",
        ...
        "semantic_signature": "chanlun-qmt-mwd-warmup-semantic-tail/v1",
...
def qmt_higher_timeframe_inputs(
    *,
    ...
    source_revision = sha256_json(
        {
            "schema": "chanlun-v3-qmt-higher-timeframe-input/v2",
            "symbol": symbol,
            "decision_time": decision,
            "daily": tuple(
                {
...
def _mwd_warmup_semantic_snapshot(
    ...
    """Retain the exact facts already hashed by the frozen v1 signature."""
        ...
        if bar_count == full_count:
            ...
            risk = build_higher_timeframe_risk_facts(
                ...
                snapshot_id=sha256_json(
                    {
                        "schema": "chanlun-qmt-mwd-warmup-prefix/v1",
    ...
    *,
    ...
    if full_count < required:
        ...
        suffix_risk = build_higher_timeframe_risk_facts(
            ...
            snapshot_id=sha256_json(
                {
                    "schema": "chanlun-qmt-mwd-warmup-suffix/v1",
                    "full_snapshot_id": snapshot_id,
