...
STRUCTURE_SIGNAL_LEDGER_SCHEMA = (
    "chanlun-frozen-structure-signal-ledger"
)
...
def build_structure_signal_ledger(
    *,
    symbol: str,
    ...
    diagnostics: list[SignalAdapterDiagnostic] = [
        ...
        SignalAdapterDiagnostic(
            ...
            "a generic sell point cannot be promoted to the strict strategy full-position reduce rule",
        ),
    ]
    rule_coverage = {name: "COMPLETE" for name in REQUIRED_STRUCTURE_RULES}
    ...
    def append_row(
        ...
        *,
        ...
        event_id = sha256_json(
            {
                "schema": "chanlun-frozen-structure-signal-event",
                "symbol": symbol,
                "signal_kind": signal_kind,
                "point_id": point.point_id,
                "decision_time": point.available_at,
