...
COVERAGE_MANIFEST_SCHEMA = "chanlun-screening-coverage-manifest/v2"
COVERAGE_STATE_CONTRACT_ID = "chanlun-screening-coverage-state/v3"
SECTOR_COVERAGE_CONTRACT_ID = "chanlun-screening-sector-coverage/v2"
MONITOR_INSTRUMENT_EXCLUSION_CONTRACT_ID = (
    "chanlun-monitor-instrument-exclusion/v2"
)
COVERAGE_EXCLUSION_REASON_CODES = frozenset(
...
def _native_daily_reconciliation_evidence_is_consistent(
    ...
    *,
    ...
    if (
        overlap_count > min(native_count, minute_count)
    ...
    if (
        raw.get("price_difference_session_count")
        != len(difference_sessions)
...
def _native_daily_calendar_coverage_evidence_is_consistent(
    ...
    *,
    ...
    calendar_reason = (
        "QMT_NATIVE_DAILY_TRADING_CALENDAR_MISMATCH" in side_reasons
    )
...
def screening_coverage_epoch_id(
    *,
    ...
    adapters.  A forward-eligible v6 snapshot separately requires a real
    ...
    if sector_strength_evidence_revision is not None and not _is_sha256_identity(
        ...
        raise ValueError(
            "sector_strength_evidence_revision must be a sha256 identity"
        )
    for field_name, value in (
    ...
    return sha256_json(
        {
            "schema": "chanlun-screening-coverage-epoch/v3",
...
def _source_screening_policy_id(payload: Mapping[str, object]) -> str:
    """Return an attested policy identity, or label old archives explicitly."""
    ...
    if (
        ...
        return declared
    return LEGACY_UNATTESTED_SCREENING_POLICY_ID
    ...
    try:
        discovered = set(
            _canonical_code_list(
                manifest.get("discovered_codes"), "discovered codes"
            )
        )
        completed = set(
            _canonical_code_list(
                manifest.get("completed_codes"), "completed codes"
            )
        )
        failed = set(
        ...
        excluded = set(
            _canonical_code_list(
                manifest.get("excluded_codes"), "excluded codes"
            )
        )
        exclusion_reasons = _coverage_exclusion_documents(
        ...
        expected_epoch_id = screening_coverage_epoch_id(
            ...
            sector_strength_evidence_revision=(
                ...
                if isinstance(
                    manifest.get("sector_strength_evidence_revision"), str
                )
                else None
            ),
            decision_core_id=str(payload.get("decision_core_id")),
            screening_policy_id=_source_screening_policy_id(payload),
            structure_version=str(payload.get("structure_version")),
            parameter_version=str(payload.get("parameter_version")),
        )
        if (
            manifest.get("schema") != COVERAGE_MANIFEST_SCHEMA
            or manifest.get("coverage_state_contract_id")
            != COVERAGE_STATE_CONTRACT_ID
            or manifest.get("signal_document_contract_id")
            != SIGNAL_DOCUMENT_CONTRACT_ID
            or manifest.get("coverage_epoch_id")
            != payload.get("coverage_epoch_id")
            or manifest.get("coverage_epoch_id") != expected_epoch_id
            or manifest.get("screening_policy_id")
            != payload.get("screening_policy_id")
            or manifest.get("market_data_as_of")
            != payload.get("market_data_as_of")
            ...
            or int(audit.get("coverage_cycle_attempted_symbol_count"))
            != len(attempted)
            or int(audit.get("coverage_cycle_completed_symbol_count"))
            != len(completed)
            or int(audit.get("coverage_cycle_excluded_symbol_count"))
            != len(excluded)
            or int(audit.get("coverage_cycle_failed_symbol_count"))
            != len(failed)
            or int(audit.get("coverage_cycle_resolved_symbol_count"))
            != len(resolved)
            or int(audit.get("immediate_pending_symbol_count")) != len(pending)
            or int(audit.get("backoff_retry_symbol_count")) != len(backoff)
            or int(audit.get("pending_symbol_count"))
            != len(pending) + len(backoff)
            or int(audit.get("next_epoch_retry_symbol_count")) != len(deferred)
            or int(audit.get("retry_symbol_count"))
            != len(backoff) + len(deferred)
        ):
            return False
        expected_ratio = (
            Decimal("0")
        ...
        if not isinstance(raw_signals, list) or any(
            ...
            or not isinstance(signal.get("code"), str)
...
def validate_live_screening_market_watermark(
    ...
    monitor_exclusions_consistent = (
        monitor_instrument_exclusions_are_consistent(payload)
    )
    if (
        payload.get("schema_version") != LIVE_SCREENING_SCHEMA
        or payload.get("signal_document_contract_id")
        != SIGNAL_DOCUMENT_CONTRACT_ID
        or payload.get("sector_coverage_contract_id")
...
def _decision_context_is_consistent(raw: object) -> bool:
    ...
    if dominant_type is None:
        ...
        if (
            dominant_type not in _POINT_TYPES
            or not _is_sha256_identity(dominant_id)
        ):
            return False
...
def _warmup_evidence_is_consistent(raw: object) -> bool:
    ...
    # Legacy/manual review fixtures may not carry the diagnostic-only rows.
    # Their absence never relaxes the active pairwise gate; current producers
    # emit the rows and they are strictly checked when present.
    ...
    if (
        ...
        return False
    expected_frequencies = SCREENING_WARMUP_FREQUENCIES[: len(rows)]
    ...
    if difference_rows:
        ...
        for warmup_row, diagnostic_row in zip(rows, difference_rows):
            ...
            if (
                diagnostic_row.get("frequency")
                != warmup_row.get("frequency")
...
def _session_evidence_is_consistent(
    ...
    *,
    ...
    trade-status source proves suspension.  The current v1 contract therefore
    ...
    expected_disposition = (
        "FAIL_CLOSED"
        if status == "UNAVAILABLE" or issues
    ...
    if status == "EXACT" and issue_codes != reason_issue_codes:
        return False
    if issues and side_gate != "UNRESOLVED":
        return False
    return True
...
def _mwd_warmup_evidence_is_consistent(
    ...
    *,
    ...
    blocking_reasons = _QMT_MWD_WARMUP_BLOCKING_CODES.intersection(
        side_reasons
    )
    if raw is None:
    ...
    if reason == "QMT_HIGHER_TIMEFRAME_WARMUP_HISTORY_INSUFFICIENT":
        ...
        consistent = (
            full_count >= required
            and suffix_count
            == expected_screening_warmup_suffix_bar_count(full_count)
            and _is_sha256_identity(suffix_signature)
            and converged
            == (reason == "QMT_HIGHER_TIMEFRAME_WARMUP_TAIL_STABLE")
    ...
    expected_disposition = (
        "FAIL_CLOSED" if blocked else "NO_WARMUP_BLOCKER"
    )
    if raw.get("entry_disposition") != expected_disposition:
        return False
    if blocked:
...
def _mwd_warmup_semantic_diagnostic_is_consistent(
    raw: object,
    *,
    envelope_raw: object,
    evidence_cutoff: datetime,
) -> bool:
    """Authenticate the human-readable facts against their v1 envelope."""

    if envelope_raw is None:
        return raw is None
    # The sibling extension is additive: historical/fake providers may
    # explicitly serialize ``None`` while still carrying the v1 envelope.
    # When facts are present they are strict and envelope-bound; their absence
    # remains visible to the effectiveness audit as NOT_RECORDED_LEGACY.
    if raw is None:
        return True
    if not isinstance(envelope_raw, Mapping) or not isinstance(raw, Mapping):
        return False
    try:
    ...
    return bool(
        ...
        and diagnostic.frequency == "d"
        and diagnostic.parameter_set_id
        == QMT_HIGHER_TIMEFRAME_WARMUP_CONVERGENCE_PARAMETER_SET_ID
        and diagnostic.diagnostic_only is True
        and diagnostic.active_gate_unchanged is True
        and diagnostic.live_status == "LIVE_DISABLED"
    )


def _mwd_warmup_mapping_supply_diagnostic_is_consistent(
    raw: object,
    *,
    semantic_raw: object,
    envelope_raw: object,
    ...
    """Authenticate point-level prefix deltas against both older siblings."""

    if envelope_raw is None:
        return raw is None and semantic_raw is None
    if raw is None:
        return True
    if not all(
        isinstance(value, Mapping)
        for value in (raw, semantic_raw, envelope_raw)
    ):
        return False
    try:
        envelope = WarmupConvergenceEnvelope.from_document(envelope_raw)
        ...
        supply = WarmupMappingSupplyDiagnosticEnvelope.from_document(raw)
        supply.validate_against(replace(envelope, diagnostic=semantic))
    except (TypeError, ValueError):
        return False
    return bool(
        supply.as_of == evidence_cutoff
        and supply.frequency == "d"
        and supply.parameter_set_id
        == QMT_HIGHER_TIMEFRAME_WARMUP_CONVERGENCE_PARAMETER_SET_ID
        and supply.diagnostic_only is True
        and supply.active_gate_unchanged is True
        and supply.live_status == "LIVE_DISABLED"
    )


def _mwd_warmup_structure_lineage_diagnostic_is_consistent(
    raw: object,
    *,
    supply_raw: object,
    semantic_raw: object,
    ...
    """Authenticate line/center/trigger lineage against all prior siblings."""

    if envelope_raw is None:
        return raw is None and semantic_raw is None and supply_raw is None
    if raw is None:
        return True
    if not all(
        isinstance(value, Mapping)
        for value in (raw, supply_raw, semantic_raw, envelope_raw)
    ):
        return False
    try:
        envelope = WarmupConvergenceEnvelope.from_document(envelope_raw)
        ...
        lineage = WarmupStructureLineageDiagnosticEnvelope.from_document(raw)
        lineage.validate_against(
            replace(bound, mapping_supply_diagnostic=supply)
        )
    except (TypeError, ValueError):
        return False
    return bool(
        lineage.as_of == evidence_cutoff
        and lineage.frequency == "d"
        and lineage.parameter_set_id
        == QMT_HIGHER_TIMEFRAME_WARMUP_CONVERGENCE_PARAMETER_SET_ID
        and lineage.diagnostic_only is True
        and lineage.active_gate_unchanged is True
        and lineage.live_status == "LIVE_DISABLED"
    )
...
def _mwd_warmup_diagnostic_chain_is_consistent(
    *,
    ...
    elif isinstance(envelope_raw, Mapping):
        try:
            envelope = WarmupConvergenceEnvelope.from_document(envelope_raw)
            if semantic_raw is None:
                result = supply_raw is None and lineage_raw is None
            elif isinstance(semantic_raw, Mapping):
                semantic = WarmupConvergenceDiagnosticEnvelope.from_document(
                    semantic_raw
                )
                semantic.validate_against(envelope)
                if not current(semantic):
                    result = False
                elif supply_raw is None:
                    result = lineage_raw is None
                elif isinstance(supply_raw, Mapping):
                    bound = replace(envelope, diagnostic=semantic)
                    supply = WarmupMappingSupplyDiagnosticEnvelope.from_document(
                        supply_raw
                    )
                    supply.validate_against(bound)
                    if not current(supply):
                        result = False
                    elif lineage_raw is None:
                        result = True
                    elif isinstance(lineage_raw, Mapping):
                        lineage = (
...
def _sector_source_extension_mode(
    ...
    *,
    ...
    """Authenticate the optional page/replay sector-source provenance.

    Older decision documents may omit the extension, but a row claiming the
    native-daily research blocker must carry it.  The precheck is deliberately
    ...
    *,
    ...
    if unresolved_snapshot:
        ...
        if any(
            ...
            return False
        if len(diagnostics) != 3 or tuple(
            value.state for value in diagnostics
        ) != states:
            return False
...
def _risk_evidence_is_consistent(
    ...
    *,
    ...
    if (
        ...
        or merged_reasons
        != tuple(
            dict.fromkeys((*market_reasons, *sector_reasons, *symbol_reasons))
        )
    ):
        return False
    ...
    if not sector_extension_valid:
        return False
    side_evidence_valid = _risk_side_evidence_is_consistent(
        raw,
        prefix="market",
        evidence_cutoff=evidence_cutoff,
    ...
    present = tuple(field in raw for field in extension_fields)
    # Rows completed before the diagnostic extension remain valid v6 decision
    # documents only when they do not claim a one-minute session problem.  A
    # generic missing/grid reason without its exact session evidence is enough
    # for the page to fail closed, but is not enough for final review: accepting
    # it would let a legacy row hide the affected dates forever.  Once any
    # extension field is present, all three are mandatory and independently
    # reconstructed.
    if not any(present):
        session_valid = not _SESSION_ISSUE_CODES.intersection(
            (*market_reasons, *sector_reasons, *symbol_reasons)
        )
    else:
        if not all(present) or raw.get("session_evidence_contract_id") != (
            HIGHER_TIMEFRAME_SESSION_EVIDENCE_CONTRACT_ID
        ):
            return False
        session_valid = _session_evidence_is_consistent(
    ...
    warmup_present = tuple(field in raw for field in warmup_fields)
    if not any(warmup_present):
        warmup_valid = not _QMT_MWD_WARMUP_BLOCKING_CODES.intersection(
            (*market_reasons, *sector_reasons, *symbol_reasons)
        )
    elif not all(warmup_present) or raw.get(
        "warmup_evidence_contract_id"
    ) != QMT_HIGHER_TIMEFRAME_WARMUP_EVIDENCE_CONTRACT_ID:
        return False
    else:
        warmup_valid = _mwd_warmup_evidence_is_consistent(
    ...
    convergence_present = tuple(field in raw for field in convergence_fields)
    strict_convergence_field = (
        "sector_strict_same_5m_warmup_convergence_evidence"
    )
    if not any(convergence_present):
        ...
        return False
    elif sector_source_mode is None:
        if strict_convergence_field in raw:
            return False
    else:
    ...
    diagnostic_present = tuple(field in raw for field in diagnostic_fields)
    strict_diagnostic_field = (
        "sector_strict_same_5m_warmup_convergence_diagnostic_evidence"
    )
    if not any(diagnostic_present):
    ...
    native_present = tuple(field in raw for field in native_fields)
    if any(native_present) and (
        not all(native_present)
    ...
    native_valid = not any(native_present) or (
        native_side_is_consistent("market", None)
    ...
    calendar_present = tuple(field in raw for field in calendar_fields)
    if not any(calendar_present):
        # A legacy success can remain readable, but an unresolved native-daily
        # calendar reason must be replayed until its exact gap dates are bound.
        return "QMT_NATIVE_DAILY_TRADING_CALENDAR_MISMATCH" not in {
            *market_reasons,
            *symbol_reasons,
        }
    if (
        not all(calendar_present)
    ...
    def calendar_side_is_consistent(
        ...
        evidence = raw.get(
            f"{prefix}_native_daily_calendar_coverage_evidence"
        )
        if evidence is None:
        ...
        if not _native_daily_calendar_coverage_evidence_is_consistent(
            ...
            return False
        try:
            coverage = QmtNativeDailyCalendarCoverageEvidence.from_document(
                evidence
            )
        except (TypeError, ValueError):
            return False
        reconciliation = raw.get(
            f"{prefix}_native_daily_reconciliation_evidence"
        )
        return (coverage.status == "EXACT") == (reconciliation is not None)

    return calendar_side_is_consistent(
        "market", None, market_reasons
    ) and calendar_side_is_consistent(
        "symbol", expected_symbol, symbol_reasons
    )


def live_review_risk_evidence_replay_codes(
    payload: Mapping[str, object],
) -> tuple[str, ...]:
    """Return symbols whose retained risk evidence must be rebuilt.

    This is a migration/preflight helper, not a relaxed review validator.  It
    deliberately reuses the exact risk-evidence predicate consumed by
    :func:`validate_live_review_snapshot`, including the signal-local causal
    cutoff.  A partially completed coverage epoch can therefore place only the
    affected symbols back into its read-only scan queue instead of discovering
    an opaque ``REVIEW_BOUNDARY_INVALID`` after every other symbol has finished.

    Rows without a usable symbol cannot be repaired by a targeted replay and
    remain the responsibility of the full validator.  An invalid page-wide
    market cutoff makes every named signal replayable, which is the safe
    fail-closed result for this helper.
    """
    ...
    named_signals = tuple(
        ...
        and bool(raw.get("code"))
    )
    try:
        market_data_as_of = datetime.fromisoformat(
            str(payload["market_data_as_of"])
        )
        if market_data_as_of.tzinfo is None:
            raise ValueError("market cutoff must be timezone-aware")
    except (KeyError, TypeError, ValueError):
        return tuple(sorted({str(raw["code"]) for raw in named_signals}))

    replay_codes: set[str] = set()
    diagnostic_memo: dict[tuple[str, str], bool] = {}
    for raw in named_signals:
        code = str(raw["code"])
        try:
            observed_at = datetime.fromisoformat(str(raw["observed_at"]))
        except (KeyError, TypeError, ValueError):
            replay_codes.add(code)
            continue
        evidence_cutoff = (
            observed_at
            if observed_at.tzinfo is not None
            and observed_at <= market_data_as_of
            else market_data_as_of
        )
        risk = raw.get("higher_timeframe_risk")
        # ``ExchangeQMT`` used to pass the inclusive decision timestamp to
        # QMT's *exclusive* history-download boundary.  A post-close 1m prefix
        # consequently omitted the exact 15:00 completion while native D had
        # already published that session.  Those old fail-closed documents are
        # internally authentic, so the generic consistency predicate below
        # cannot distinguish them from an observation made after the transport
        # repair.  Put only the affected displayed symbols back into the frozen
        # epoch queue; a successful replay removes this reason and therefore
        # makes the migration naturally idempotent.
        transport_repair_reason = "QMT_NATIVE_DAILY_AHEAD_OF_ONE_MINUTE_BASE"
        reason_fields = (
            "reason_codes",
            "market_reason_codes",
            "sector_reason_codes",
        ...
        requires_transport_replay = isinstance(risk, Mapping) and any(
            transport_repair_reason in tuple(risk.get(field) or ())
            for field in reason_fields
        )
        if requires_transport_replay or not _risk_evidence_is_consistent(
            risk,
            evidence_cutoff=evidence_cutoff,
            ...
            replay_codes.add(code)
    return tuple(sorted(replay_codes))
...
def _buy_decision_evidence_is_consistent(
    ...
    *,
    ...
    if policy.get("require_sector_eligibility") is True and sector.get(
        "hard_block"
    ) is True:
        entry_reasons.append("sector_hostile")
    if policy.get("require_thirty_minute_context") is True and context.get(
        "hard_block"
    ) is True:
...
def _sector_exclusion_documents(
    ...
    for document in raw:
        ...
        if (
            ...
            or document.get("eligibility")
            != "MINIMUM_SECTOR_MEMBERS_NOT_MET"
            or document.get("reason_code")
...
def _validated_sector_documents(
    ...
    *,
    ...
    for raw in raw_sectors:
        ...
        if failure is not None or exclusion is not None:
            ...
            expected_components = {
                ...
                "five_support": (
                    30 if five.get("disposition") == "supportive" else 0
                ),
                "one_support": (
                    10 if one.get("disposition") == "supportive" else 0
                ),
        ...
        try:
            strength = (
                None if raw_strength is None else Decimal(str(raw_strength))
            )
            anchor = (
                None
    ...
    expected_ranked_order = [
        sector_id
        for _missing, _strength, _score, sector_id in sorted(sortable)
    ]
    ...
    expected_completed = (
        len(sectors) - len(sector_failures) - len(sector_exclusions)
    )
    ...
    if (
        ...
        or audit.get("sector_failure_counts")
        != dict(sorted(failure_counts.items()))
...
def validate_live_review_snapshot(
    ...
    *,
    ...
    if isinstance(decision_core, Mapping):
        try:
            verified_decision_core_id = (
                validate_human_assisted_contract_document(decision_core)
            )
        except (TypeError, ValueError):
    ...
    try:
        sector_coverage = Decimal(
            str(audit.get("sector_completion_ratio"))
        ) if isinstance(audit, Mapping) else Decimal("NaN")
        stock_coverage = Decimal(
            str(audit.get("coverage_cycle_completion_ratio"))
        ) if isinstance(audit, Mapping) else Decimal("NaN")
    except (InvalidOperation, TypeError, ValueError):
    ...
    for raw in raw_signals:
        ...
        market_reasons = (
            risk.get("market_reason_codes")
            if isinstance(risk, Mapping)
            else None
        )
        symbol_reasons = (
            risk.get("symbol_reason_codes")
            if isinstance(risk, Mapping)
            else None
        )
        sector_reasons = (
            risk.get("sector_reason_codes")
            if isinstance(risk, Mapping)
            else None
        )
        merged_reasons = (
            risk.get("reason_codes") if isinstance(risk, Mapping) else None
        )
        market_diagnostics = (
            risk.get("market_period_diagnostics")
            if isinstance(risk, Mapping)
            else None
        )
        symbol_diagnostics = (
            risk.get("symbol_period_diagnostics")
            if isinstance(risk, Mapping)
            else None
        )
        sector_diagnostics = (
            risk.get("sector_period_diagnostics")
            if isinstance(risk, Mapping)
            else None
        )
        ...
        # every consumer of this validator on the same semantic boundary.
            for signal in signals:
                ...
                live_signal_human_review_alert(
                    ...
                    sector_ranking_document=sector_documents.get(
                        str(sector_id)
                    ),
                    sector_ranking_observed_at=datetime.fromisoformat(
                        str(
                            strength_evidence.evidence_document()[
                                "decision_time"
                            ]
                        )
                    ),
                    ...
                    sector_catalog_revision=str(
                        manifest["sector_catalog_revision"]
                    ),
                )
    except (ArithmeticError, TypeError, ValueError) as exc:
        raise ValueError(
            "live screening signal review conversion is invalid"
        ) from exc
    ...
    for signal in signals:
        ...
        if (
            ...
            raise ValueError(
                "live screening signal decision evidence is invalid"
            )
    ...
    for signal in signals:
        ...
        expected_point_counts[point_type] = (
            expected_point_counts.get(point_type, 0) + 1
        )
...
def _alert_type(signal: Mapping[str, object]) -> str:
    ...
    # opportunity.  Do not manufacture that role from the legacy recursive
    # tower comparison: every physical point is ``formal / level 0``.  Preserve
...
def live_signal_human_review_alert(
    ...
    *,
    ...
    if not all(
        isinstance(value, Mapping)
        for value in (risk, warmup, sector, setup, context)
    ) or (trigger is not None and not isinstance(trigger, Mapping)):
    ...
    if (
        sector_ranking_document is not None
        and sector_ranking_observed_at is None
    ):
    ...
    warnings = tuple(
        dict.fromkeys(
            (
                ...
                *(
                    (
                        ...
                        (
                            ...
                            if entry_boundary.entry_valid_until
                            <= review_available_at
                ...
                *(
                    ()
                    if warmup.get("converged") is True
    ...
    structure_snapshot_id = sha256_json(
        {
            "schema": "chanlun-live-human-review-structure/v2",
        ...
        sorted(
            (
                live_signal_human_review_alert(
                    ...
                    sector_catalog_revision=str(
                        live_snapshot["coverage_manifest"][
    ...
    stable: dict[str, object] = {
        ...
        **(
            ...
            if decision_source_snapshot is None
                ...
                "decision_source_snapshot": _jsonable(
        ...
        "input_hashes": {
            ...
            **(
                ...
                if source_implementation_id is None
                    ...
                    "decision_source_snapshot_id": source_implementation_id
                }
            ),
        },
        "candidate_funnel": {
        ...
        "signal_counts": {
            ...
            "by_point_type": dict(
                live_snapshot.get("counts_by_point_type") or {}
            ),
            "by_alert_type": {
