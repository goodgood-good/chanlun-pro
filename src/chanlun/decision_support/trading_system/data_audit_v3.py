...
@dataclass(frozen=True, slots=True)
class V3BarProxyDataGateResult:
    eligibility: DataEligibility
...
def audit_v3_data_contract(evidence: V3DataContractEvidence) -> V3DataGateResult:
...
def audit_v3_bar_proxy_data_contract(
    evidence: V3DataContractEvidence,
) -> V3BarProxyDataGateResult:
    ...
    if not evidence.missing_data_retained_as_rejection:
        warnings.append("missing_data_deletion_bias_not_excluded")
    return V3BarProxyDataGateResult(
        eligibility=eligibility,
