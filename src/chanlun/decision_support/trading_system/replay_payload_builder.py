...
PRESCREEN_SCHEMA = "chanlun-cached-symbol-prescreen"
FACT_LEDGER_SCHEMA = "chanlun-frozen-decision-fact-ledger"
CORPORATE_ACTION_SCHEMA = "chanlun-qmt-etf-corporate-actions"
REPLAY_INPUT_SCHEMA = "chanlun-strict-multisymbol-replay-input"
PAYLOAD_BUILD_SCHEMA = "chanlun-strict-replay-payload-build"
_CN_OPEN = time(9, 30)
...
def _chain(value: Mapping[str, object]) -> AlignedEntryChain:
    return AlignedEntryChain(
...
def _latest_bar_at(
    ...
    eligible = tuple(
        value for value in bars if value.complete and value.closed_at <= observed_at
    )
    return max(
        eligible,
...
def _candidate_trace_reasons(
    ...
    *,
    ...
    passed = {str(item.get("gate")) for item in checks if item.get("passed") is True}
    ...
    if candidate.get("higher_timeframe_risk_buyable") is not True:
        reasons.append("UNRESOLVED_HIGHER_TIMEFRAME_RISK_BUYABILITY")
    if len(gates) != len(set(gates)) or not (
        ETF_REQUIRED_CANDIDATE_GATES.issubset(passed)
    ):
...
def build_replay_payload(
    *,
    ...
    discovered: list[tuple[str, AlignedEntryChain, Mapping[str, object]]] = []
    for artifact in prescreen_artifacts:
        ...
        if not _valid_sha256(artifact.get("content_sha256")) or artifact.get(
            "content_sha256"
        ...
        alignment_bound = (
            artifact.get("alignment_contract") == alignment_contract().document()
        ...
        for report_value in _sequence(
            ...
            if count and not alignment_bound:
                diagnostics.append(
                    PayloadDiagnostic(
                        "UNRESOLVED",
                        "UNRESOLVED_PRESCREEN_ALIGNMENT_IDENTITY",
                        "legal chains cannot be promoted without the effective alignment identity",
                        symbol,
                    )
                )
                continue
    ...
    if fact_ledger is None:
        ...
        fact_hash_ok = _valid_sha256(
            fact_ledger.get("content_sha256")
        ) and fact_ledger.get("content_sha256") == _stable_content_hash(
            fact_ledger,
        ...
        if not fact_identities_ok:
            diagnostics.append(
                PayloadDiagnostic(
                    ...
                    "fact ledger is not bound to parent ETF strict strategy + override + effective alignment",
                )
            )
    ...
    for symbol, aligned, report in sorted(
        ...
        if candidate_reasons:
            diagnostics.append(
                PayloadDiagnostic(
                    "UNRESOLVED",
                    candidate_reasons[0],
                    "candidate is not bound to the accepted parent strict strategy ETF trace: "
        ...
        rule_coverage_ok = isinstance(rule_coverage_value, Mapping) and all(
            rule_coverage_value.get(rule) == "COMPLETE"
            for rule in REQUIRED_STRUCTURE_RULES
        )
        coverage_ok = (
            ...
            and datetime.fromisoformat(str(coverage.get("end_at"))).date() >= source_end
        ...
        if not coverage_ok:
            diagnostics.append(
                PayloadDiagnostic(
                    ...
                    "complete 30m/5m/1m level-zero coverage, every strict strategy strategic/tactical rule, and an exact source hash are required through source end",
                    symbol,
                    aligned.l0_point_id,
                )
            )
    ...
    for row in sorted(
        ...
        if row.get("emit_to_replay") is False:
            reasons = tuple(
                str(value) for value in row.get("unresolved_reason_codes", ())
            )
            diagnostics.append(
                PayloadDiagnostic(
                    "UNRESOLVED" if reasons else "INFO",
                    (reasons[0] if reasons else "STRUCTURE_SIGNAL_NOT_ELIGIBLE_UNDER"),
        ...
        if row.get("all_required_facts_resolved", True) is not True:
            ...
            diagnostics.append(
                PayloadDiagnostic(
                    "UNRESOLVED",
                    (reasons[0] if reasons else "UNRESOLVED_STRUCTURE_SIGNAL_FACTS"),
            ...
            continue
        coverage = coverages.get(symbol)
        if coverage is None or row.get("source_ledger_sha256") != coverage.get(
        ...
        event = {
            ...
            "facts": {
                ...
                "tactical": dict(_mapping(row.get("tactical", {}), "signal.tactical")),
                "cycle_ledger": None,
                "candidate": None,
                "q_plan": 0,
    ...
    if generated_symbols:
        if corporate_action_ledger is None:
            ...
            for symbol, entry_at in sorted(generated_symbols.items()):
                ...
                if (
                    instrument is None
                    or instrument.get("status") != "EFFECTIVE_DATED_EVENTS_AVAILABLE"
                ):
                    diagnostics.append(
                        PayloadDiagnostic(
                            "UNRESOLVED",
    ...
    for index, decision in enumerate(decision_times):
        ...
        batches.append(
            {
                "batch_id": f"batch:{decision.isoformat()}",
