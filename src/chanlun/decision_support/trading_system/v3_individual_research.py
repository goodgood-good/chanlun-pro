"""Financial-data service evidence for the V3 three-program stock path.
...
INDUSTRY_CLASSIFICATION_URL = "/api/v1/stock_fnd/industry-classification"
DAILY_VALUATION_URL = "/api/v1/stock/daily-valuation-indicators"
SECTOR_VALUATION_URL = "/api/v1/sector/sector-valuation"
...
FINANCIAL_SERVICE_CATALOG_ID = sha256_json(
    {
        "schema": "chanlun-v3-financial-data-service-catalog/v1",
        "programs": {
...
class FinancialDataEvidence:
    ...
    def visible_at(self, decision_time: datetime) -> bool:
        ...
        capture_is_causal = self.captured_at <= decision or self.point_in_time_versioned
...
class IndividualResearchEvidenceBundle:
    ...
    def readiness_blockers(
        ...
        if program == "INDUSTRY_OPPORTUNITY":
            ...
            if not any(
                value.service_url
                not in {INDUSTRY_CLASSIFICATION_URL, "/api/v1/sector/sector-financial-point", "/api/v1/sector/sector-financial-cumulative"}
...
def build_individual_selection_facts(
    ...
    *,
    ...
    """Validate independent evidence lanes and emit the existing V3 snapshot."""
    ...
    if visible_evidence and market_cap_visible and known_at <= adjudication.effective_at:
        ...
        snapshot = SelectionResearchSnapshot(
            snapshot_id=sha256_json(
                {
                    "schema": "chanlun-v3-individual-selection/v1",
