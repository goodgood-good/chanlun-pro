...
STRATEGY_V3_ID = "CL-HIER-30M5M1M-v3.0"
...
class StrategyV3Parameters:
    selection_path: SelectionPath
    strategy_id: str = STRATEGY_V3_ID
    ...
    def __post_init__(self) -> None:
        if self.selection_path not in {
            ...
            raise ValueError("unsupported v3 selection path")
        if self.strategy_id != STRATEGY_V3_ID:
            raise ValueError("strategy_id is frozen")
        if self.pen_definition_mode != "ORIGINAL_OLD_PEN":
            raise ValueError("v3 only permits ORIGINAL_OLD_PEN")
        frozen_values = {
        ...
        if changed:
            raise ValueError(f"v3 frozen parameters changed: {','.join(changed)}")
        ...
        if self.fractal_ma_coarse:
            raise ValueError("FRACTAL_MA_COARSE is frozen off in v3")
        if self.allow_leverage or self.allow_short:
            raise ValueError("v3 is cash-only and long-only")
        if self.live_status != LIVE_STATUS:
            raise ValueError("v3 integration cannot enable live trading")
...
def individual_parameter_snapshot() -> StrategyV3Parameters:
    return StrategyV3Parameters("INDIVIDUAL_THREE_PROGRAM")


def etf_parameter_snapshot() -> StrategyV3Parameters:
    return StrategyV3Parameters("ETF_PROXY")
...
def parameter_snapshot_manifest() -> dict[str, object]:
    ...
    manifest = {
        "schema": "chanlun-v3-parameter-snapshots/v1",
