...
"""Causal, read-only facts for the strict V3 ``ETF_PROXY`` path.
...
def _provider_code(symbol: str) -> str:
    market, code = _canonical_symbol(symbol).split(".", 1)
...
class EtfProxyCandidateDecisionFacts:
    """Auditable envelope around the unchanged V3 candidate decision core."""
...
class RiskMappingPointEvidenceFacts:
    ...
    def identity(
        *,
        ...
        return _fingerprint(
            {
                "contract": "chanlun-risk-mapping-point-identity/v1",
...
class RiskDiagnosticBuyPointEvidenceFacts:
    ...
    def identity(
        *,
        ...
        return _fingerprint(
            {
                "contract": "chanlun-risk-diagnostic-buy-point-identity/v1",
                "source_symbol": source_symbol,
                "source_frequency": source_frequency,
                "center_id": center_id,
                "center_level_rank": center_level_rank,
...
class RiskMappingSupplyFacts:
    ...
    The frozen V3 mapping still accepts only a completed lower center carrying
    ...
    # ``None`` means a legacy count-only document.  Current builders always
    # supply a tuple (possibly empty), preserving public read compatibility
    # without pretending that old aggregates contain point identities.
    point_evidence: tuple[RiskMappingPointEvidenceFacts, ...] | None = None
    ...
    # ``None`` is reserved for legacy documents that never recorded them;
    # it must not be interpreted as an observed zero.
    diagnostic_buy_point_type_counts: tuple[tuple[str, int], ...] | None = None
    # Stable diagnostic identities are optional for backward compatibility.
    # ``None`` means counts existed without point-level provenance.
    diagnostic_buy_point_evidence: (
        tuple[RiskDiagnosticBuyPointEvidenceFacts, ...] | None
    ) = None
    ...
    def __post_init__(self) -> None:
        ...
        if self.point_evidence is not None:
            details = tuple(self.point_evidence)
            if any(
                not isinstance(value, RiskMappingPointEvidenceFacts)
            ...
            detailed_sell12 = tuple(
                value
                for value in details
                if value.point_type in {"1sell", "2sell"}
            )
            detailed_inside = tuple(
                value
                for value in detailed_sell12
                if value.inside_active_top_interval
            )
            if (
        ...
        if self.diagnostic_buy_point_type_counts is not None:
            diagnostic = tuple(self.diagnostic_buy_point_type_counts)
            if tuple(name for name, _count in diagnostic) != ("1buy", "2buy"):
                raise ValueError(
                    "diagnostic buy-point counts must be ordered 1buy/2buy"
                )
            diagnostic_counts = tuple(count for _name, count in diagnostic)
            if any(
                type(value) is not int or value < 0 for value in diagnostic_counts
            ):
                raise ValueError(
            ...
            object.__setattr__(
                self, "diagnostic_buy_point_type_counts", diagnostic
            )
        if self.diagnostic_buy_point_evidence is not None:
            if self.diagnostic_buy_point_type_counts is None:
                raise ValueError(
                    "diagnostic buy-point identities require recorded counts"
                )
            ...
            if len({value.point_id for value in diagnostic_details}) != len(
                ...
                raise ValueError(
                    "diagnostic buy-point identities must be unique"
                )
            diagnostic_detail_counts = tuple(
                sum(value.point_type == point_type for value in diagnostic_details)
                for point_type in ("1buy", "2buy")
            )
            recorded_diagnostic_counts = tuple(
                count for _name, count in self.diagnostic_buy_point_type_counts
            )
            if diagnostic_detail_counts != recorded_diagnostic_counts:
                raise ValueError(
                    "diagnostic buy-point identities do not reconcile"
                )
            object.__setattr__(
                self, "diagnostic_buy_point_evidence", diagnostic_details
            )
    ...
    def from_document(cls, raw: object) -> "RiskMappingSupplyFacts":
        ...
        diagnostic_evidence_field = {"diagnostic_buy_point_evidence"}
        allowed = {
            frozenset(expected),
            frozenset(expected | {"point_evidence"}),
            frozenset(expected | diagnostic_fields),
            frozenset(expected | {"point_evidence"} | diagnostic_fields),
            frozenset(expected | diagnostic_fields | diagnostic_evidence_field),
            frozenset(
                expected
                | {"point_evidence"}
                | diagnostic_fields
                | diagnostic_evidence_field
            ),
        }
        if not isinstance(raw, Mapping) or set(raw) not in allowed:
        ...
        if "diagnostic_buy_point_type_counts" in raw:
            ...
            if any(
                type(raw_diagnostic.get(name)) is not int
                for name in ("1buy", "2buy")
            ):
                raise ValueError("diagnostic buy-point counts are malformed")
        ...
        if "diagnostic_buy_point_evidence" in raw:
        ...
        if diagnostic_counts is not None and raw.get(
            "diagnostic_directional_classification"
    ...
    revision = _fingerprint(
        {
            ...
            "mode": "QMT_EFFECTIVE_DR_CAUSAL_FORWARD_V1",
        }
    )
    return CausalAdjustedStructureBars(
        bars=tuple(output),
...
def select_unique_top_center_mapping(
    ...
    *,
    ...
    Both high-fractal interval boundaries are inclusive, matching the V3
...
def _risk_mapping_supply_facts(
    ...
    *,
    ...
    if (source_symbol is None) != (source_frequency is None):
        raise ValueError("risk mapping point source identity must be complete")
    if any(
    ...
    point_evidence = None
    diagnostic_point_evidence = None
    if source_symbol is not None and source_frequency is not None:
        point_evidence = tuple(
...
def build_etf_proxy_candidate_decision(
    ...
    *,
    ...
    facts as required by V3; this adapter never aliases the broad-market risk
    into those labels.
    """
    ...
    if sector_risk is None:
        blockers.append(
            FactBlocker(
                ...
                "V3 forbids aliasing broad-market risk into sector risk",
            )
        )
