...
class ComparisonLeg:
    ...
    @property
    def price_anchor_unit(self) -> ConstituentUnit:
        # Keep the structural boundary at the terminal unit, but locate the
        # comparison envelope's extreme on its actual source candle. On equal
        # extremes use the last occurrence, consistently with a final retest.
        side = "sell" if self.terminal_unit.direction == "up" else "buy"
        return max(self.units, key=lambda unit: (
            unit.high_tick if side == "sell" else -unit.low_tick,
            unit.price_anchor_time(side),
            unit == self.terminal_unit,
        ))
...
def compare_terminal_trend_divergence(
    ...
    *,
    ...
    """比较末中枢的进入 b 与完整离开 c；b 可以短于 c。"""

    values = tuple(centers)
    if len(values) < 2 or provider is None:
        return None
    # Lesson 37: c must contain the terminal center's third-class departure.
    # A weakening pending leave alone is only a consolidation comparison.
    terminal_center = values[-1]
    if not terminal_center.physically_completed:
        return None
    ...
    if (
        earlier is None
        or later is None
        or terminal_center.completion_direction != later.measurement_unit.direction
        or terminal_center.completion_return_unit.unit_id
        not in tuple(unit.unit_id for unit in later.units)
...
def compare_comparison_legs(
    ...
    *,
    ...
    if (earlier_leg.width != later_leg.width
            and not (kind == "trend" and earlier_leg.width == 1 and later_leg.width == 3)):
        raise ValueError("only a trend may compare a one-unit entry with a complete three-unit departure")
    ...
    return DivergenceEvidence(
        ...
        price_anchor_unit_id=later_leg.price_anchor_unit.unit_id,
        anchor_at=later_leg.price_anchor_unit.price_anchor_time("sell" if later.direction == "up" else "buy"),
        anchor_tick=(
            later_leg.price_anchor_unit.high_tick
            if later.direction == "up"
            else later_leg.price_anchor_unit.low_tick
        ),
