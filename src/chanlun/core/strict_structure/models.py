...
from chanlun.core.strict_structure.identity import (build_center_id, build_trend_id, build_strict_evidence_revision, stable_structure_id)

Direction = Literal["up", "down"]
StrictPointType = Literal["1buy", "2buy", "3buy", "1sell", "2sell", "3sell"]
STRICT_POINT_TYPES = ("1buy", "2buy", "3buy", "1sell", "2sell", "3sell")
STRICT_POINT_TYPE_SET = frozenset(STRICT_POINT_TYPES)
...
class ConstituentUnit:
    ...
    def price_anchor_time(self, side: str) -> datetime:
        """买卖点必须使用其价格极值的发生时间，不能借用结构结束时间。"""
        if side not in ("buy", "sell"):
            raise ValueError("point side must be buy or sell")
        moment = self.extreme_market_time("low" if side == "buy" else "high")
        if moment is None:
            raise ValueError("point price extreme requires source time evidence")
        return moment
...
def combined_extreme_market_time(units, side: str) -> datetime | None:
    """组合保留最后一次真实极值的来源时间；缺失来源不能臆造时间。"""
    if side not in ("low", "high"):
        raise ValueError("extreme side must be low or high")
    values = tuple(units)
    extreme = (min if side == "low" else max)(getattr(u, side + "_tick") for u in values)
    moments = tuple(u.extreme_market_time(side) for u in values if getattr(u, side + "_tick") == extreme)
...
class TrendCenter:
    ...
    def _validate_recursive(self) -> None:
        ...
        object.__setattr__(self, 'failed_departure_units', tuple(self.failed_departure_units))
        object.__setattr__(self, 'supersession_bridge_units', tuple(self.supersession_bridge_units))
        ...
        if len(self.initial_units) != expected_initial_count:
            raise ValueError('initial_units must contain the source-specific center body')
        ...
        if any((item.structural_level != self.structural_level or item.source_kind is not self.source_kind for item in self.body_units)):
            raise ValueError('center body level/source mismatch')
        if any((item.price_basis_revision != self.price_basis_revision for item in self.body_units)):
        ...
        if self.entry_unit is not None:
            if self.entry_unit.structural_level != self.structural_level or self.entry_unit.source_kind is not self.source_kind:
        ...
        if establishment_leave is not None:
            raise ValueError('recursive trend-type center has no physical establishment leave')
        ...
        if any((not self._overlaps_core(item) for item in self.body_units)):
            raise ValueError('each center body unit must overlap the frozen center core')
        if len({item.unit_id for item in self.body_units}) != len(self.body_units):
            raise ValueError('center body unit ids must be unique')
        if any((current.market_start < previous.market_end for (previous, current) in zip(self.body_units, self.body_units[1:]))):
        ...
        if self.established_market_time != maturity_unit.market_end:
            raise ValueError('established market time must equal final initial body unit end')
        if self.established_at != maturity_unit.confirmed_at:
            raise ValueError('established_at must equal final initial body unit confirmation')
        ...
        center_evidence = (() if self.entry_unit is None else (self.entry_unit,)) + self.body_units + self.failed_departure_units + (() if self.establishment_leave_unit is None else (self.establishment_leave_unit,))
        ...
        for terminal in (self.establishment_leave_unit, *self.failed_departure_units, self.pending_leave_unit, self.completion_leave_unit, self.completion_return_unit):
            if terminal is not None and (terminal.structural_level != self.structural_level or terminal.source_kind is not self.source_kind):
                raise ValueError('center lifecycle unit level/source mismatch')
            if terminal is not None and terminal.price_basis_revision != self.price_basis_revision:
        ...
        if any((not self._touches_core(item) or not self._outside_in_direction(item) for item in self.failed_departure_units)):
            raise ValueError('failed departure history has invalid leave geometry')
        if set((item.unit_id for item in self.failed_departure_units)) & set((item.unit_id for item in self.body_units)):
            raise ValueError('failed departures must stay outside center body')
        if self.entry_unit is not None and self.entry_unit.unit_id in {item.unit_id for item in self.failed_departure_units}:
        ...
        def validate_pending_leave() -> None:
            ...
            if self.available_at < pending.available_at:
                raise ValueError('center availability must cover pending leave evidence')
        ...
        def validate_completion() -> None:
            ...
            if False and leave.direction == ret.direction:
            ...
            if self.available_at < max(self.completed_at, leave.available_at, ret.available_at):
                raise ValueError('center availability must cover completion evidence')
        has_supersession = self.superseded_by_center_id is not None or self.superseded_at is not None or bool(self.supersession_bridge_units)
        ...
        if self.state is CenterState.ONGOING:
            if self.completion_leave_unit is not None or self.completion_return_unit is not None or self.completed_at is not None:
                raise ValueError('ongoing center cannot retain completion evidence')
            validate_pending_leave()
            if self.boundary_divergence_id is not None or self.boundary_anchor_unit_id is not None:
            ...
            if self.pending_leave_unit is not None or self.completion_leave_unit is not None or self.completion_return_unit is not None or (self.completed_at is not None):
                raise ValueError('superseded center cannot fabricate third-class completion')
            if self.boundary_divergence_id is not None or self.boundary_anchor_unit_id is not None:
                raise ValueError('superseded center cannot carry boundary evidence')
            if not self.superseded_by_center_id or self.superseded_by_center_id == self.center_id or self.superseded_at is None:
                raise ValueError('superseded center requires a distinct successor and close time')
            ...
            if self.available_at < self.superseded_at:
                raise ValueError('superseded center availability must cover successor evidence')
            bridge = self.supersession_bridge_units
            if any((item.structural_level != self.structural_level or item.source_kind is not self.source_kind or item.price_basis_revision != self.price_basis_revision or (not item.locked) for item in bridge)):
                raise ValueError('supersession bridge context is incompatible')
            if {item.unit_id for item in bridge} & {item.unit_id for item in (*self.body_units, *self.failed_departure_units)}:
                raise ValueError('supersession bridge must stay outside body and failed history')
            if bridge and self.available_at < max((item.available_at for item in bridge)):
                raise ValueError('superseded center availability must cover bridge evidence')
        else:
            if not self.boundary_divergence_id or not self.boundary_anchor_unit_id:
                raise ValueError('divergence-closed center requires divergence and anchor ids')
            ...
            if has_pending == has_completion:
                raise ValueError('divergence closure requires either pending or completed leave proof')
            if has_pending:
                if self.completion_return_unit is not None or self.completed_at is not None:
                    raise ValueError('pending divergence closure cannot retain completion evidence')
                validate_pending_leave()
            else:
                validate_completion()
        if establishment_leave is not None:
            lifecycle_owners = (*self.failed_departure_units, *self.supersession_bridge_units, *(() if self.pending_leave_unit is None else (self.pending_leave_unit,)), *(() if self.completion_leave_unit is None else (self.completion_leave_unit,)))
            establishment_owners = tuple((item for item in lifecycle_owners if item.unit_id == establishment_leave.unit_id))
            if establishment_owners != (establishment_leave,):
                raise ValueError('physical establishment leave must belong to exactly one lifecycle role')
        transition_units = (*self.body_units, *self.failed_departure_units, *self.supersession_bridge_units, *(() if self.pending_leave_unit is None else (self.pending_leave_unit,)), *(() if self.completion_leave_unit is None else (self.completion_leave_unit,)), *(() if self.completion_return_unit is None else (self.completion_return_unit,)))
        ...
        if self.entry_unit is not None and self.entry_unit.unit_id in set(transition_ids):
            raise ValueError('center entry must stay outside transition ownership')
        ordered_transitions = tuple(sorted(transition_units, key=lambda item: (item.market_start, item.market_end, item.unit_id)))
        if not ordered_transitions or ordered_transitions[0] != self.body_units[0]:
            raise ValueError('center transition history must start at its first body unit')
        ...
        if tuple((item for item in ordered_transitions if item.unit_id in body_ids)) != self.body_units:
            raise ValueError('center body order conflicts with physical transitions')
        if tuple((item for item in ordered_transitions if item.unit_id in failed_ids)) != self.failed_departure_units:
            raise ValueError('failed departure order conflicts with physical transitions')
        if tuple((item for item in ordered_transitions if item.unit_id in bridge_ids)) != self.supersession_bridge_units:
            raise ValueError('supersession bridge order conflicts with physical transitions')
        physical_chain = (*((self.entry_unit,) if self.entry_unit is not None else ()), *ordered_transitions)
        for (previous, current) in zip(physical_chain, physical_chain[1:]):
            pair_label = 'center body' if previous.unit_id in body_ids and current.unit_id in body_ids else 'center transition'
            if False and previous.direction == current.direction:
        ...
        transition_offset = {item.unit_id: offset for (offset, item) in enumerate(ordered_transitions)}
        ...
        expected_center_id = build_center_id(price_basis_revision=self.price_basis_revision, structural_level=self.structural_level, source_kind=self.source_kind.value, entry_unit_id=None if self.entry_unit is None else self.entry_unit.unit_id, initial_unit_ids=tuple((item.unit_id for item in self.initial_units)), establishment_leave_unit_id=None if self.establishment_leave_unit is None else self.establishment_leave_unit.unit_id, zd_tick=self.zd_tick, zg_tick=self.zg_tick)
        if self.center_id != expected_center_id:
            raise ValueError('center_id must match the immutable center seed')

    def __post_init__(self) -> None:
        if SourceKind(self.source_kind) is SourceKind.TREND_TYPE:
            return self._validate_recursive()
        object.__setattr__(self, "source_kind", SourceKind(self.source_kind))
        object.__setattr__(self, "state", CenterState(self.state))
...
class CenterPreview:
    ...
    def _validate_recursive(self) -> None:
        ...
        object.__setattr__(self, 'failed_departure_unit_ids', tuple(self.failed_departure_unit_ids))
        if type(self.structural_level) is not int or self.structural_level < 0:
            raise ValueError('structural_level must be >= 0')
        if not self.price_basis_revision or not self.price_basis_revision.strip():
            raise ValueError('price_basis_revision is required')
        ...
        if len(set(self.failed_departure_unit_ids)) != len(self.failed_departure_unit_ids):
        ...
        if self.failed_departure_unit_ids and (len(self.unit_ids) < 3 or self.zd_tick is None or self.zg_tick is None or (self.state is CenterPreviewState.TOUCH_ONLY)):
            raise ValueError('preview failed departures require a source-valid center core')
        if (self.zd_tick is None) != (self.zg_tick is None):
            raise ValueError('preview core ticks must be both present or both absent')
        if self.state is CenterPreviewState.TOUCH_ONLY and (self.zd_tick is None or self.zd_tick != self.zg_tick):
            raise ValueError('touch-only preview requires a zero-width core')
        if self.state is CenterPreviewState.FORMING and self.zd_tick is not None and (self.zd_tick > self.zg_tick):
            raise ValueError('forming preview core violates source overlap contract')
        if self.state is CenterPreviewState.COMPLETED:
            if len(self.unit_ids) < 3 or self.zd_tick is None or self.zg_tick is None or (self.zd_tick > self.zg_tick):
                raise ValueError('completed preview requires a source-valid center core')
        ...
        elif self.completion_leave_unit_id is not None or self.completion_return_unit_id is not None:
            raise ValueError('non-completed preview cannot retain completion evidence')
        lifecycle_ids = tuple((value for value in (self.pending_leave_unit_id, self.completion_leave_unit_id, self.completion_return_unit_id) if value is not None))
        ...
        if set(lifecycle_ids) & set(self.failed_departure_unit_ids):
            raise ValueError('preview current lifecycle must stay outside failed history')
        ...
        if self.establishment_leave_unit_id is not None:
            raise ValueError('trend-type preview has no physical establishment leave')
    ...
    def __post_init__(self) -> None:
        ...
        for center_offset, center in enumerate(self.centers):
            ...
            if missing:
                ...
                # a superseded center to the successor movement.
                ...
                superseded_tail_is_external = (
                    ...
                    and (
                        center.source_kind is SourceKind.TREND_TYPE
                        or (
                            center.source_kind is SourceKind.SEGMENT
                            and bool(center.supersession_bridge_units)
                        )
                    )
                    and center_offset == len(self.centers) - 1
        ...
        if self.terminal_divergence is None:
            ...
            anchor_source = unit_by_id.get(divergence.price_anchor_unit_id)
            if anchor_source not in signal_leg:
                raise ValueError("divergence price anchor is outside its signal leg")
            ...
            if (
                ...
                or anchor_source.price_anchor_time("buy" if self.direction == "down" else "sell") != divergence.anchor_at
                or (anchor_source.high_tick if self.direction == "up" else anchor_source.low_tick) != signal_extreme
                or signal_extreme != divergence.anchor_tick
    ...
    def __post_init__(self) -> None:
        ...
        for boundary in boundaries:
            ...
            if (
                ...
                or boundary.divergence.anchor_at != boundary.anchor_at
            ):
                raise ValueError(
...
class StrictEvidenceResult:
    ...
    def __post_init__(self) -> None:
        ...
        for point in all_points:
            ...
            anchors = tuple(
                unit for unit in level.units if unit.unit_id == point.price_anchor_unit_id
            )
            if len(anchors) != 1:
            ...
            if (
                ...
                or anchor.price_anchor_time(point.side) != point.anchor_at
        ...
        for point in self.approaching_points:
            ...
            if (parent.status is not StrictPointStatus.CONFIRMED
                    and "parent_first_class_confirmed" not in point.missing_conditions):
                raise ValueError("approaching second-class must disclose its pending first-class parent")
        ...
        boundaries = tuple(
            ...
            for level in self.structure.levels
            for boundary in level.decomposition_boundaries
            if boundary.divergence.kind == "trend"
        )
    ...
    def _validate_small_to_large_second(
        ...
        *,
        ...
        if (
            ...
            or signal.price_anchor_time(point.side) != parent.anchor_at
        ...
        if parent.price_anchor_unit_id not in signal_descendants:
...
class DivergenceEvidence:
    ...
    price_anchor_unit_id: str | None = None
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "price_anchor_unit_id", self.price_anchor_unit_id or self.signal_unit_id)
        if self.price_anchor_unit_id not in signal_leg_ids:
            raise ValueError("divergence price anchor must belong to its signal leg")
        ...
        if (
            ...
            or len(signal_leg_ids) not in (1, 3)
            or (len(signal_leg_ids) != len(compare_leg_ids)
                and not (self.kind == "trend" and len(compare_leg_ids) == 1 and len(signal_leg_ids) == 3))
            ...
            raise ValueError(
                "divergence requires complete legs; only trend allows one-unit entry and three-unit departure"
            )
    ...
    def comparison_width(self) -> int:
        """Number of source units in the earlier comparison leg (b)."""
        return len(self.compare_leg_unit_ids)

    @property
    def signal_width(self) -> int:
        """Number of source units in the later signal leg (c)."""
        return len(self.signal_leg_unit_ids)
...
class StrictPointEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        if self.point_type in {"1buy", "1sell"} and (
            ...
            raise ValueError("一类买卖点必须来自本级趋势背驰，盘整背驰不是一类点")
    ...
    @property
    def price_anchor_unit_id(self) -> str:
        """Price provenance; anchor_unit_id retains the structural carrier."""
        if self.point_type in {"1buy", "1sell"} and self.divergence is not None:
            return self.divergence.price_anchor_unit_id
