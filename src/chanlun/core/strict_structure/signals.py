...
def _divergence_evidence_codes(divergence) -> tuple[str, ...]:
    codes = [
        ...
        f"signal_leg_width_{divergence.signal_width}",
...
class StrictSignalEngine:
    ...
    def _projected_level_zero_points(
        ...
        *,
        ...
        cache_key = (
            "strict-level-zero-projected-points-v3",
    ...
    def _convert_projected_point_graph(
        ...
        *,
        ...
        def convert(point_id: str) -> StrictPointEvidence:
            ...
            parent_pending = parent is not None and parent.status is not StrictPointStatus.CONFIRMED
            ...
            value = replace(
                ...
                evidence_codes=tuple(
                    dict.fromkeys(
                        (*(
                            code.replace("confirmed_first_class_parent", "formed_first_class_parent")
                            .replace("confirmed_lower_level_first_class_parent", "formed_lower_level_first_class_parent")
                            if parent_pending else code
                            for code in raw.evidence_codes
                ...
                missing_conditions=(
                    ...
                    *(("parent_first_class_confirmed",) if parent_pending else ()),
                ),
            )
    ...
    def _approaching_first_class(
        ...
        for trend in reversed(level.trend_types):
            ...
            if (
                ...
                or divergence.anchor_at != tail.price_anchor_time("buy" if divergence.direction == "down" else "sell")
            ...
            return StrictPointEvidence(
                ...
                price_basis_revision=trend.price_basis_revision,
                anchor_unit_id=tail.unit_id,
                anchor_at=tail.price_anchor_time(side),
                confirmed_at=None,
                available_at=max(
                    trend.available_at,
                    last_center.available_at,
    ...
    def _approaching_second_class(
        ...
        for parent in first_points:
            if parent.structural_level != level.structural_level:
                continue
            sequence = self._first_retest_units(level, parent)
            if sequence is None:
                continue
            signal, rebound, pullback = sequence
            if (
    ...
    def _approaching_small_to_large_second_class(
        ...
        for parent in first_points:
            ...
            matches = [
                ...
                if self._contains_lower_anchor(unit, levels, parent.price_anchor_unit_id)
            ]
            ...
            if (
                ...
                or signal.extreme_market_time("low" if parent.side == "buy" else "high") != parent.anchor_at
    ...
    @staticmethod
    def _first_retest_units(level, parent):
        """Find the first same-level retest after the actual first-point extreme.

        A composite proof can become available only after that retest finishes.
        The retest inherits the parent's confirmation time; a later retest cannot
        replace it. An internal extreme inside an opposite-direction unit still
        needs finer geometry before claiming a complete same-level rebound.
        """
        matches = [index for index, unit in enumerate(level.units)
                   if unit.unit_id == parent.price_anchor_unit_id]
        if len(matches) != 1:
            raise ValueError("first-class price anchor must occur once in level units")
        index = matches[0]
        direction = "down" if parent.side == "buy" else "up"
        if level.units[index].direction != direction:
            # A shared endpoint can be attributed to either adjacent unit.
            index -= 1
        if index < 0 or len(level.units) < index + 3:
            return None
        signal, rebound, pullback = level.units[index:index+3]
        extreme = signal.low_tick if parent.side == "buy" else signal.high_tick
        if (signal.direction != direction or extreme != parent.anchor_tick
                or signal.price_anchor_time(parent.side) != parent.anchor_at):
            return None
        return signal, rebound, pullback
    ...
    def _approaching_second_class_point(
        ...
        *,
        ...
        return StrictPointEvidence(
            ...
            price_basis_revision=parent.price_basis_revision,
            anchor_unit_id=pullback.unit_id,
            anchor_at=pullback.price_anchor_time(parent.side),
            confirmed_at=None,
            available_at=max(
                parent.available_at,
            ...
            missing_conditions=(
                ...
                *(("parent_first_class_confirmed",) if not parent_is_confirmed else ()),
            ),
        ...
        *,
        ...
        for level in calculation_structure.levels:
            for trend in level.completed_trends:
                ...
                if divergence is None or divergence.kind != "trend":
                    continue
    ...
    def _second_class_point(
        ...
        *,
        ...
        return StrictPointEvidence(
            ...
            price_basis_revision=parent.price_basis_revision,
            anchor_unit_id=pullback.unit_id,
            anchor_at=pullback.price_anchor_time(side),
            ...
            evidence_codes=(
                ...
                *(("internal_first_retest",) if parent.price_anchor_unit_id != parent.anchor_unit_id else ()),
                (
                    "prior_extreme_held"
                    if variant is StrictPointVariant.STRICT
    ...
    def _third_class_point(
        ...
        *,
        ...
        return StrictPointEvidence(
            ...
            price_basis_revision=center.price_basis_revision,
            anchor_unit_id=ret.unit_id,
            anchor_at=ret.price_anchor_time(side),
