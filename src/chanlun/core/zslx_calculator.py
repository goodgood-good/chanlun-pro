"""zslx_calculator.py — 级别无关的缠论走势类型划分。

用双信号状态机把中枢序列切成走势类型(ZSLX)：边界由「背驰」或「中枢方向
断裂」任一触发。盘整 = 1 中枢；趋势 = ≥2 依次同向中枢。复用背驰内核。
"""
...
def _wt_beichi(
    ...
    离开段 = 最新中枢的最后一段核心(zss[-1].lines[-1]，离开段已计入核心)。
    ≥2 中枢走趋势背驰 beichi_qs，1 中枢走盘整背驰 beichi_pz。
    ``frequency`` 透传到背驰内核决定黄白线口径。
    """
...
def _finalize(
    ...
    # 回填中枢方向：中枢方向由其**构成**决定——向上走势里的中枢=下-上-下、
    # 向下=上-下-上，即「核心首段的反向」。
    #   - 趋势走势类型：继承趋势方向(与各中枢构成一致)。
    #   - 盘整走势类型：单中枢仍有构成方向(从高/低点起算),不应统一抹成 zd——否则
    #     「向上=下上下 / 向下=上下上」无从体现。
    # 中枢方向**一律由构成判定**(核心首段的反向)：下-上-下=向上、上-下-上=向下。
    # 正确划分下,这与所在趋势方向一致;若某趋势走势类型里的中枢构成与趋势相反,
    # 说明该中枢划分有误(更深的"走势类型-aware 划分"问题),此处按构成如实标注
...
class ZslxCalculator:
    ...
    def backfill_zs_directions(self, zss: List[ZS]) -> None:
        """笔层中枢方向回填——纯 per-zs 副作用:zs.type = 核心首段方向的反向
        (向上走势的中枢=下-上-下→type=up)。**不依赖走势类型分组**:正常中枢
