# Read-only audit of the app.py-owned V3 forward scheduler after migration.
[CmdletBinding()]
param(
...
$QmtOwnerPath = Join-Path $ProjectRoot '.cache\chanlun_v3_scheduler\qmt_execution_owner.json'
...
if ($null -eq $qmtRuntime) {
    ...
    if ($qmtRuntime.schema -ne 'chanlun-qmt-runtime-readiness/v1') {
        Add-Reason 'APP_QMT_SCHEMA_MISMATCH'
    }
    if ($qmtRuntime.contract_id -ne 'chanlun-qmt-runtime/app-runtime-contract/v1') {
...
if ($null -eq $forward) {
    ...
    if ($forward.schema -ne 'chanlun-v3-forward-scheduler-readiness/v1') {
        Add-Reason 'APP_FORWARD_SCHEMA_MISMATCH'
    }
    if ($forward.contract_id -ne 'chanlun-v3-forward-scheduler/app-runtime-contract/v1') {
...
if ($legacyPresent.Count -gt 0) {
    Add-Reason 'LEGACY_FORWARD_TASKS_STILL_PRESENT'
}
$qmtTask = Get-ScheduledTask `
    -TaskName 'Chanlun-QMT-DailyRestart' `
    -ErrorAction SilentlyContinue
if ($null -ne $qmtTask) {
    Add-Reason 'LEGACY_QMT_TASK_STILL_PRESENT'
}

$ready = $Reasons.Count -eq 0
$result = [ordered]@{
    schema = 'chanlun-v3-app-forward-scheduler-readiness/v1'
    ...
    legacy_forward_tasks_present = @($legacyPresent)
    legacy_qmt_task_present = $null -ne $qmtTask
    legacy_qmt_task_name = 'Chanlun-QMT-DailyRestart'
