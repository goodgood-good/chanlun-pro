(function (root, factory) {
  ...
  function boot() {
    ...
    const state = {data:null,serial:0,version:'',rows:[],selected:null,watch:[],quotes:new Map(),
      ...
      mutationPending:false,settingsInitialized:false,settingsEdited:false,
      reviewsLoadedSource:null,reviewSerial:0,reviewWriteEpoch:0,savingReview:false,
      notifications:null,notificationLimit:40,stopChartSync:null,chartPeriods:[]};
    function node(tag, text, className) { const el=document.createElement(tag); if(text!==undefined)el.textContent=text;if(className)el.className=className;return el; }
    function message(text) {$('page-message').textContent=text;$('page-message').hidden=!text;}
    ...
    const version = s => [s.run_id,s.status,s.completed,s.cancel_requested,s.source_current,s.cutoff_current,s.freshness_state,s.evidence_version].join(':');
    ...
    function renderSummary() {
      ...
      $('symbol-count').textContent=new Set(candidates.map(r=>r.market+':'+r.code)).size;
      $('signal-count').textContent=candidates.length;
      const waiting=candidates.filter(isWaiting).length;
      $('stage-counts').textContent=`已确认 ${candidates.length-waiting} · 形成与等待 ${waiting}`;
      ...
      $('point-counts').replaceChildren(...Object.entries(POINTS).map(([p,label])=>node('span',`${label} ${state.data.point_counts[p]||0}`)));
    ...
    function renderQueue() {
      ...
      for (const row of state.rows.slice(0,state.queueLimit)) {
        ...
        b.append(title,node('small',`${row.code} · ${row.frequency} · ${STAGES[row.stage]||row.stage}`),
          node('small',row.origin==='watchlist'?'人工关注 · 点击查看图表':`拐点 ${date(row.point.anchor_at)}`),
          node('small',row.origin==='watchlist'?'':`${isWaiting(row)?'观测':'确认'} ${date(row.point.available_at)}`),
      ...
      $('queue-empty').textContent='最新一轮结果中没有符合当前过滤条件的候选。可重置筛选、切换关注组，或查看上方选股进度及下方数据诊断。';
    ...
    function selectRow(row) {
      ...
      $('selected-tags').textContent=[row.frequency,pointLabel(p),STAGES[row.stage]||row.stage].filter(Boolean).join(' · ');
      const current=state.data.status.source_current===true&&state.data.status.cutoff_current===true;
      $('decision-title').textContent=row.origin==='watchlist'?'人工关注 · 查看当前结构':isWaiting(row)?(nested?'形成与等待 · 5m 或 1m 尚缺确认':'形成与等待 · 买点尚未确认'):current?(nested?'5m 买卖点与 1m 区间套确认通过':'本次筛选通过 · 待人工判断'):'本次结果待更新';
      $('decision-detail').textContent=row.origin==='watchlist'?'关注组不代表买卖点确认。':
        `拐点 ${date(p.anchor_at)}；${isWaiting(row)?'观测':'确认'} ${date(p.available_at)}。${!isWaiting(row)&&row.confirmation_delay_sessions!==undefined?`确认间隔 ${row.confirmation_delay_sessions} 个交易日。`:''}`+
      ...
      if(p.center_zd_price!==undefined&&p.center_zd_price!==null)established.unshift(`中枢区间 [${price(p.center_zd_price)}, ${price(p.center_zg_price)}]${p.center_ordinal?` · 第 ${p.center_ordinal} 个中枢`:''}`);
    ...
    function renderChart(force=false) {
      ...
      $('open-chart').href=url;
      $('open-chart').textContent='打开完整画图工作台';
    ...
    function screeningSettings() {
      ...
      return {scope,codes:scope==='codes'?$('codes').value.toUpperCase().split(/[\s,，;；]+/).filter(Boolean):[],
        ...
        recent_sessions:Number($('recent').value),max_anchor_sessions:Number($('anchor').value),
      ...
      try {
        if(!force) {
          ...
          if(version(status)===state.version){
            if(state.reviewsLoadedSource!==state.data?.source){await loadReviews(state.data.source,serial);message('');}
            return;
          }
        ...
        if(serial!==state.serial)return;
        showResults(data);state.version=version(data.status);message('');
    ...
    function renderNotifications(){
      const filter=$('notification-filter').value,query=$('notification-search').value.trim().toUpperCase();
      const rows=(state.notifications||[]).filter(r=>(filter==='all'||r.status===filter)&&(!query||String(r.code||'').includes(query)));
      $('notification-count').textContent=`${rows.length} 条历史记录`;
      $('notification-list').replaceChildren(...rows.slice(0,state.notificationLimit).map(row=>{
        const article=node('article'),link=node('a',row.code||'未知标的');link.href=chartUrl({code:row.code,market:row.market||'a'});link.target='_blank';link.rel='noopener';
        article.append(link,node('span',` · ${POINTS[row.setup_point_type]||'结构变化'} · ${STAGES[row.old_stage]||row.old_stage||'—'} → ${STAGES[row.new_stage]||row.new_stage||'—'}`),
          node('p',`${date(row.recorded_at)} · ${{delivered:'已发送',suppressed:'未发送'}[row.status]||row.status||'状态未知'} · ${row.reason||''}`,'muted'));return article;
      }));$('notification-more').hidden=state.notificationLimit>=rows.length;
    }
    $('notifications-panel').addEventListener('toggle',async()=>{
      if(!$('notifications-panel').open||state.notifications!==null)return;
      try{const data=await api('/screening/notifications');state.notifications=data.events;renderNotifications();}catch(error){$('notification-count').textContent=error.message;}
    });
    $('notification-filter').addEventListener('change',renderNotifications);$('notification-search').addEventListener('input',renderNotifications);
    $('notification-more').addEventListener('click',()=>{state.notificationLimit+=100;renderNotifications();});
    ...
    for(const [id,key] of [['market-filter','market'],['origin-filter','origin'],['frequency-filter','frequency'],['review-filter','review'],['stage-filter','stage']])$(id).addEventListener('change',()=>{state.filters[key]=$(id).value;state.queueLimit=100;renderQueue();});
    ...
    $('reset').addEventListener('click',()=>{
      ...
      for(const id of ['market-filter','frequency-filter','review-filter','stage-filter'])$(id).value='all';$('origin-filter').value='screening';syncPoints();renderSectors();renderQueue();
    });
    ...
    $('chart-reset-size').addEventListener('click',()=>{$('chart-frame').parentElement.style.height='';});
    ...
    $('toggle-list').addEventListener('click',()=>{const hidden=$('workspace').classList.toggle('list-hidden');$('toggle-list').textContent=hidden?'展开列表':'收起列表';$('toggle-list').setAttribute('aria-pressed',String(hidden));});
    function theater(on){document.body.classList.toggle('theater',on);$('theater').textContent=on?'退出影院':'影院模式';$('theater').setAttribute('aria-pressed',String(on));}
